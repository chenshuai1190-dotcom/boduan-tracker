...
v10.7.9.43  🧠 (06-10) 预警文案理性化 (保留进攻性)          ← 当前
            - L6 留应急弹药 / L8 先核维持率再进攻 / L9 强调分批
            - FGI "梭哈"→分批进攻, "清仓"→留核心仓
v10.7.9.42  💰 (06-10) 资产走势 Modal 黑金化 + 环比金额
v10.7.9.41  🎯 (06-10) 修复猎手状态 QQQ 回撤拉取不到
            - QQQ 没进请求列表 + 52周高写死 640.47, 双重 bug
            - 现 QQQ/TQQQ 走完整接口, 用真实 week52High
v10.7.9.40  EODHD All-In-One 升级 + 重要日历 + 公司 Modal 重做 + 摊薄成本
v10.7.8.6   复盘改名"目标" + 更新日志折叠
...
8. QQQ/TQQQ 是"核心参数"不是普通 watchlist 股 (v10.7.9.41 教训)

   猎手状态默认基准是 QQQ, 但 QQQ 必须显式加进 API 请求列表
   错: symbols = [...watchlist, 'VIX','FGI','INDICES']  → QQQ 不在里面
   对: symbols = [...new Set([...watchlist, 'QQQ','TQQQ']), 'VIX','FGI','INDICES']
   注意: INDICES 接口里也有 QQQ 但只返回当日高 (dayHigh), 没有 52周高
        算回撤必须用完整 stock 接口的 week52High

9. qqqHigh 不要写死 + 不要 Math.max(prev) 粘滞 (v10.7.9.41 教训)

   错: setQqqHigh(prev => Math.max(prev, 当前价))  → 永远停在初始 640.47
   对: setQqqHigh(Math.max(apiWeek52High, 当前价))  → 信任 API
   原因: API 的 week52High 本身就是滚动 52 周最高, 直接用即可
        用 max(prev) 会被脏数据顶死降不下来
