...
- 股票决策位于交易的全部功能中，输入代码后才读取已鉴权 `/api/quote?view=stock-decision`，不增加首页请求、不读取持仓、不产生交易。四项判断为趋势、位置、量能、动量；统一使用 EODHD 已完成日线复权 OHLC，成交量保留供应商拆股调整口径，RSI 复用既有算法。支撑压力由近 120 个交易日左右各 3 根确认的枢轴形成区域，0.5 ATR(14) 聚合容差在识别时固定，独立触及至少间隔 5 根；保留原支撑破位，不用更低区域掩盖。放量突破保存事前已知区域和当日相对前 20 日均量，次日量能回落不直接否定突破；收盘失守区域下沿或超过 10 个后续交易日失效。此工具不请求财报日历，财报不参与结论；已知技术风险优先显示，核心数据不足不能生成“可进一步观察”。阈值为 v1 观察规则，未验证预测收益。日线按代码和完成日缓存，失败退避及旧数据状态独立于正式行情和所有收益账本。
- 股票决策的财报估值通过已鉴权 `/api/quote?view=stock-valuation` 独立读取，不影响上述四项技术结论。首批仅 MSFT、NVDA、META；查询时发现最新 SEC 业绩报告，解析已公开实际值及指引，按版本化规则滚动预测后四个财季，生成三档 EPS 与20／25／30倍 PE 敏感性价格，不能标为公司盈利预测、技术支撑、公允价格或买入建议。结果按代码及财报身份缓存10分钟，页面打开时独立检查，失败退避、不回填过期旧情景；新报告尚未解析完成则显示待更新。价格使用同一 EOD 日线的实际 `close`，不以含分红复权价配合每股盈利。最新股数口径、SBC、税项与预测假设必须可核验，缺失不置零；不增加首页行情请求、不读取或写入任何个人账本。
...
- 首页自选 RSI(6) 复用股票行情已经获取的 EODHD 完成日线，以 `adjusted_close` 按 Wilder 方法计算，不新增指标请求或刷新定时器，不参与资产、盈亏或首页就绪判断。80/90 为超买/严重超买、20 为超卖的展示分界；顶背离独立按复权收盘价的两个已确认高点判断（左右各 2 根、间隔 5–60 根，价更高而 RSI 更低），确认后最多保留 10 根日线，突破第二高点即失效。缺失数据不显示为 0、正常或无顶背离。
- 股票趋势页的 `stockRsi.trendMomentum` 独立保存 RSI 状态、即时背离与 `chaseBuyBlocked`，复用同一 Wilder 序列；70 起超买、80 起强超买。即时参考先按时间选择最近有效收盘/RSI 波段峰：左右各 2 根完整日线确认、同日 RSI 为局部峰且至少 70、相距 5–60 根；不能为了得到背离而退选更早峰值。价格更高且 RSI 更低为 WATCH，价格突破至少 0.5% 且 RSI 下降至少 3 点为 POTENTIAL；后续沿用左右各 3 根的价格 high pivot 检测，以该峰当日复权收盘与 RSI 的高价/低 RSI 关系确认 CONFIRMED，日期只能记为右侧数据完整时。确认最多保留 20 根，价格及 RSI 恢复新高、回撤达到 8% 或 RSI 降至 40 时结束。强超买叠加 POTENTIAL/CONFIRMED 才禁止追买，WATCH 不触发，也不生成 SELL 或改变均线/趋势。旧背离生命周期与其风险评分、股票决策规则保持独立；旧缓存缺少新合同必须显示背离未知。
...
- 财报发行人专项解析：LLY 以官方收入披露 text-block 的显式表格层级区分产品叶子和地区，不混合领域小计；HOOD 将交易、净利息、其他收入按官方同层级勾稽，不叠加交易子项或毛利息；AVGO 8-K EX-99.1、ARM 6-K EX-99.2 只接受已验证发行人和官方季度表，表内独立总额必须一致。ARM 发布公告与同日季度 6-K 不可混淆，附件 Type 必须精确匹配。Provider 月末可以校正为官方财季末，但显式官方日期不可被容差覆盖。真实 fixture 和负例见 `tests/fixtures/sec-{lilly-business,robinhood,release-business}/`；这些格式的覆盖不等于所有财期或所有区块必然可解析。
...
- 首页与交易页资产头部以交易页紧凑报告为统一尺寸基准：标签 `12px`，主金额 `clamp(28px, 8.8vw, 40px)`，小数为主金额的 `0.64em`；两页头部内边距、双列间距、控件高度和窄屏换行必须保持一致。普通说明使用 `12px / white 50%`，时间、口径和次级信息使用 `11–12px / white 40%`，实体副标题使用 `11px / white 35%`，同类字段标签使用 `13px / white 50%`。交易持仓继续使用左右滑动表格与固定名称列。
- 徽章、图表刻度、tooltip 和紧凑控件允许使用 `10px`，但任何可见文字不得小于 `10px`；禁止重新引入 `8px`、`8.5px`、`9px` 或 `9.5px` 字号。
- 后续新增或重新设计的页面与弹窗统一使用正常字距，不使用负 `letter-spacing`、`tracking-tight` / `tracking-tighter` 等压缩数字或文字。金额保留约定的小数精度；空间不足优先调整布局或合理换行，不靠收紧字距塞入。既有其他页面不因这条规则批量改版。
