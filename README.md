...
Pending `v10.7.9.348` fixes the one-time upgrade gap: production read-only inspection found 23 existing locked competition snapshots (`8 / 7 / 8` for 2026-07-13 through 2026-07-15) but zero publication-marker rows, so the API returned waiting even though historical data remained intact. `supabase/snapshot_publication_marker_bootstrap_20260716.sql` runs only while the competition marker is empty, considers dates before 2026-07-16, locks the relevant profile/member/snapshot/marker boundary, and selects the newest exact complete locked batch. Every expected member must have a completed profile, valid eligibility and ranking baseline, a strict 64-hex ledger hash, and a non-negative revision; any missing or unexpected row rejects that date, and no complete candidate raises an exception and rolls back. The migration inserts only the service-only marker and never creates, updates, deletes, or backfills snapshots, returns, rankings, trades, or holdings. The expected complete candidate is 2026-07-13; incomplete 2026-07-14/15 batches remain for the normal post-17:00 ET scheduler. Source commit, production SQL, and production verification are still pending.
