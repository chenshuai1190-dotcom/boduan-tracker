...
`v10.7.9.351` makes the existing late-retry Cron the protected operational recovery path for this class of system gap. A manual Vercel invocation before 17:00 ET selects only the latest already-completed US session, then runs the same ordered, idempotent P&L and competition catch-up and exact-batch publication gate. It cannot accept an arbitrary date, remains behind the existing `CRON_SECRET`, and does not change the normal automatic `21/22/23 UTC` schedule or any ledger, EOD, revision, hash, eligibility, or marker rule.

Production recovery completed on 2026-07-16: the protected late-retry returned `200`; aggregate database readback proved July 14 at 8 expected / 8 complete / 0 missing and July 15 at 9 / 9 / 0. The latest competition publication marker is July 15. No snapshot, return, trade, identity, position, or financial amount was fabricated or exposed during verification.
