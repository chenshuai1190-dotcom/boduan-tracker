...
- 股票决策位于交易全部功能的摊薄工具上方，输入代码后才读取已鉴权 `/api/quote?view=stock-decision`，不增加首页请求、不读取持仓、不产生交易。四项判断为趋势、位置、量能、动量；统一使用 EODHD 已完成日线复权 OHLC，成交量保留供应商拆股调整口径，RSI 复用既有算法。支撑压力由近 120 个交易日左右各 3 根确认的枢轴形成区域，0.5 ATR(14) 聚合容差在识别时固定，独立触及至少间隔 5 根；保留原支撑破位，不用更低区域掩盖。放量突破保存事前已知区域和当日相对前 20 日均量，次日量能回落不直接否定突破；收盘失守区域下沿或超过 10 个后续交易日失效。此工具不请求财报日历，财报不参与结论；已知技术风险优先显示，核心数据不足不能生成“可进一步观察”。阈值为 v1 观察规则，未验证预测收益。日线按代码和完成日缓存，失败退避及旧数据状态独立于正式行情和所有收益账本。
