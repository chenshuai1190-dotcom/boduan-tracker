...
- 最新待发布补充: `v10.7.9.290` 首页股票代码和公司名称继续降亮已获授权直接部署;自选/持仓股票代码从 `text-white/80` 降为 `text-white/70`,公司名称从 `text-white/40` 降为 `text-white/35`,不调整股票 Logo、价格或数据逻辑。生产暂仍为 `v10.7.9.289`,部署成功后回填实际提交和入口。
- 最新已上线补充: `v10.7.9.289` 首页持仓盈亏和自选亮度修复已上线,production runtime commit `42582e03432b71eb4a6893069ed04303c633f0e0`;持仓盈亏金额和收益率取消粗体并分别跟随系统涨跌颜色,持仓盈亏列参考交易页从 `112px` 扩为 `144px`,金额/收益率固定单行显示;自选股票代码和价格统一为当前信号“等待中”的 `text-white/80`。持仓盈亏计算、交易账本、行情接口、收益快照、数据库、RLS 和鉴权不变。
...
- 当前 GitHub `main`: 最新运行时代码提交 `42582e03432b71eb4a6893069ed04303c633f0e0` 包含 `v10.7.9.289` 首页持仓盈亏和自选亮度修复;本文件所在最新提交为该 runtime 部署证据回填。
- 当前生产运行时基准提交: `42582e03432b71eb4a6893069ed04303c633f0e0`。
- 设置页版本: `v10.7.9.290`（待发布;生产暂为 `v10.7.9.289`）。
- 当前生产地址: `https://boduan-tracker.vercel.app`。
- 最新 Vercel 状态: `npm run verify:deploy-status -- 42582e0` pass;GitHub Actions run `29107423264` success,Vercel status success,target `https://vercel.com/chenshuai1190-7580s-projects/boduan-tracker/ZxxvCwqxFtudpym1roAk9W8VvsXJ`;production alias 已更新,入口 `/assets/index-CWyS0wxR.js`。
- 最近交接文档刷新部署: 本文件所在最新提交为 `v10.7.9.289` runtime production 验证证据回填。
- 线上关键验证: `npm run verify:deploy-status -- 42582e0` pass;GitHub Actions run `29107423264` success;Vercel status success;生产入口 `/assets/index-CWyS0wxR.js`;未登录 `/api/quote?symbols=VIX` 和 `/api/earnings-calendar?symbols=NVDA` 均返回 `401`;线上 `App-Da4AhrVY.js`、`HomeTab-B-xw838a.js`、`SettingsTab-DLDR7U4-.js` 和 `settingsChangelog-BHojI-oU.js` 与本地构建 SHA-256 一致,生产 marker 确认 `v10.7.9.289`、144px 持仓盈亏列、正常字重/单行保护和自选价格 `text-white/80` 存在,旧 112px、粗体持仓盈亏和 `text-white/78` marker 不存在。
...
- 当前 GitHub source 基准提交: 最新运行时代码提交 `42582e03432b71eb4a6893069ed04303c633f0e0` 包含 `v10.7.9.289` 首页持仓盈亏和自选亮度修复;本文件所在最新提交为该 runtime 部署证据回填。
- 当前生产运行时基准提交: `42582e03432b71eb4a6893069ed04303c633f0e0`。
- 最近应用代码提交: production 最近应用代码提交 `42582e03432b71eb4a6893069ed04303c633f0e0` 包含 `v10.7.9.289` 首页持仓盈亏和自选亮度修复;此前 `c3fe394abe7f8ec10f7e14eb535b2fda9377cba9` 包含 `v10.7.9.288` 首页财报和股票文字层级降亮,`ca932917d893ce966a05a999d4ead2d415291724` 包含 `v10.7.9.287` 首页行情超限分批热修,`aa7fe68429491a170b637889ea4c95cd8670e3c3` 包含 `v10.7.9.286` 首页财报日历智能上移;更早收益报表、个股详情、财报日历、BTC/指数拆分和 iOS 主屏实时链路历史见开发日志。
- 最近文档/配置记录提交: 本文件所在最新提交;流程工具链运行提交为 `c47b6e0b78115ea0e004c8cc5b498a2505527fc4`。
- 设置页版本: `v10.7.9.290`（待发布;生产暂为 `v10.7.9.289`）。
- Vercel 最新部署: `v10.7.9.289` runtime commit `42582e03432b71eb4a6893069ed04303c633f0e0` 已 success,target `https://vercel.com/chenshuai1190-7580s-projects/boduan-tracker/ZxxvCwqxFtudpym1roAk9W8VvsXJ`,production 入口 `/assets/index-CWyS0wxR.js`,关键 chunks 包括 `/assets/App-Da4AhrVY.js`,`/assets/HomeTab-B-xw838a.js`,`/assets/SettingsTab-DLDR7U4-.js`,`/assets/settingsChangelog-BHojI-oU.js`,`/assets/i18n-DirP5Edj.js`。
- 最近交接文档刷新部署: 本文件所在最新提交只同步 `v10.7.9.289` runtime production 验证证据。
...
- 最新补充部署记录: `v10.7.9.289` runtime code commit `42582e03432b71eb4a6893069ed04303c633f0e0` pushed via project SSH key;GitHub Actions `CI` run `29107423264` success;Vercel status success,target `https://vercel.com/chenshuai1190-7580s-projects/boduan-tracker/ZxxvCwqxFtudpym1roAk9W8VvsXJ`;production alias updated;active runtime assets and auth boundaries verified。生产入口 `/assets/index-CWyS0wxR.js`;关键 chunks 包括 `/assets/App-Da4AhrVY.js`,`/assets/HomeTab-B-xw838a.js`,`/assets/SettingsTab-DLDR7U4-.js`,`/assets/settingsChangelog-BHojI-oU.js`,`/assets/i18n-DirP5Edj.js`。上一条 `v10.7.9.288` 首页财报和股票文字层级降亮为 runtime code commit `c3fe394abe7f8ec10f7e14eb535b2fda9377cba9`;更早运行时代码部署历史见 `docs/development-log.md`。
...
当前 GitHub main: 最新运行时代码提交 `42582e03432b71eb4a6893069ed04303c633f0e0` 包含 `v10.7.9.289` 首页持仓盈亏和自选亮度修复
当前运行时基准提交: `42582e03432b71eb4a6893069ed04303c633f0e0`
设置页版本: `v10.7.9.290`（待发布;生产暂为 `v10.7.9.289`）
最新 Vercel 部署: `42582e03432b71eb4a6893069ed04303c633f0e0` success,target `https://vercel.com/chenshuai1190-7580s-projects/boduan-tracker/ZxxvCwqxFtudpym1roAk9W8VvsXJ`
最新生产入口: `/assets/index-CWyS0wxR.js`

关键线上验证:
- `v10.7.9.290` 已获授权直接部署:首页自选/持仓股票代码降为 70% 白色,公司名称降为 35% 白色;生产暂仍为 `v10.7.9.289`
- `npm run verify:deploy-status -- 42582e0` pass: GitHub Actions run `29107423264` success,Vercel status success,生产入口 `/assets/index-CWyS0wxR.js`,未登录 quote/earnings 均为 `401`
