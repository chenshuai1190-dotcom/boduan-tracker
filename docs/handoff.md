...
- 当前本地发布候选: `v10.7.9.326`;添加自选股票弹窗已新增真实美股收盘涨/跌幅榜并修正已添加股票的减号状态,等待 sensitive 门禁和生产部署证据。
- `v10.7.9.326` 使用已登录 `/api/quote?view=market-movers` 和服务端 EODHD 真收盘数据,再与 Nasdaq Trader 当前上市目录交集验证,涨/跌各返回 30 只;范围严格限定 NASDAQ / NYSE / NYSE American 普通股,官方目录异常时 fail closed,生产无 EODHD-only 或 mock 榜单兜底。
- 当前已验证生产仍为 `v10.7.9.325`,production runtime `05a0b9198d386ef1cafd95d28ad7c0083a271be0`,入口 `/assets/index-BXaiQwsS.js`;v326 上线后必须回填新 runtime、Actions、Vercel target 和入口。
...
- 本地设置页版本: `v10.7.9.326`;当前生产设置页版本仍为 `v10.7.9.325`。生产运行时基准为 `05a0b9198d386ef1cafd95d28ad7c0083a271be0`,入口 `/assets/index-BXaiQwsS.js`。
...
- 当前本地设置页版本为 `v10.7.9.326`;生产仍为 `v10.7.9.325`,等待本轮正式部署验证。
...
- 线上关键验证: 当前生产未登录 quote、earnings 均为 `401`;生产 `HomeTab-fnYwGkqp.js`、`SettingsTab-KA5_cIMI.js`、`settingsChangelog-CsPcDcVn.js` 和入口与 v325 本地 build SHA-256 一致。v326 上线后必须追加未登录 market-movers `401`、已登录真实 30/30 榜单和新版产物 marker。
...
- 当前本地设置页版本为 `v10.7.9.326`,生产仍为 `v10.7.9.325`。v326 新增真实收盘涨跌榜并修正已添加减号;v325 将首页财报公司卡片默认打开列表视图。
- 当前 GitHub source 基准提交: 以本文件所在最新交接证据提交为准,接手后执行 `git log -1 --oneline`;当前已验证生产运行时代码提交为 `05a0b9198d386ef1cafd95d28ad7c0083a271be0`。
- 当前生产运行时基准提交: `05a0b9198d386ef1cafd95d28ad7c0083a271be0`。
- 最近应用代码提交: 本文件所在待发布提交包含 `v10.7.9.326` 真实收盘涨跌榜;当前生产 `05a0b9198d386ef1cafd95d28ad7c0083a271be0` 包含 `v10.7.9.325` 财报弹窗默认列表视图。
...
- 当前本地/待发布设置页版本: `v10.7.9.326`;当前生产设置页版本: `v10.7.9.325`。
...
- 设置页版本为 `v10.7.9.326`;若部署证据回填提交尚未出现,生产仍可能暂时显示 `v10.7.9.325`。
...
- 设置页版本: `v10.7.9.326`（待部署;当前生产 `v10.7.9.325`）
