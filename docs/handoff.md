...
- EODHD 本地真实 key smoke: pass;`npm run smoke:eodhd-calendar -- --symbols=NVDA,MSFT,GOOGL,META,TSM --from=2026-07-01 --to=2026-09-30` 返回 5 条 earnings、472 条 trends、5 条 merged revenue。结论:预计营收来自 `/api/calendar/trends` 的 `revenueEstimateAvg`,不是 `/api/calendar/earnings`;若生产仍空,优先查生产 EODHD key 权限或 Vercel env。

以下保留历史验证摘录,用于追溯旧问题;接手时以本节最上面的 `v10.7.9.251` 证据为当前线上基线。
...
当前运行时代码提交: `1a5ae2605e51838514f3b508eb9ec6c769a7268a`
设置页版本: `v10.7.9.251`
最新运行时 Vercel 部署: success,`v10.7.9.251` production marker verified
...
- 本地 EODHD smoke: `npm run smoke:eodhd-calendar -- --symbols=NVDA,MSFT,GOOGL,META,TSM --from=2026-07-01 --to=2026-09-30` 返回 `ok: true`,5/5 财报事件合并到预计营收;命令和排障说明见 `docs/eodhd-local-testing.md`
...
- 首页底部财报日历已在 `v10.7.9.249` 独立重构:新增 `/api/earnings-calendar`,前端不接触 EODHD token,旧 `CALENDAR:` / NASDAQ calendar / 白色事件弹窗已移除;`v10.7.9.250` 已把首页预览压缩为固定一行,日期字号同步弹窗日历日期,并删除首页/弹窗标题旁信息图标;`v10.7.9.251` 已取消第一项默认高亮并修复 EODHD trends 嵌套数组导致预计营收无法合并的问题。
