...
- 目标页: 北极星目标、年度目标进度、复盘和投资戒律。`v10.7.9.111` 起目标页第一阶段统一深色移动端风格,北极星目标支持 USD/RMB 切换并使用现有汇率状态,年度目标和投资戒律都改为点击记录后弹出操作面板,投资戒律保留置顶/取消置顶;`v10.7.9.112` 修正目标页视觉对齐,头部卡片压回移动端紧凑高度,年度进度条微光限制在进度条内,年度目标区域删除多余外层卡片,当前年补回右侧目标/落后信息,未开始年度补回起点、目标、增长目标虚线和两端金额结构;`v10.7.9.113` 目标页金额改为首页同款完整数字和正常字重,头部卡片进一步压缩,USD/RMB 切换同步首页尺寸,头部卡删除右下角半圆和金色边框,年度目标区域继续外扩,涨跌粉色同步首页颜色体系;`v10.7.9.114` 目标页金额取消两位小数,本年卡边框同步北极星头卡弱边框,头卡 `设置` 按钮上移;`v10.7.9.115` 只在北极星头卡大目标金额恢复两位小数,小数后缀用小字号显示,年度目标等其它金额仍保持无小数。
...
- `DevVisualPreview` 是只读 mock 资产/目标视觉预览,提供固定账户、月度快照、目标页年度数据、投资戒律和复盘日志 mock,用于快速检查资产页 UI、目标页 UI、弹窗、输入框显色、0 余额账户隐藏和操作面板;目标页可直接打开 `http://127.0.0.1:5173/?tab=review`。
...
- `v10.7.9.115` local validation: `npm test` pass,64 tests;`npm run build` pass;`npm audit` pass,0 vulnerabilities;`git diff --check` pass.
- `v10.7.9.115` local visual verification: Vite dev server `http://127.0.0.1:5173/?tab=review`, in-app browser viewport `390x844`;北极星头卡大目标金额实际渲染为 `$14,860,167.41`,拆成 `$14,860,167` 34px 正常字重和 `.41` 20px 正常字重两段;北极星卡 `358x244`,年度区域和 2026 本年卡 `374px` 宽,页面 `scrollWidth=390`;年度目标金额仍无小数,包含 `当前 $2,470,000`, `+$70,000`, `目标 $2,880,000`, `落后 $410,000`;浏览器 console error/warn 为空。
- `v10.7.9.115` deployment: pending Vercel production deployment.
- Latest completed runtime remains `v10.7.9.114`: `npm test` pass,64 tests;`npm run build` pass;`npm audit` pass,0 vulnerabilities;`git diff --check` pass。
- Latest completed local visual verification: Vite dev server `http://127.0.0.1:5173/?tab=review`, in-app browser viewport `390x844`;目标页 mock 可见、背景深色、无横向溢出、无融资杠杆监控;北极星目标卡实测 `358x244`;年度区域和 2026 本年卡实测 `374px` 宽;本年卡边框和头卡边框同为 `rgba(255,255,255,0.1)`;头卡 `设置` 按钮离底边约 `7px`;首屏金额均无两位小数,包含 `$14,860,167`, `当前 $2,470,000`, `+$70,000`, `目标 $2,880,000`, `落后 $410,000`;浏览器 console error/warn 为空。
- Latest completed production unauthenticated `GET /api/quote?symbols=VIX`: HTTP 401。
- `v10.7.9.114` runtime 已推送并部署成功;runtime commit `a0c79b5a42f5284c2ab93103e8b409d1fdf7162a`, Vercel target `https://vercel.com/chenshuai1190-7580s-projects/boduan-tracker/GptMiYxiSqezAJHF3n53iS968aoM`。
...
- 本轮本地构建 runtime chunks: `/assets/index-CuyJMdZj.css`, `/assets/ReviewTab-BoRRo5hi.js`, `/assets/SettingsTab-i-nStSeY.js`, `/assets/App-B90sqG3G.js`, `/assets/AnalysisTab-S1iYuxfx.js`, `/assets/HomeTab-MC5TFijP.js`, `/assets/TradesTab-GTTATZ2u.js`。
- 生产 entry chunks: `/assets/index-CVgvhXEq.js`, `/assets/rolldown-runtime-QTnfLwEv.js`, `/assets/react-vendor-0zZBvgmv.js`, `/assets/index-CuyJMdZj.css`。
- 生产 runtime chunks: `/assets/App-B1ojJ4p1.js`, `/assets/ReviewTab-BoRRo5hi.js`, `/assets/SettingsTab-i-nStSeY.js`, `/assets/marketColorMode-DqRpGks3.js`, `/assets/AnalysisTab-S1iYuxfx.js`, `/assets/HomeTab-MC5TFijP.js`, `/assets/TradesTab-GTTATZ2u.js`, `/assets/Login-D5hE3XbQ.js`。

关键 marker:

- 生产 `index-CuyJMdZj.css` 不包含全局 `overscroll-behavior-y:none`,已回退的全局 scrollbar/overscroll suppression 没有恢复。
- 生产 `App-B1ojJ4p1.js` 引用最新业务 chunks,包括 `ReviewTab-BoRRo5hi.js` 和 `SettingsTab-i-nStSeY.js`。
- 生产 `ReviewTab-BoRRo5hi.js` 包含 0 位小数金额 formatter marker、`mb-1.5 mt-auto flex -translate-y-2`、`rounded-[20px] border border-white/10 bg-[#0b0f14] p-4 text-left shadow-` 和 `h-[244px]`;且不包含 `fmtWan`、` 万`、`money(ageGoalAmount, 2)`、`border-[#f6b54b]/65`、`bottom-[-78px] h-48 w-48` 或 `融资杠杆监控`。
- 生产 `SettingsTab-i-nStSeY.js` 包含 `v10.7.9.114` 和 `目标页数字密度微调`。
...
设置页版本: `v10.7.9.115`
Vercel 最新运行时部署: pending, latest deployed runtime remains `v10.7.9.114` target `https://vercel.com/chenshuai1190-7580s-projects/boduan-tracker/GptMiYxiSqezAJHF3n53iS968aoM`
部署记录: `v10.7.9.115` pending;latest deployed `v10.7.9.114` runtime commit `a0c79b5a42f5284c2ab93103e8b409d1fdf7162a`;production `GET https://boduan-tracker.vercel.app/?v=a0c79b5-runtime` HTTP 200
...
- 本地没有 Supabase 配置时,开发环境会通过 `src/AuthGate.jsx` 自动进入 `src/DevVisualPreview.jsx` 的只读 mock 预览,可以快速检查资产页和目标页深色背景、卡片间距、按钮、输入框显色、年度目标结构和操作弹窗。
- `DevVisualPreview` 不连接真实 Supabase,不会提交 `.env`,不会改生产数据;只能用于视觉和交互烟测,不能当真实数据来源。
- 涉及真实登录、真实账户数据、行情、RLS、鉴权或部署缓存切换时,仍要用生产地址做线上验证。

当前已完成:
- `v10.7.9.115` 北极星目标小数层级优化待验证和部署:只在目标页北极星头卡大目标金额恢复两位小数,小数后缀改为小字号;年度目标、计划、实际、落后等其它金额仍保持无小数完整数字。
