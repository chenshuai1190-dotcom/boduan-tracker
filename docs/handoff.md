...
- 最新 Vercel 状态: `v10.7.9.274` production `success`,target `https://vercel.com/chenshuai1190-7580s-projects/boduan-tracker/7PBu3saeop8YqvTw8ZfkxT2ESfQG`;production alias 已更新,入口 `/assets/index-BXhGeJGW.js`。
- 最近交接文档刷新部署: 本文件所在最新提交为准;本次 docs-only follow-up 回填 `v10.7.9.274` 部署证据,运行时代码仍是 `162d7a9230f578e6075e1475a498ad9aef6465c4`。
- 线上关键验证: `HomeTab-82VW8JC7.js` 包含 `fixed-calendar`、`data-earnings-calendar-selected-list`、`86dvh` 和 `grid-rows-6`;`SettingsTab-DfoY-rBe.js` 包含 `v10.7.9.274`;`settingsChangelog-J9NxzMa_.js` 包含 `v10.7.9.274`、`财报日历弹窗高度固定`、上一版 `v10.7.9.273` 和 `持仓列宽恢复 v230 口径`;未登录 `/api/quote?symbols=VIX` 和 `/api/earnings-calendar?symbols=NVDA` 均返回 `401`。
- 当前产品焦点: 英文模式已分阶段覆盖设置页、底部导航、首页、交易页、资产页和目标页。`v10.7.9.176` 起股票涨跌幅按现价和昨收重算;`v10.7.9.177` 到 `v10.7.9.207` 主要处理股票 realtime、iOS 主屏 snapshot、BTC/指数拆分和卡位稳定;`v10.7.9.208` 到 `v10.7.9.211` 主要处理三大指数去 Yahoo 图源、固定卡位和分时曲线锁定;`v10.7.9.212` 到 `v10.7.9.228` 建立收益报表独立页、真实快照读取、手动收盘快照回填、收益日历和周期统计;`v10.7.9.229` 起新增全账户自动收盘快照;`v10.7.9.230` 到 `v10.7.9.248` 主要处理只读个股收益详情页、收益线交互、持仓周期卖出收益口径、历史脏 ticker 修复和个股风险指标;`v10.7.9.249` 起首页底部财报日历改为独立 EODHD endpoint,并删除旧 NASDAQ calendar/`CALENDAR:` 混用链路;`v10.7.9.250` 起首页财报日历视觉压缩为固定一行并同步标题/日期层级;`v10.7.9.251` 起财报预计营收正确兼容 EODHD trends 嵌套数组;`v10.7.9.255` 起已公布财报使用券商式同比对比口径;`v10.7.9.256-259` 已上线列表视图收紧、上一财季回看、请求缓存和首页细节降重;`v10.7.9.260-268` 已上线财报日期选择修复、持仓收益试算弹窗、iOS 输入跳顶修复和价格位置条当前价标记修复;`v10.7.9.269` 已上线交易页持仓表格行对齐;`v10.7.9.270` 已上线财报列表过滤和持仓列距微调;`v10.7.9.271` 已上线持仓当日盈亏列距优化;`v10.7.9.272` 已上线持仓列距再平衡;`v10.7.9.273` 已上线持仓列宽恢复 v230 口径;`v10.7.9.274` 已上线财报日历弹窗固定高度和选中日期列表独立滚动。用户自写内容、中文显示、主交易账本、摊薄工具、行情鉴权和 `/api/quote` 鉴权保持不变。
...
- `v10.7.9.274` 财报日历弹窗高度固定已完成部署和线上验证。当前生产运行时代码提交为 `162d7a9230f578e6075e1475a498ad9aef6465c4`;生产入口为 `/assets/index-BXhGeJGW.js`;Vercel production status 为 `success`,target `https://vercel.com/chenshuai1190-7580s-projects/boduan-tracker/7PBu3saeop8YqvTw8ZfkxT2ESfQG`。
- 当前生产 marker: `HomeTab-82VW8JC7.js` 包含 `fixed-calendar`、`data-earnings-calendar-selected-list`、`86dvh` 和 `grid-rows-6`;`SettingsTab-DfoY-rBe.js` 包含 `v10.7.9.274`;`settingsChangelog-J9NxzMa_.js` 包含 `v10.7.9.274`、`财报日历弹窗高度固定`、上一版 `v10.7.9.273` 和 `持仓列宽恢复 v230 口径`。
...
- `v10.7.9.274` 部署前本地检查: `npm test` 通过 173 个测试;`npm run build` 成功;`npm audit --audit-level=moderate` 返回 0 vulnerabilities;`git diff --check` 干净;本地 390x844 视觉 smoke 确认财报日历弹窗 1 项、2 项和 0 项日期切换时外框保持 `y=59/h=726`,日历区保持 `y=182/h=558`,只有选中日期列表自身滚动。
- GitHub Actions `CI` run `29028434079` completed with `success`。

以下保留历史验证摘录,用于追溯旧问题;接手时以本节最上面的 `v10.7.9.274` 证据为当前线上基线。
...
当前运行时代码提交: `162d7a9230f578e6075e1475a498ad9aef6465c4`
设置页版本: `v10.7.9.274`
最新运行时 Vercel 部署: success,`v10.7.9.274` production marker verified
最新生产入口: `/assets/index-BXhGeJGW.js`

关键线上验证:
- `HomeTab-82VW8JC7.js` 包含 `fixed-calendar`、`data-earnings-calendar-selected-list`、`86dvh` 和 `grid-rows-6`
- `SettingsTab-DfoY-rBe.js` 包含 `v10.7.9.274`
- `settingsChangelog-J9NxzMa_.js` 包含 `v10.7.9.274`、`财报日历弹窗高度固定`、上一版 `v10.7.9.273` 和 `持仓列宽恢复 v230 口径`
- 未登录 `/api/quote?symbols=VIX` 和 `/api/earnings-calendar?symbols=NVDA` 均返回 `401`
- GitHub Actions `CI` run `29028434079` completed with `success`;本地 `npm test` / `npm run build` / `npm audit --audit-level=moderate` / `git diff --check` 已全部通过
