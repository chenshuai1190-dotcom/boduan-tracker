...
- 当前待发布 `v10.7.9.342`：资产页点击精确账户的名称/类型后，读取该 `accountId` 在 `balance_snapshots` 中的真实原币月末余额并显示近 12 个自然月走势；缺月不补零、不插值、不沿用旧余额，也不合并同名或同类型账户。右侧金额仍进入原有账户修改/删除弹窗。定向测试、完整 410/410 测试、build、toolchain、audit high、diff check 和 iPhone 17 Pro Simulator 只读截图已通过；GitHub `main`、Actions、Vercel 与生产产物仍待本轮 runtime 推送后回填。
- 当前生产已上线 `v10.7.9.341`：EODHD 实测 ASML actual EPS `7.58`、estimate `7.98`、actual revenue `null`；缺失营收继续显示 `--`。EUR 营收按真实外汇数据换算 USD，盘前反应只复用现有登录态 WebSocket 新鲜报价并以前一交易日普通收盘价为基准，正式反应使用报告日普通 `close` 并优先锁定；`unknown` 时段不猜测，收盘后缺反应事件自动有界补读。生产路径不使用 mock、预期值或估算值冒充真实结果。本地完整测试 403/403、build、toolchain、audit high、RLS REST、diff check 和 iPhone 17 Pro Simulator 只读截图均已通过；runtime `ae9eb03e725d4a3407eca020b7832e94edd972c4`、Actions `29417637784`、Vercel `BmcXTYQaBfw7oZq5oSovXjqfXQ1W`、入口 `/assets/index-DAkXD4bD.js`、7 个关键生产产物一致性、新版 marker、开发 fixture 缺席和未登录 401 均已验证。
...
- 当前已验证 production runtime 为 `ae9eb03e725d4a3407eca020b7832e94edd972c4`,入口 `/assets/index-DAkXD4bD.js`;GitHub Actions run `29417637784` success,Vercel target `BmcXTYQaBfw7oZq5oSovXjqfXQ1W` success。
...
- 当前本地设置页版本为 `v10.7.9.342`；当前生产设置页版本仍为 `v10.7.9.341`，已验证生产运行时基准为 `ae9eb03e725d4a3407eca020b7832e94edd972c4`,入口 `/assets/index-DAkXD4bD.js`。
...
- 当前 GitHub `main`: 数据库源提交为 `0f52700761beab0d4488e067ca9e968aea9a9bc1`;最新已上线运行时代码提交为 `ae9eb03e725d4a3407eca020b7832e94edd972c4`。v341、v340、v339、v338、v337、v336、v335、v334、v333、v332 和此前 ET gate/rebaseline 的生产 SQL/runtime 均已部署并通过门禁。
- 当前生产运行时基准提交: `ae9eb03e725d4a3407eca020b7832e94edd972c4`。
- 当前本地设置页版本为 `v10.7.9.342`；当前生产设置页版本在 runtime 推送前仍为 `v10.7.9.341`。
...
- 最新运行时部署: `npm run verify:deploy-status -- ae9eb03` pass;GitHub Actions run `29417637784` success,Vercel status success,target `https://vercel.com/chenshuai1190-7580s-projects/boduan-tracker/BmcXTYQaBfw7oZq5oSovXjqfXQ1W`;production alias 已更新,入口 `/assets/index-DAkXD4bD.js`。
...
- 线上关键验证: 当前生产未登录 quote、market-movers、earnings、pnl-benchmark、competition GET/POST、competition snapshot GET 和 P&L snapshot GET 均为 `401`;生产入口为 `/assets/index-DAkXD4bD.js`,v341 的入口、App、HomeTab、SettingsTab、settings changelog、i18n 和 stockRealtime 共 7 个关键产物与本地 production build SHA-256 一致，命中 v341、盘前实时、等待收盘和 EODHD WebSocket source marker，且无开发 fixture。`2026-07-13` 的上一 runtime 历史快照聚合为个人 12 users / 12 portfolio rows、54 symbol rows / 18 symbols;比赛 8 users / 8 locked rows,active 9、eligible 9、initialized 8、invalid 0,另 1 名尚未排名成员 `eligible_ledger_hash_mismatch`。新 revision/CAS SQL、metadata、RLS、并发 smoke 和 ET-gated runtime 已有生产证据;真实 D1/D2 尚待收盘观察。
...
- 当前本地设置页版本为 `v10.7.9.342`；当前生产设置页版本在 runtime 推送前仍为 `v10.7.9.341`。v342 新增精确单账户真实原币月度资产走势，不补零、不插值或沿用旧余额；v341 的真实财报结果与盘前/正式收盘反应继续保留。
- 当前 GitHub source 基准提交: 以本文件所在最新交接证据提交为准,接手后执行 `git log -1 --oneline`;当前已验证生产运行时代码提交为 `ae9eb03e725d4a3407eca020b7832e94edd972c4`。
- 当前生产运行时基准提交: `ae9eb03e725d4a3407eca020b7832e94edd972c4`。
- 最近已部署应用代码提交: `ae9eb03e725d4a3407eca020b7832e94edd972c4` 包含 v341 财报真实盘前与正式收盘反应;上一 runtime `863fbf059c9fa20799865ebbd49a358ab073d23b` 包含 v340 财报真实 actual 的美东到期局部刷新和 iOS PWA 恢复检查。
...
- 当前本地设置页版本: `v10.7.9.342`；当前生产设置页版本在 runtime 推送前仍为 `v10.7.9.341`。
- Vercel 最新部署: runtime commit `ae9eb03e725d4a3407eca020b7832e94edd972c4` 已 success,target `https://vercel.com/chenshuai1190-7580s-projects/boduan-tracker/BmcXTYQaBfw7oZq5oSovXjqfXQ1W`,production 入口 `/assets/index-DAkXD4bD.js`;GitHub Actions run `29417637784` success。7 个关键产物与本地 build SHA-256 一致,quote/earnings 未授权边界均为 `401`。
...
- 本地设置页版本为 `v10.7.9.342`；生产在 runtime 推送前仍为 `v10.7.9.341`，以最新提交和部署证据为准。
- 当前已验证生产运行时为 `ae9eb03e725d4a3407eca020b7832e94edd972c4`,入口 `/assets/index-DAkXD4bD.js`;接手后以最新部署证据为准。
...
当前生产基准:
- 运行时代码: `ae9eb03e725d4a3407eca020b7832e94edd972c4`
- 设置页版本: `v10.7.9.342`
- 生产入口: `/assets/index-DAkXD4bD.js`
- Runtime Actions: `29417637784` success
- Runtime Vercel: `https://vercel.com/chenshuai1190-7580s-projects/boduan-tracker/BmcXTYQaBfw7oZq5oSovXjqfXQ1W` success
- 最新 docs evidence: 本文件所在提交（已回填 v341 runtime、Actions、Vercel、生产入口、7 个产物一致性、开发 fixture 缺席和未授权边界；接手后以 `git log -1` 核验）
...
最近完成:
- `v10.7.9.342`:资产页点击精确账户名称/类型后，只读取该账户 `balance_snapshots` 的真实原币月末余额并显示近 12 个自然月走势；缺月不补零、不插值、不沿用旧余额，也不合并其他账户。右侧金额继续进入原有修改/删除弹窗。定向测试、完整 410/410 测试、build、toolchain、audit high、diff check 与 iPhone 17 Pro Simulator 只读截图已通过；runtime、Actions、Vercel、生产入口和产物一致性待本轮发布后回填。
