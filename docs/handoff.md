...
- 当前生产运行时基准提交: `ca31dec9f1b39dac47bf7d0c319cc607fa0d1f69`;最近已验证的 runtime marker 为 `v10.7.9.205`。
- 最近应用代码提交: `ca31dec9f1b39dac47bf7d0c319cc607fa0d1f69`,把首页三大指数和 BTC 拆成两个独立行情系统,`INDICES` 只返回三大指数并优先用 Yahoo chart 当前价,BTC 继续使用独立 `/api/btc-realtime`。上一轮 iOS snapshot 提速提交为 `d6ef1695bb0fb9c3cb2206a50f61756e85789eb2`,上一轮 BTC 恢复提交为 `1d414466c4443bcc5e552019d1cff22440f0e847`,上一轮回退提交为 `6797bce8c49a2844ed21ff0252b4fc519778771c`,上一轮实验代码提交为 `29f596bf7ffacacbc6b580fb5be04c34f019a5b5`,上一轮稳定生产 `v10.7.9.201` 应用代码提交为 `1b70bf5717d7701e432244a5f622f17eee87cfcb`。
- 最近文档/配置记录提交: 本文件所在最新提交。
- 设置页版本: `v10.7.9.205`。
- Vercel 最新运行时部署: success,`v10.7.9.205` production marker verified,`v10.7.9.202` marker absent。
- 最近交接文档刷新部署: 本文件所在最新提交为准;`v10.7.9.205` 运行时代码已部署成功。
...
- 最新补充部署记录: `v10.7.9.205` runtime code commit `ca31dec9f1b39dac47bf7d0c319cc607fa0d1f69` pushed via project SSH key;Vercel status success,target `https://vercel.com/chenshuai1190-7580s-projects/boduan-tracker/68MCvt2Mb39kpVo1LWhkSMx24ZYT`;production alias updated;active runtime assets and auth markers verified。上一条 `v10.7.9.204` runtime code commit `d6ef1695bb0fb9c3cb2206a50f61756e85789eb2` pushed via project SSH key;Vercel status success,target `https://vercel.com/chenshuai1190-7580s-projects/boduan-tracker/4bMGSLpskj7RBjYDr2pCpZzjXUwX`;production alias updated;active runtime assets and auth markers verified。
- Supabase 项目 ref: `ykgotnmtqcqdzqtrlayq`
- 交接文档刷新提交: 本文件所在最新提交,接手后以 `git log -1 --oneline` 为准。

产品现在可用。最近一轮重点是首页自选/持仓体验、交易账本口径、BTC 独立实时行情、PWA 图标、找回密码链路、Supabase Auth URL 配置,`v10.7.9.93` 到 `v10.7.9.149` 的已部署功能见下方历史条目;`v10.7.9.150` 到 `v10.7.9.155` 持续收紧目标页标题/操作弹窗和首页 CNN 恐慌仪表盘刻度;`v10.7.9.156` 新增英文模式第一阶段;`v10.7.9.157` 到 `v10.7.9.161` 梳理盘前实时行情和股票核心行情来源;`v10.7.9.162` 到 `v10.7.9.166` 分阶段把英文模式扩展到交易页、资产页和目标页;`v10.7.9.167` 到 `v10.7.9.173` 处理交易录入、CNY 显示、目标页命名和弹窗细节;`v10.7.9.176` 到 `v10.7.9.183` 处理股票涨跌幅重算、REST/WS 合并保护和 iOS PWA realtime 恢复;`v10.7.9.184` 调整全局行情红色;`v10.7.9.185` 到 `v10.7.9.190` 完成券商口径今日盈亏、盘后/夜盘收盘锁定和锁定标签弱化;`v10.7.9.191` 修复英文头卡持仓文案挤压;`v10.7.9.192` 新增股票盘口 WebSocket 兜底;`v10.7.9.193` 到 `v10.7.9.201` 处理股票实时连接、iOS 主屏 snapshot 轮询、启动实时预热、持仓现价 freshness 遮罩、iOS 滑动误触发遮罩和占位符视觉优化;`v10.7.9.202` 首屏当日盈亏兜底实验已回退;`v10.7.9.203` 恢复 iOS 主屏 BTC 独立 WebSocket,避免 BTC 被股票/指数 snapshot burst 标成同步中;`v10.7.9.204` 提升 iOS 主屏股票/指数 active snapshot 速度,盘前/盘中/盘后 1.25 秒轮询并前移 burst;`v10.7.9.205` 拆分首页三大指数和 BTC 两套行情状态,修复指数卡被 EODHD 旧收盘值/共享数组逻辑影响的问题。中文默认显示、用户自写内容和核心交易/行情/数据库边界保持不变。
