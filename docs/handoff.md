...
| 运行时代码 | `v10.7.9.422`：首页未设置现金默认显示 `¥0`/`$0`，并与下方融资负债按同一列起点对齐；保留 v421 可用现金资产与个人收益联动、v420 波段部分卖出、v417 股票实时稳定版 v10 及全部既有正式行情边界；精确发布提交以 GitHub `main` HEAD 为准 |
...
- 首页头部在总资产后显示同字号可用现金；点击金额打开录入窗口，支持 USD 与 CNY。未设置时按当前币种显示 `¥0`/`$0`，不显示“设置”文字，但内部仍与明确设置为 `0` 区分；现金与下方融资负债使用同一列起点和左侧间距，不改变原卡片结构。
...
- v422 只调整首页现金默认显示和与融资负债的列对齐；v421 的现金资产与个人收益完成收盘快照口径不变，也不改变正式交易、股票收益率、QQQ、比赛或 EODHD 行情。其余继续保留波段部分卖出、收益比赛共享 EODHD 缓存与熔断、交易持仓 EODHD 收盘估值、主 `/api/quote` 的 15/30/60 分钟客户端门控、个人收益只读页面和股票实时稳定版 v10。TSM 业绩趋势在每个冷实例首次读取时仍会产生一笔 Fundamentals 与一笔完整历史 FX 请求；6 小时实例缓存仍不是跨实例全局缓存。
