...
- 当前开发与生产版本均为 `v10.7.9.352`。v352 只把顶部日榜指标恢复为“本日收益率”、榜单左标题恢复为“收益率排行榜”；实际收益率、QQQ、跑赢 QQQ 排序、快照与右下真实截止日期完全不变。v351 已通过受保护 late-retry 补齐 07/14 `8/8/0 missing`、07/15 `9/9/0 missing`，最新 marker 为 `2026-07-15`。
- 当前 runtime `eff19f34b9a27688ba863c79ecf56a8f48a25859`；Actions `29494393256`、Vercel `2NwyNBCGvcc48oh8FqsYPfB1ogx4` success，生产入口 `/assets/index-MUBSO0ki.js`。生产 Settings/changelog 命中 v352，比赛 chunk 命中恢复后的双语标题且不再调用 `dayAsOf`；未登录 quote、earnings 均为 `401`。单纯文字改动按 UI-fast 通过定向测试与 build，没有用桌面证据冒充 iOS Simulator 视觉验收。
...
- 当前已验证 production runtime 为 `eff19f34b9a27688ba863c79ecf56a8f48a25859`,入口 `/assets/index-MUBSO0ki.js`;GitHub Actions run `29494393256` success,Vercel target `2NwyNBCGvcc48oh8FqsYPfB1ogx4` success。受保护 late-retry 已执行，07/14 与 07/15 分别完整为 `8/8`、`9/9`，marker 为 07/15。
...
- 当前本地与生产设置页版本均为 `v10.7.9.352`；v352 runtime、Actions、Vercel、生产入口和 v351 生产补漏证据见本节最新部署记录。
...
- 当前 GitHub `main`: 数据库源提交 `0bc0ef2cf423e9f4ac91daafc9cf8c68ba3c7d16` 的生产 marker migration、metadata/grant 回读和 22 tables + 2 RPCs 门禁均已完成；最新已上线运行时代码提交为 `eff19f34b9a27688ba863c79ecf56a8f48a25859`，包含 v352 展示标题恢复、v351 受保护即时补漏、v350 完整批次 gate/真实截止日期以及 v344-v349。
- 当前生产运行时基准提交: `eff19f34b9a27688ba863c79ecf56a8f48a25859`。
- 当前本地与生产设置页版本均为 `v10.7.9.352`。
...
- 最新运行时部署: `npm run verify:deploy-status -- eff19f3` pass;GitHub Actions run `29494393256` success,Vercel status success,target `https://vercel.com/chenshuai1190-7580s-projects/boduan-tracker/2NwyNBCGvcc48oh8FqsYPfB1ogx4`;production alias 已更新,入口 `/assets/index-MUBSO0ki.js`。
...
- 线上关键验证: 当前生产入口为 `/assets/index-MUBSO0ki.js`；设置页/中英文 changelog 命中 `v10.7.9.352`，比赛 chunk 命中“收益率排行榜 / Return Ranking / 本日收益率 / Daily Return”且无 `dayAsOf` 调用。未登录 quote、earnings 均为 `401`；v351 受保护 late-retry 的 07/14 `8/8/0 missing`、07/15 `9/9/0 missing` 与最新 marker `2026-07-15` 继续有效。
...
- 当前本地与生产设置页版本均为 `v10.7.9.352`。v351 已完成受保护即时补漏；v350 的完整批次 gate/真实截止日期、v349 的 PWA 等待缓存唤醒、v348 的真实完整 publication marker 和 v347 的正式成交价准入/QQQ 超额收益排序继续生效。
- 当前 GitHub source 基准提交: 以本文件所在最新交接证据提交为准,接手后执行 `git log -1 --oneline`;当前已验证生产运行时代码提交为 `eff19f34b9a27688ba863c79ecf56a8f48a25859`。
- 当前生产运行时基准提交: `eff19f34b9a27688ba863c79ecf56a8f48a25859`。
- 最近已部署应用代码提交: `eff19f34b9a27688ba863c79ecf56a8f48a25859` 包含 v352 展示标题恢复，并继承 v351 受保护即时补漏及 v350 完整批次 gate/真实截止日期；上一 runtime `69a882bb851ff6f627c8c6ed6d0a0e729c4feced` 为 v351。
...
- 当前设置页版本: `v10.7.9.352`；生产版本相同。v352 只恢复两处用户确认的展示标题；v351 的生产补跑 `8/8` / `9/9` 和 marker 07/15 已核验。
- Vercel 最新部署: runtime commit `eff19f34b9a27688ba863c79ecf56a8f48a25859` 已 success,target `https://vercel.com/chenshuai1190-7580s-projects/boduan-tracker/2NwyNBCGvcc48oh8FqsYPfB1ogx4`,production 入口 `/assets/index-MUBSO0ki.js`;GitHub Actions run `29494393256` success。生产 Settings/changelog/competition chunks 和未授权 quote/earnings 边界均已验证。
...
- 本地与生产设置页版本均为 `v10.7.9.352`；以后续真实提交和部署证据为准。
...
当前生产基准:
- 运行时代码: `eff19f34b9a27688ba863c79ecf56a8f48a25859`
- 生产设置页版本: `v10.7.9.352`
- 生产入口: `/assets/index-MUBSO0ki.js`
- Runtime Actions: `29494393256` success
- Runtime Vercel: `https://vercel.com/chenshuai1190-7580s-projects/boduan-tracker/2NwyNBCGvcc48oh8FqsYPfB1ogx4` success
- 生产数据: 07/14 expected/complete/missing `8/8/0`；07/15 `9/9/0`；latest competition marker `2026-07-15`
- 最新 docs evidence: 本文件所在 docs-only 提交（已回填 v352 runtime、Actions、Vercel、生产入口、恢复后的双语标签及 v351 补漏聚合证据）
