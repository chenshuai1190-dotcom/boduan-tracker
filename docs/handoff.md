...
- 当前发布候选为 `v10.7.9.336`:交易页持仓表把 8px 从“名称/代码”列让给“市值/数量”列,表格总宽和其余四列不变,千万级人民币市值可完整显示。iPhone 17 Pro Simulator、47/47 定向测试和 build 已通过;runtime commit、Actions、Vercel 与生产入口仍 pending,不得提前视为已上线。
- 当前生产已上线 `v10.7.9.335`:资产页走势图/账户分组卡和收益比赛五类主卡统一为首页不透明黑色 `#0b0f14` 与 10% 灰边,保留有业务含义的状态色。iPhone 17 Pro Simulator、UI-fast 门禁、runtime `154daab23ea2712b1bd0717e2bc7696a62799b86`、Actions `29350707864`、Vercel `2gih2yMfRhxKsFeGjK1cWewSnBCn` 和 7 个关键生产产物均已验证。
...
- 当前已验证 production runtime 为 `154daab23ea2712b1bd0717e2bc7696a62799b86`,入口 `/assets/index-Bm6EmNTo.js`;GitHub Actions run `29350707864` success,Vercel target `2gih2yMfRhxKsFeGjK1cWewSnBCn` success。
...
- 当前本地设置页版本为 `v10.7.9.336` 发布候选,生产仍为 `v10.7.9.335`;当前已验证生产运行时基准为 `154daab23ea2712b1bd0717e2bc7696a62799b86`,入口 `/assets/index-Bm6EmNTo.js`。
...
- 当前 GitHub `main`: 数据库源提交为 `0f52700761beab0d4488e067ca9e968aea9a9bc1`;最新已上线运行时代码提交为 `154daab23ea2712b1bd0717e2bc7696a62799b86`。v335、v334、v333、v332 和此前 ET gate/rebaseline 的生产 SQL/runtime 均已部署并通过门禁。
- 当前生产运行时基准提交: `154daab23ea2712b1bd0717e2bc7696a62799b86`。
- 当前本地设置页版本为 `v10.7.9.336` 发布候选,生产仍为 `v10.7.9.335`。
...
- 最新运行时部署: `npm run verify:deploy-status -- 154daab` pass;GitHub Actions run `29350707864` success,Vercel status success,target `https://vercel.com/chenshuai1190-7580s-projects/boduan-tracker/2gih2yMfRhxKsFeGjK1cWewSnBCn`;production alias 已更新,入口 `/assets/index-Bm6EmNTo.js`。
...
- 线上关键验证: 当前生产未登录 quote、market-movers、earnings、pnl-benchmark、competition GET/POST、competition snapshot GET 和 P&L snapshot GET 均为 `401`;生产入口为 `/assets/index-Bm6EmNTo.js`,v335 的 7 个关键产物与本地 production build SHA-256 一致。`2026-07-13` 的上一 runtime 历史快照聚合为个人 12 users / 12 portfolio rows、54 symbol rows / 18 symbols;比赛 8 users / 8 locked rows,active 9、eligible 9、initialized 8、invalid 0,另 1 名尚未排名成员 `eligible_ledger_hash_mismatch`。新 revision/CAS SQL、metadata、RLS、并发 smoke 和 ET-gated runtime 已有生产证据;真实 D1/D2 尚待收盘观察。
...
- 当前本地设置页版本为 `v10.7.9.336` 发布候选,生产仍为 `v10.7.9.335`。v336 完整显示千万级持仓市值;v335 统一资产页和比赛页主卡黑色;v334 精简比赛更新时间、补昵称并统一页面宽度。
- 当前 GitHub source 基准提交: 以本文件所在最新交接证据提交为准,接手后执行 `git log -1 --oneline`;当前已验证生产运行时代码提交为 `154daab23ea2712b1bd0717e2bc7696a62799b86`。
- 当前生产运行时基准提交: `154daab23ea2712b1bd0717e2bc7696a62799b86`。
- 最近已部署应用代码提交: `154daab23ea2712b1bd0717e2bc7696a62799b86` 包含 v335 资产页和比赛页主卡颜色统一;上一 runtime `4f19c6fc14461ddff1a4c0e9be13a61b440a4ce3` 包含 v334 比赛头卡和页面宽度,更早 runtime `d20f1cee3d258879cd53ff527e5c22fad44364a5` 包含 v333 日期对齐。
...
- 当前本地设置页版本: `v10.7.9.336`（发布候选）；当前生产设置页版本仍为 `v10.7.9.335`。
- Vercel 最新部署: runtime commit `154daab23ea2712b1bd0717e2bc7696a62799b86` 已 success,target `https://vercel.com/chenshuai1190-7580s-projects/boduan-tracker/2gih2yMfRhxKsFeGjK1cWewSnBCn`,production 入口 `/assets/index-Bm6EmNTo.js`;GitHub Actions run `29350707864` success。7 个关键产物与本地 build SHA-256 一致,quote/earnings/competition 未授权边界均为 `401`。
...
- 本地设置页版本为 `v10.7.9.336` 发布候选,生产仍为 `v10.7.9.335`。
- 当前已验证生产运行时为 `154daab23ea2712b1bd0717e2bc7696a62799b86`,入口 `/assets/index-Bm6EmNTo.js`;接手后以最新部署证据为准。
...
当前生产基准:
- 运行时代码: `154daab23ea2712b1bd0717e2bc7696a62799b86`
- 设置页版本: `v10.7.9.336`（发布候选；当前生产仍为 `v10.7.9.335`）
- 生产入口: `/assets/index-Bm6EmNTo.js`
- Runtime Actions: `29350707864` success
- Runtime Vercel: `https://vercel.com/chenshuai1190-7580s-projects/boduan-tracker/2gih2yMfRhxKsFeGjK1cWewSnBCn` success
...
最近完成:
- `v10.7.9.336` 发布候选:交易页持仓表把“名称/代码”列由 100px 缩为 92px,把“市值/数量”列由 80px 增为 88px;总宽和其余四列不变,千万级人民币市值完整显示。iPhone 17 Pro Simulator 已用 `10,026,370.00` 边界样例验收,47/47 定向测试和 build 通过;金额计算、汇率、持仓数据、交易账本、API 和安全边界不变,部署证据 pending。
