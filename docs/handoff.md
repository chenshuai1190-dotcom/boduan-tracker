...
- 最新接手补充: `v10.7.9.215` 新增收益报表快照基础。交易主账本 `stock_trades` 仍是唯一事实源,收益报表新增独立组合快照、单股票快照和重算脏区间 SQL;交易新增/编辑/删除后只 best-effort 标记报表从受影响日期重算,不改交易页实时持仓/盈亏显示。`supabase/pnl_report_snapshots.sql` 已在 Supabase 生产项目 `ykgotnmtqcqdzqtrlayq` 的 SQL Editor 执行成功;执行后 `npm run verify:rls:rest` 通过,匿名 REST 对新三张收益报表表返回 0 行。
...
- 最新 Vercel 状态: `95e8cc5` production deployment success,target `https://vercel.com/chenshuai1190-7580s-projects/boduan-tracker/33fKMVqUZZhQFsiN9WnJbi7rifVH`,`v10.7.9.207` production marker verified,`v10.7.9.202` marker absent。
- 最近交接文档刷新部署: 本文件所在最新提交为准;`v10.7.9.207` 运行时代码已部署成功。
- 线上关键验证: 生产入口 `/assets/index-NfQApHns.js`;runtime chunks include `App-CXXSy1Eo.js`,`HomeTab-DNFGFngd.js`,`SettingsTab-D48cKnBI.js`,`settingsChangelog-DQGjDvmG.js`,`btcRealtime-CDUQ9WKl.js`,`i18n-QTvefRC5.js`;production assets contain `v10.7.9.207`,`BTC 卡位保留`,retained `v10.7.9.206`,BTC placeholder marker `PENDING` / `BTC-USD.CC`,`/api/btc-realtime`,`/api/stocks-realtime`,`/api/indices-realtime`;production assets no longer contain `v10.7.9.202`;未登录 `GET /api/quote?symbols=VIX` 返回 `401`;HTTPS 普通 `GET /api/stocks-realtime` 返回 `426`;未登录 `GET /api/btc-realtime?snapshot=1`、`GET /api/indices-realtime?snapshot=1` 均返回 `401`。
- 当前产品焦点: 英文模式已分阶段覆盖设置页、底部导航、首页、交易页、资产页和目标页。`v10.7.9.176` 起股票涨跌幅按现价和昨收重算;`v10.7.9.177` 到 `v10.7.9.182` 主要处理启动、iOS PWA 回前台和三套 realtime 恢复重连;`v10.7.9.183` 起保护实时 tick 缺少有效昨收时不覆盖完整行情行;`v10.7.9.185` 起按券商口径优先使用日线基准;`v10.7.9.189` 起正式拆分“展示当前价”和“今日盈亏计算价”:盘前/盘中今日盈亏随实时价更新,盘后/夜盘今日盈亏锁定到常规盘收盘价,盘后现价仍可显示并影响市值/持仓盈亏;`v10.7.9.190` 起首页/交易页“收盘锁定”状态标签改为中性灰色;`v10.7.9.191` 起英文首页/交易页头部持仓数量只显示 holdings 并恢复中文同款三列间距;`v10.7.9.192` 起股票 realtime relay 保留 `/ws/us` 成交价为主源,新增 `/ws/us-quote` bid/ask 中间价作为盘前成交稀疏时的兜底;`v10.7.9.195` 起 warm upstream 新客户端接入会补发订阅;`v10.7.9.196` 起客户端不再因首轮 symbol 覆盖不足反复断开股票 WebSocket;`v10.7.9.197` 起 iOS 主屏 standalone Web App 自动切换为认证 HTTP snapshot 轮询,BTC、指数和股票不再使用浏览器 WebSocket,普通 Safari/桌面仍走原有 WebSocket;`v10.7.9.198` 起 iOS 主屏首次进入/回前台会先跑 realtime snapshot burst,并避免启动恢复链路先用 `/api/quote` REST 快照覆盖实时价格;`v10.7.9.199` 起 iOS 主屏预热阶段未拿到本轮新 tick 的首页/交易持仓现价只显示 `----`,拿到新 tick 后自动恢复;`v10.7.9.200` 起 iOS 主屏普通 focus/触摸滑动触发的 snapshot 不再刷新 freshness 遮罩,只有真正恢复可见、联网、云数据加载和 BFCache 恢复才重开遮罩周期;`v10.7.9.201` 起首页/交易持仓现价遮罩占位从 `----` 改为更轻的 `--`;`v10.7.9.202` 首屏当日盈亏兜底实验已回退;`v10.7.9.203` 起 BTC 在 iOS 主屏恢复独立 WebSocket,不再被股票/指数 snapshot burst 标成同步中;`v10.7.9.204` 起 iOS 主屏股票/指数 snapshot 在盘前、盘中和盘后使用 1.25 秒活跃轮询,启动/回前台 burst 前移到 0/0.8/1.6/3/5 秒,BTC 继续保持独立 WebSocket;`v10.7.9.205` 起首页三大指数和 BTC 拆成独立状态,`INDICES` 只返回三大指数并优先用 Yahoo chart 当前价,BTC 继续走独立 `/api/btc-realtime`;`v10.7.9.206` 起 BTC 有近期有效 tick 时不再因上拉刷新、切页或回前台短状态显示“连接中”;`v10.7.9.207` 起 BTC 首个 tick/snapshot 未到时也保留第四张 BTC 卡位并用 `--` 占位。用户自写内容、中文显示、持仓/交易计算、交易账本、摊薄工具、行情鉴权和数据库结构不变。
...
- 当前生产运行时基准提交: `95e8cc5ee25486e9ebee5e0a1d3e44b18d3d2133`;最近已验证的 runtime marker 为 `v10.7.9.207`。
- 最近应用代码提交: `95e8cc5ee25486e9ebee5e0a1d3e44b18d3d2133`,保留 BTC 卡位:BTC 首个 tick/snapshot 未到时首页仍显示第四张 BTC 卡,价格/涨跌幅以 `--` 占位。上一轮 BTC 连接态稳定提交为 `857a16e8e77ec079865ac402519e993b83ee4957`,上一轮三大指数/BTC 拆分提交为 `ca31dec9f1b39dac47bf7d0c319cc607fa0d1f69`,上一轮 iOS snapshot 提速提交为 `d6ef1695bb0fb9c3cb2206a50f61756e85789eb2`,上一轮 BTC 恢复提交为 `1d414466c4443bcc5e552019d1cff22440f0e847`,上一轮回退提交为 `6797bce8c49a2844ed21ff0252b4fc519778771c`,上一轮实验代码提交为 `29f596bf7ffacacbc6b580fb5be04c34f019a5b5`,上一轮稳定生产 `v10.7.9.201` 应用代码提交为 `1b70bf5717d7701e432244a5f622f17eee87cfcb`。
- 最近文档/配置记录提交: 本文件所在最新提交。
- 设置页版本: `v10.7.9.207`。
- Vercel 最新运行时部署: success,`v10.7.9.207` production marker verified,`v10.7.9.202` marker absent。
- 最近交接文档刷新部署: 本文件所在最新提交为准;`v10.7.9.207` 运行时代码已部署成功。
...
- 最新补充部署记录: `v10.7.9.207` runtime code commit `95e8cc5ee25486e9ebee5e0a1d3e44b18d3d2133` pushed via project SSH key;Vercel status success,target `https://vercel.com/chenshuai1190-7580s-projects/boduan-tracker/33fKMVqUZZhQFsiN9WnJbi7rifVH`;production alias updated;active runtime assets and auth markers verified。上一条 `v10.7.9.206` runtime code commit `857a16e8e77ec079865ac402519e993b83ee4957` pushed via project SSH key;Vercel status success,target `https://vercel.com/chenshuai1190-7580s-projects/boduan-tracker/8Rso7jTHYcCRU1WL8iKbNGHNZ1dz`;production alias updated;active runtime assets and auth markers verified。
- Supabase 项目 ref: `ykgotnmtqcqdzqtrlayq`
- 交接文档刷新提交: 本文件所在最新提交,接手后以 `git log -1 --oneline` 为准。

产品现在可用。最近一轮重点是首页自选/持仓体验、交易账本口径、BTC 独立实时行情、PWA 图标、找回密码链路、Supabase Auth URL 配置,`v10.7.9.93` 到 `v10.7.9.149` 的已部署功能见下方历史条目;`v10.7.9.150` 到 `v10.7.9.155` 持续收紧目标页标题/操作弹窗和首页 CNN 恐慌仪表盘刻度;`v10.7.9.156` 新增英文模式第一阶段;`v10.7.9.157` 到 `v10.7.9.161` 梳理盘前实时行情和股票核心行情来源;`v10.7.9.162` 到 `v10.7.9.166` 分阶段把英文模式扩展到交易页、资产页和目标页;`v10.7.9.167` 到 `v10.7.9.173` 处理交易录入、CNY 显示、目标页命名和弹窗细节;`v10.7.9.176` 到 `v10.7.9.183` 处理股票涨跌幅重算、REST/WS 合并保护和 iOS PWA realtime 恢复;`v10.7.9.184` 调整全局行情红色;`v10.7.9.185` 到 `v10.7.9.190` 完成券商口径今日盈亏、盘后/夜盘收盘锁定和锁定标签弱化;`v10.7.9.191` 修复英文头卡持仓文案挤压;`v10.7.9.192` 新增股票盘口 WebSocket 兜底;`v10.7.9.193` 到 `v10.7.9.201` 处理股票实时连接、iOS 主屏 snapshot 轮询、启动实时预热、持仓现价 freshness 遮罩、iOS 滑动误触发遮罩和占位符视觉优化;`v10.7.9.202` 首屏当日盈亏兜底实验已回退;`v10.7.9.203` 恢复 iOS 主屏 BTC 独立 WebSocket,避免 BTC 被股票/指数 snapshot burst 标成同步中;`v10.7.9.204` 提升 iOS 主屏股票/指数 active snapshot 速度,盘前/盘中/盘后 1.25 秒轮询并前移 burst;`v10.7.9.205` 拆分首页三大指数和 BTC 两套行情状态,修复指数卡被 EODHD 旧收盘值/共享数组逻辑影响的问题;`v10.7.9.206` 稳定 BTC 连接态徽标,避免近期有效 tick 下临时显示“连接中”;`v10.7.9.207` 保留 BTC 首 tick 前第四卡位,避免首页只剩三张指数卡。中文默认显示、用户自写内容和核心交易/行情/数据库边界保持不变。
