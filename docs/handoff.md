...
- 当前本地发布候选: `v10.7.9.327`;个股详情新增基于正式交易/收盘快照和已登录 QQQ 普通收盘数据的真实收益对比。同一部署包含独立提交的 `v10.7.9.326` 真实美股收盘涨/跌幅榜与已添加股票减号修复,等待 sensitive 门禁和生产部署证据。
...
- 当前已验证生产仍为 `v10.7.9.325`,production runtime `05a0b9198d386ef1cafd95d28ad7c0083a271be0`,入口 `/assets/index-BXaiQwsS.js`;v327 上线后必须回填新 runtime、Actions、Vercel target 和入口。
...
- 本地设置页版本: `v10.7.9.327`;当前生产设置页版本仍为 `v10.7.9.325`。生产运行时基准为 `05a0b9198d386ef1cafd95d28ad7c0083a271be0`,入口 `/assets/index-BXaiQwsS.js`。
- `v10.7.9.327` 发布候选验证:toolchain、收盘榜/收益对比/API/分页/边界定向 94/94、完整测试 295/295、build、high audit 0 vulnerabilities、docs/diff、匿名 RLS 20/20 和 12-function 上限均 pass;真实 server-only QQQ EOD 探针返回 28/28 条有效 `rawClose`,no-store 生效且不泄露 key。已在本机 Xcode iOS 26.5 `iPhone 17 Pro` Simulator 只读 `DevVisualPreview` 使用 NVDA 验收主卡、首笔买入/对比起点小字、方法弹窗、分享预览、后续加仓/减仓成本流与纵向滚动。个股侧沿用现有正式收盘快照口径,QQQ 侧明确使用普通收盘价;公司行动附近可能存在口径差异,当前不做调整价估算或伪造补偿。
- `v10.7.9.326` 发布候选验证:market-movers/边界定向 55/55、完整测试 264/264、build、零 production-fixture marker、真实 EODHD + Nasdaq Trader 交集后 30/30 同日收盘探针和 iOS 26.5 iPhone 17 Pro Simulator 只读视觉/交互均 pass;`MSPR` / `BLMZF` 等已转 OTC/退市代码已从真实榜单排除。正式推送前还需重跑 sensitive 全套门禁,推送后核对三条未登录 quote/earnings/movers `401` 和已登录榜单。
...
- 当前 GitHub `main`: 以本文件所在最新交接证据提交为准,接手后执行 `git log -1 --oneline`;最近已上线运行时代码提交为 `05a0b9198d386ef1cafd95d28ad7c0083a271be0`。
- 当前生产运行时基准提交: `05a0b9198d386ef1cafd95d28ad7c0083a271be0`。
- 当前本地设置页版本为 `v10.7.9.327`;生产仍为 `v10.7.9.325`,等待本轮正式部署验证。
...
- 最新运行时部署: `npm run verify:deploy-status -- 05a0b91` pass;GitHub Actions run `29225315430` success,Vercel status success,target `https://vercel.com/chenshuai1190-7580s-projects/boduan-tracker/8Pz4D2XZAKYHGLN84vXHB2UK49Db`;production alias 已更新,入口 `/assets/index-BXaiQwsS.js`。
...
- 线上关键验证: 当前生产未登录 quote、earnings 均为 `401`;生产 `HomeTab-fnYwGkqp.js`、`SettingsTab-KA5_cIMI.js`、`settingsChangelog-CsPcDcVn.js` 和入口与 v325 本地 build SHA-256 一致。v327 上线后必须追加未登录 market-movers 和 pnl-benchmark `401`、已登录真实 30/30 榜单、真实 QQQ 普通收盘数据和新版产物 marker。
...
- 当前本地设置页版本为 `v10.7.9.327`,生产仍为 `v10.7.9.325`。v327 新增个股收益与 QQQ 真实同期现金流对比;同一部署包含 v326 真实收盘涨跌榜和已添加减号修复。
- 当前 GitHub source 基准提交: 以本文件所在最新交接证据提交为准,接手后执行 `git log -1 --oneline`;当前已验证生产运行时代码提交为 `05a0b9198d386ef1cafd95d28ad7c0083a271be0`。
- 当前生产运行时基准提交: `05a0b9198d386ef1cafd95d28ad7c0083a271be0`。
- 最近应用代码提交: 本文件所在待发布提交包含 `v10.7.9.327` 个股收益对比,其上一独立提交包含 `v10.7.9.326` 真实收盘涨跌榜;当前生产 `05a0b9198d386ef1cafd95d28ad7c0083a271be0` 包含 `v10.7.9.325` 财报弹窗默认列表视图。
...
- 当前本地/待发布设置页版本: `v10.7.9.327`;当前生产设置页版本: `v10.7.9.325`。
- Vercel 最新部署: `v10.7.9.325` runtime commit `05a0b9198d386ef1cafd95d28ad7c0083a271be0` 已 success,target `https://vercel.com/chenshuai1190-7580s-projects/boduan-tracker/8Pz4D2XZAKYHGLN84vXHB2UK49Db`,production 入口 `/assets/index-BXaiQwsS.js`,财报/设置/更新日志 markers 与产物一致性已验证。
...
- 设置页版本为 `v10.7.9.327`;若部署证据回填提交尚未出现,生产仍可能暂时显示 `v10.7.9.325`。
- 当前已验证生产运行时为 `05a0b9198d386ef1cafd95d28ad7c0083a271be0`,入口 `/assets/index-BXaiQwsS.js`;接手后以最新部署证据为准。
...
当前生产基准:
- 运行时代码: `05a0b9198d386ef1cafd95d28ad7c0083a271be0`
- 设置页版本: `v10.7.9.327`（待部署;当前生产 `v10.7.9.325`）
- 生产入口: `/assets/index-BXaiQwsS.js`
- Runtime Actions: `29225315430` success
- Runtime Vercel: `https://vercel.com/chenshuai1190-7580s-projects/boduan-tracker/8Pz4D2XZAKYHGLN84vXHB2UK49Db` success
