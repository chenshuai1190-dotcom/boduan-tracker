...
- 当前 GitHub `main`: 本文件所在最新提交为准;`v10.7.9.193` 股票实时连接首包重连正在随本次代码提交推进部署。
- 当前生产运行时代码提交: `v10.7.9.193` 部署前仍以最近已验证的 `v10.7.9.192` source commit `c6cb3a95d28e754a6ab6f8977497d84591e3035c` 为线上基准;本次部署完成后需回填真实 production source commit。
- 设置页版本: `v10.7.9.193`。
...
- 最近交接文档刷新部署: 本文件所在最新提交为准;`v10.7.9.193` 部署完成后需回填 production marker。
- 线上关键验证: `v10.7.9.193` 部署前仍参考上一轮 `v10.7.9.192` 验证:生产入口 `/assets/index-Bmtf1M7G.js`;entry imports `App-Bu4RlFxR.js`、`SettingsTab-BUZvbevv.js`、`settingsChangelog-BhWZoWnz.js`、`HomeTab-Qi2A0DUW.js` and `TradesTab-DVzcLE6A.js`;`SettingsTab-BUZvbevv.js` contains `v10.7.9.192`;`settingsChangelog-BhWZoWnz.js` contains `v10.7.9.192` and `盘前股票盘口兜底`;`App-Bu4RlFxR.js` contains `EODHD_WS_QUOTE` and `/api/stocks-realtime`;未登录 `GET /api/quote?symbols=VIX` 返回 `401` 且 `cache-control: private, no-store, max-age=0, must-revalidate`;普通 HTTP `GET /api/stocks-realtime` 返回 `426` 且 `cache-control: no-store`。本次 `v10.7.9.193` 需验证 `STOCK_REALTIME_FIRST_TICK_TIMEOUT_MS`、`股票实时首包超时`、设置页版本和 API 边界。
- 当前产品焦点: 英文模式已分阶段覆盖设置页、底部导航、首页、交易页、资产页和目标页。`v10.7.9.176` 起股票涨跌幅按现价和昨收重算;`v10.7.9.177` 到 `v10.7.9.182` 主要处理启动、iOS PWA 回前台和三套 realtime 恢复重连;`v10.7.9.183` 起保护实时 tick 缺少有效昨收时不覆盖完整行情行;`v10.7.9.185` 起按券商口径优先使用日线基准;`v10.7.9.189` 起正式拆分“展示当前价”和“今日盈亏计算价”:盘前/盘中今日盈亏随实时价更新,盘后/夜盘今日盈亏锁定到常规盘收盘价,盘后现价仍可显示并影响市值/持仓盈亏;`v10.7.9.190` 起首页/交易页“收盘锁定”状态标签改为中性灰色;`v10.7.9.191` 起英文首页/交易页头部持仓数量只显示 holdings 并恢复中文同款三列间距;`v10.7.9.192` 起股票 realtime relay 保留 `/ws/us` 成交价为主源,新增 `/ws/us-quote` bid/ask 中间价作为盘前成交稀疏时的兜底,且最近成交 tick 15 秒内优先于盘口中间价;`v10.7.9.193` 起股票 WebSocket 打开后 8 秒内没有首个 `stock_tick` 会自动重建连接,无任何 live/tick activity 的连接不再停在静态 REST 快照。用户自写内容、中文显示、持仓/交易计算、交易账本、摊薄工具、行情鉴权和数据库结构不变。
...
- 当前生产运行时基准提交: `v10.7.9.193` 部署前仍以最近已验证的 `v10.7.9.192` source commit `c6cb3a95d28e754a6ab6f8977497d84591e3035c` 为线上基准;部署完成后需回填真实 production source commit。
- 最近应用代码提交: 本文件所在最新提交包含 `v10.7.9.193` 股票实时连接首包重连;上一轮生产 `v10.7.9.192` 应用代码提交为 `c6cb3a95d28e754a6ab6f8977497d84591e3035c`。
- 最近文档/配置记录提交: 本文件所在最新提交。
- 设置页版本: `v10.7.9.193`。
- Vercel 最新运行时部署: `v10.7.9.193` pending;上一轮 `v10.7.9.192` production marker verified。
- 最近交接文档刷新部署: 本文件所在最新提交为准;`v10.7.9.193` 部署完成后需回填 production marker。
