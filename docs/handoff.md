...
- 当前开发版本 `v10.7.9.352`，生产仍为 `v10.7.9.351`，本轮 UI-fast 正在发布。v352 只把顶部日榜指标恢复为“本日收益率”、榜单左标题恢复为“收益率排行榜”；实际收益率、QQQ、跑赢 QQQ 排序、快照与右下真实截止日期完全不变。v351 已通过受保护 late-retry 补齐 07/14 `8/8/0 missing`、07/15 `9/9/0 missing`，最新 marker 为 `2026-07-15`。
- 当前 runtime `69a882bb851ff6f627c8c6ed6d0a0e729c4feced`；Actions `29493223459`、Vercel `F5yRg7aV7PC6vbHe2XM3K7HWNiUc` success，生产入口 `/assets/index-DK7xUcq9.js`。生产 Settings/changelog 命中 v351，比赛 chunk 命中真实日期 UI；未登录 quote、earnings、competition day/status 和主/late-retry Cron 均为 `401`。iOS 主屏 PWA 最终视觉验收仍只认可本机 Xcode Simulator，当前生产数据与服务端链路已验证，未用桌面证据冒充视觉通过。
...
- 同一生产版本继续包含 `v10.7.9.347`：修复收益比赛第 9 个模拟账户被 raw high/low 门槛拒绝及榜单按绝对收益率而非跑赢 QQQ 幅度排序的问题。内部比赛接受正式账本正数成交价，不再用 provider 日内区间拒绝；精确 EOD、同一纽约日且 16:00 前写入、USD、不超卖、revision/hash/CAS 等安全规则保留。每位用户保留自然周期内自己的真实累计起点，QQQ 从该用户同日起算，榜单按“本人收益率 - 本人同期 QQQ 收益率”降序；新用户首份有效收盘后立即参加，但不重置旧用户日期或累计收益。v352 只恢复“收益率排行榜”展示标题，不改变该排序规则。
...
- 同一生产 runtime 包含 `v10.7.9.344`：ASML 财报合并改为精确同财季 `0q` Trends EPS 一致预期优先、Calendar 回退，并按最终 actual/estimate 重算差异与超预期；EPS 按报告币种、营收按 USD 分开标识。已公布但真实 actual revenue 缺失的事件在报告日起两天内继续有界补拉，partial null 不覆盖已有真实 actual，生产无 mock 或网页补数。
- 上一生产版本 `v10.7.9.343`：账户走势初次打开不再默认显示月份浮层，点击图表外只关闭小浮层；近 12 个月缺少早期快照时，从窗口内首个真实月份计算累计增长，缺月继续保持缺失。每位家庭成员的账户按当前余额折算人民币后降序展示，USD/HKD 复用现有每日汇率且不写回数据库顺序。其 runtime `e30b1773159548bb9806fbd4e694f5f987ff804f`、Actions `29442304541`、Vercel `AS2TjUyLbX9YSgtKjSWjQA2zcxsk` 和入口 `/assets/index-Lpb4oLJK.js` 均已验证。
- 上一生产版本 `v10.7.9.342`：资产页点击精确账户的名称/类型后，读取该 `accountId` 在 `balance_snapshots` 中的真实原币月末余额并显示近 12 个自然月走势；缺月不补零、不插值、不沿用旧余额，也不合并同名或同类型账户。右侧金额仍进入原有账户修改/删除弹窗。runtime `f6b53a834df5550bcdeef18e1181ddd8b382a9da`、Actions `29438656651`、Vercel `FvseyF5Gu2Dh3gJm8C7M4twbxRuE`、入口 `/assets/index-DN3icOHQ.js` 和未登录 401 均已验证。
- 上一生产版本 `v10.7.9.341`：EODHD 实测 ASML actual EPS `7.58`、estimate `7.98`、actual revenue `null`；缺失营收继续显示 `--`。EUR 营收按真实外汇数据换算 USD，盘前反应只复用现有登录态 WebSocket 新鲜报价并以前一交易日普通收盘价为基准，正式反应使用报告日普通 `close` 并优先锁定；`unknown` 时段不猜测，收盘后缺反应事件自动有界补读。生产路径不使用 mock、预期值或估算值冒充真实结果。runtime `ae9eb03e725d4a3407eca020b7832e94edd972c4`、Actions `29417637784`、Vercel `BmcXTYQaBfw7oZq5oSovXjqfXQ1W`、入口 `/assets/index-DAkXD4bD.js` 和 7 个关键生产产物一致性均已验证。
- 上一生产版本 `v10.7.9.340`：财报日历按 `America/New_York` 发布窗口只重读已到期且缺少真实 actual 的股票，五分钟限频并在 iOS 主屏 PWA 恢复、聚焦或恢复联网时重新检查。只有 EODHD 真实 EPS/营收 actual 才标记已公布，时间、预期、官网数字和 mock 绝不代替；当时 ASML 直接探针仍为 `actual=null`，该值只是 v340 发布时的历史探针。runtime `863fbf059c9fa20799865ebbd49a358ab073d23b`、Actions `29405821570`、Vercel `6bQGtGVpQMQD1AorHXhGAEfAC8zr` 和入口 `/assets/index-DWtNyKll.js` 已验证。
...
- 上一生产版本 `v10.7.9.337` 放大比赛本人昵称并收紧头卡底部留白;其 runtime `f3b354f06914ffede57b08c18d81bb95aa1815a7`、Actions `29362348502` 与 Vercel `EcTkB5yWLNTDZ1qvoSiiyHnsXpZe` 已验证。
- 上一生产版本 `v10.7.9.336` 把交易页持仓表的 8px 从“名称/代码”列让给“市值/数量”列,表格总宽和其余四列不变,千万级人民币市值可完整显示。
- 上一生产版本 `v10.7.9.335` 统一资产页走势图/账户分组卡和收益比赛五类主卡为首页不透明黑色 `#0b0f14` 与 10% 灰边,保留有业务含义的状态色。
- 上一生产版本 `v10.7.9.334` 精简比赛更新时间、补充本人昵称并统一页面宽度;真实 `asOfDate`、缓存、请求和比赛数据逻辑不变。
...
- 上一前端版本 `v10.7.9.331` 已上线主动投资价值分享卡同行布局;该功能继续包含在当前 v332 生产构建中。
...
- 当前已验证 production runtime 为 `69a882bb851ff6f627c8c6ed6d0a0e729c4feced`,入口 `/assets/index-DK7xUcq9.js`;GitHub Actions run `29493223459` success,Vercel target `F5yRg7aV7PC6vbHe2XM3K7HWNiUc` success。受保护 late-retry 已执行，07/14 与 07/15 分别完整为 `8/8`、`9/9`，marker 为 07/15。
...
- 当前本地设置页版本为 `v10.7.9.352`，生产仍为 `v10.7.9.351`，v352 正在按 UI-fast 发布；v351 runtime、Actions、Vercel、生产入口和生产补漏证据见本节最新部署记录。
...
- 当前 GitHub `main`: 数据库源提交 `0bc0ef2cf423e9f4ac91daafc9cf8c68ba3c7d16` 的生产 marker migration、metadata/grant 回读和 22 tables + 2 RPCs 门禁均已完成；最新已上线运行时代码提交为 `69a882bb851ff6f627c8c6ed6d0a0e729c4feced`，包含 v351 受保护即时补漏、v350 完整批次 gate/真实日期 UI 以及 v344-v349。
- 当前生产运行时基准提交: `69a882bb851ff6f627c8c6ed6d0a0e729c4feced`。
- 当前本地设置页版本为 `v10.7.9.352`，生产仍为 `v10.7.9.351`，v352 发布验证待回填。
...
- 最新运行时部署: `npm run verify:deploy-status -- 69a882b` pass;GitHub Actions run `29493223459` success,Vercel status success,target `https://vercel.com/chenshuai1190-7580s-projects/boduan-tracker/F5yRg7aV7PC6vbHe2XM3K7HWNiUc`;production alias 已更新,入口 `/assets/index-DK7xUcq9.js`。
...
- 线上关键验证: 当前生产未登录 quote、market-movers、earnings、pnl-benchmark、competition day/status、competition snapshot、P&L snapshot 和三个统一 scheduler 路径均为 `401`;生产入口为 `/assets/index-DK7xUcq9.js`。设置页/中英文 changelog 命中 `v10.7.9.351`，比赛 chunk 命中真实 `asOfDate` 文案。受保护 late-retry 已返回 `200`；07/14 为 `8/8/0 missing`，07/15 为 `9/9/0 missing`，最新 marker 已前移至 `2026-07-15`。
...
- 当前本地设置页版本为 `v10.7.9.352`，生产仍为 `v10.7.9.351`，v352 发布验证待回填。v351 已完成受保护即时补漏；v350 的完整批次 gate/真实截止日期、v349 的 PWA 等待缓存唤醒、v348 的真实完整 publication marker 和 v347 的正式成交价准入/QQQ 超额收益排序继续生效。
- 当前 GitHub source 基准提交: 以本文件所在最新交接证据提交为准,接手后执行 `git log -1 --oneline`;当前已验证生产运行时代码提交为 `69a882bb851ff6f627c8c6ed6d0a0e729c4feced`。
- 当前生产运行时基准提交: `69a882bb851ff6f627c8c6ed6d0a0e729c4feced`。
- 最近已部署应用代码提交: `69a882bb851ff6f627c8c6ed6d0a0e729c4feced` 包含 v351 受保护即时补漏及 v350 完整批次 gate/真实日期 UI；上一 runtime `d6011a2cdb7443a2bf7680b261faa73fb196047f` 为 v350 首次上线。
...
- 当前设置页版本: `v10.7.9.352`；生产暂为 v351，v352 只恢复两处用户确认的展示标题，发布证据待回填；v351 的生产补跑 `8/8` / `9/9` 和 marker 07/15 已核验。
- Vercel 最新部署: runtime commit `69a882bb851ff6f627c8c6ed6d0a0e729c4feced` 已 success,target `https://vercel.com/chenshuai1190-7580s-projects/boduan-tracker/F5yRg7aV7PC6vbHe2XM3K7HWNiUc`,production 入口 `/assets/index-DK7xUcq9.js`;GitHub Actions run `29493223459` success。生产 Settings/changelog/competition chunks 和未授权 API/Cron 边界均已验证。
...
- 设置页: 当前版本见第 1 节;多账户切换、语言、显示、改密、社区资料、头像选择和管理员邀请码逻辑继续保留。
...
- 本地与生产设置页版本均为 `v10.7.9.351`；以后续真实提交和部署证据为准。
- 当前已验证生产运行时为 `69a882bb851ff6f627c8c6ed6d0a0e729c4feced`,入口 `/assets/index-DK7xUcq9.js`;接手后以最新部署证据为准。
...
当前准备发布修复:
- 设置页版本: `v10.7.9.352`
- 修复目标: 顶部日榜指标恢复“本日收益率”，不把日期加入该标题；榜单左标题恢复“收益率排行榜”。
- 不变边界: 右侧收益率/跑赢 QQQ 两列、真实排序算法、QQQ 基准、快照、缓存、API 和右下“数据截至 MM.DD 收盘”均不变。

上一版已发布修复（v351）:
- 修复目标: 用现有受保护 late-retry Cron 立即选择最近已完成收盘日，并从最后真实锁定快照顺序补齐系统遗漏的 07/14、07/15；完整后立即发布。
- 发布门槛: 目标日必须覆盖全部 active、资料完整且已开始排名的成员。`7/8` 或 `8/9` 返回可重试状态且不发布，补到 `8/8`、`9/9` 后 marker 才前进；later-start 未排名成员不误阻塞历史日。
- 真实日期: v352 只在右下显示 `数据截至 MM.DD 收盘`；marker 的完成动作时间只保留为 API 审计/缓存字段，不冒充收益日期。
- 生产结果: 受保护 late-retry 已返回 `200`；07/14 `8/8/0 missing`，07/15 `9/9/0 missing`，最新 marker `2026-07-15`。没有用模拟、估算或人工收益补数。
...
- 当前状态: v349 比赛专项 `178/178`、完整测试 `467/467`、build/toolchain/audit/docs/workspace/RLS/diff、GitHub Actions/Vercel、73 个生产文件一致性和未授权边界均已通过。修复后的 iOS 主屏 PWA 登录态视觉验收因 Mac 锁屏仍 pending，未用桌面证据替代。

当前生产基准:
- 运行时代码: `69a882bb851ff6f627c8c6ed6d0a0e729c4feced`
- 生产设置页版本: `v10.7.9.351`
- 生产入口: `/assets/index-DK7xUcq9.js`
- Runtime Actions: `29493223459` success
- Runtime Vercel: `https://vercel.com/chenshuai1190-7580s-projects/boduan-tracker/F5yRg7aV7PC6vbHe2XM3K7HWNiUc` success
- 生产数据: 07/14 expected/complete/missing `8/8/0`；07/15 `9/9/0`；latest competition marker `2026-07-15`
- 最新 docs evidence: 本文件所在 docs-only 提交（已回填 v351 runtime、Actions、Vercel、生产入口、受保护补漏与聚合回读证据）
...
- 未带 `CRON_SECRET` 三个 `/api/close-snapshot-schedule*`: 均为 `401`

同一生产 runtime 包含 `v10.7.9.347`（以及 v346/v345/v344）:
- 收益比赛内部成交价不再受 provider raw high/low 区间限制；仍只接受正式 USD 账本正数成交价，并保留精确目标日正数 EOD、纽约同日 16:00 前数据库时间、字段/顺序/不超卖、revision/hash/CAS、连续快照与 insert-only 边界。
- 日榜按当日；周/月/年榜为每位用户保留自然周期起点与本人固定排名起点中较晚者，并从本人同一起点计算 QQQ。榜单按个人收益减本人同期 QQQ 的超额收益排序，新用户首份有效收盘后立即加入，但不重置旧用户的日期或累计收益。参赛人数显示 active 且资料完整的报名总数，其他比例统计使用真实数据完整的排名集合；旧绝对收益率排序缓存由 cache v4 淘汰。
- UTC `21/22/23` 三个自动入口将统一 rewrite 到同一个受 `CRON_SECRET` 保护的收盘 scheduler；同一调用只解析一次美东目标日，再并行启动个人收益报表和收益比赛两个保持独立模型、表、D1/D2、revision CAS、hash 与锁顺序的 runner。原个人/比赛 endpoint 继续保留为受保护人工修复路径。
- 比赛只有完整批次结束后才写 `snapshot_publication_markers`。marker 仅含 channel、snapshot date、不透明 version 和 database-authored completed time，不含用户、排名、收益、交易或持仓数据；表启用 FORCE RLS，仅 service role 可读写，不加入 Supabase Realtime publication。已登录 `snapshot-status` 只返回脱敏状态；状态时间与最新 publication 使用独立 user-scoped metadata，四周期共享检查。完整榜、authoritative waiting 与明确清缓存通过 Web Lock 单调提交，旧标签页不能回滚新 marker。
- 当前比赛核心专项 `108/108`、完整 `npm test` `464/464`、build、toolchain、audit high、docs consistency 和 diff check 已通过。生产 marker SQL metadata/grant、匿名 `22 tables + 2 RPCs`、runtime Actions/Vercel、33 个产物一致性与生产未授权边界均已通过；iOS 主屏 PWA 因 Mac 锁屏仍 pending，未用桌面证据替代。
- sensitive 发布顺序已完成数据库源提交 → 生产 marker migration/metadata/`22 tables + 2 RPCs` 门禁 → v347-v351 runtime/线上验证。历史系统缺口已由 v351 受保护 late-retry 正式补齐并以 `8/8`、`9/9` 聚合回读验证，不得改写为模拟或人工补数。

已发布 `v10.7.9.349`:
- cache v5 淘汰可能卡住的 v4 waiting cache；只有合资格、当前窗口已耗尽两次完整读取且尚无 status check 的等待页才启动轻量状态轮询。
- marker 未推进时每分钟最多读取一次四字段状态，不读取完整榜；marker 日期或版本推进后才读取一次带 expected-publication 校验的完整榜。
- runtime `9709638b7501964b9c006488cb94d438ee40945a`、Actions `29490534414`、Vercel `BXzTNc6GRu6XJqYisgv7YwYcQpWG`、入口 `/assets/index-DnXeydcq.js`、73 个生产文件和未授权边界均已验证；没有写数据库或修改比赛数据。

已发布 `v10.7.9.348`:
- marker 升级前的真实锁定快照没有被删除；空 marker 是 API 返回 waiting 的直接原因。
- bootstrap 只会发布最新 exact complete locked batch，且只写一条不含用户或财务数据的 marker。它不修复、补造或改写任何 snapshot，不改 ranking/收益公式/交易账本。
- database-first 顺序已完成：精确 SQL 源已提交并核验，生产 SQL 已应用，marker 与 23 条快照不变性已回读，runtime/docs、Actions/Vercel 和线上静态产物均已验证。

最新已部署 sensitive 变更:
- `v10.7.9.332` 已上线按账户/周期隔离的比赛权威快照缓存和新头部布局;客户端不生成、不修正也不伪造收益,动态日期只读取服务端 `asOfDate`。上一条 backend-only 发布时设置页版本为 `v10.7.9.331`,其 ET gate/rebaseline 安全规则继续有效。
...
最近完成:
- `v10.7.9.347`: 已上线比赛超额收益排行与内部成交规则修正。第 9 个模拟账户并非 8 人上限，而是正式 SPY 成交价低于 raw low `0.13 USD` 后被旧门槛拒绝并产生快照缺口；v347 仅取消该 raw high/low 门槛，保留真实收盘和完整账本安全边界。新用户首份有效收盘后即可进入全部榜单；每位用户保留本人自然周期内的真实累计起点，QQQ 从本人同一起点计算，榜单按个人收益减本人同期 QQQ 的超额收益排序。标题精简为“跑赢 QQQ”。runtime `1b07a7d`、Actions `29482122875`、Vercel `7uyffcqEZQA4yh8cKkT69DsL1ZwJ` 和 33 个生产产物一致性均通过；显式 07-14/07-15 补跑尚未执行。
- `v10.7.9.346`: 已随 v347 上线收盘快照同步与比赛按需刷新。三个自动入口统一进入同一受保护 scheduler，同一调用只解析一次美东目标日并独立并行启动个人与比赛 runner；只有比赛完整批次成功后才发布不含用户数据的 service-only marker。可见旧榜每分钟最多读取一次轻量 `snapshot-status`，四周期共享最新 publication，只有 marker 推进才读当前周期完整榜；Web Lock、持久清缓存 generation 和单调 publication 防止旧标签页回写。生产无循环式全榜单重读、无 Supabase Realtime marker、无 mock 或客户端收益修正。database-first 源提交、生产 RLS/SQL、migration、`22 tables + 2 RPCs`、runtime Actions/Vercel、产物验证和未授权边界均已完成；iOS 主屏 PWA 因 Mac 锁屏仍 pending。
- `v10.7.9.345`: 已随 v347 上线个人账户零余额语义修正。某月余额保存为 `0` 或空白会幂等删除本人精确账户月份记录，未触碰余额不误删；历史零行按不存在处理，走势从剩余第一个正数月份重新起算，最低/最高资产、环比和累计增长同步重算。删除按当前用户、账户、月份三重限定并同步清理离线缓存；不批量改写生产历史数据，不影响其他账户、月份、用户或交易/收益/比赛账本。Mac 锁屏导致系统键盘保存 `0/空白` 的最终交互验收仍 pending，未用桌面证据替代。
- `v10.7.9.344`: 已随 v347 上线财报真实数据修正。精确同财季 `0q` Trends EPS 一致预期优先，Calendar 仅作回退，并按最终 actual/estimate 重算超预期；EPS 按报告币种、营收按 USD 分开标识。已公布但真实 actual revenue 缺失时，报告日起两天内有界补拉，保留五分钟节流及 PWA 恢复检查；partial null 不覆盖既有真实 actual，生产不用 mock 或网页数字补 actual。
- `v10.7.9.343`:账户走势初次打开不显示月份浮层，点击图表外只关闭小浮层；近 12 个月缺少早期快照时，从窗口内首个真实月份计算累计增长，缺月不补值。每位家庭成员的账户按当前余额人民币等值降序展示，USD/HKD 复用现有每日汇率且不写回数据库排序。定向 54/54、完整 410/410、toolchain、build、audit high、docs consistency、diff check 和 iPhone 17 Pro Simulator 只读预览均已通过；runtime `e30b1773159548bb9806fbd4e694f5f987ff804f`、Actions `29442304541`、Vercel `AS2TjUyLbX9YSgtKjSWjQA2zcxsk`、入口 `/assets/index-Lpb4oLJK.js`、8 个关键生产产物一致性、功能 marker、开发 fixture 缺席和未登录 401 均已通过并上线。
- `v10.7.9.342`:资产页点击精确账户名称/类型后，只读取该账户 `balance_snapshots` 的真实原币月末余额并显示近 12 个自然月走势；缺月不补零、不插值、不沿用旧余额，也不合并其他账户。右侧金额继续进入原有修改/删除弹窗。定向测试、完整 410/410 测试、build、toolchain、audit high、diff check 与 iPhone 17 Pro Simulator 只读截图均已通过；runtime `f6b53a834df5550bcdeef18e1181ddd8b382a9da`、Actions `29438656651`、Vercel `FvseyF5Gu2Dh3gJm8C7M4twbxRuE`、入口 `/assets/index-DN3icOHQ.js`、`index.html` 与 30 个生产 assets 字节一致、功能 marker、开发 fixture 缺席和未登录 401 均已通过并上线。
- `v10.7.9.341`:ASML EODHD 真实 actual EPS `7.58`、estimate `7.98`，actual revenue 仍为 `null` 并显示 `--`；EUR 营收按真实外汇数据换算 USD。盘前反应复用现有登录态 WebSocket 新鲜行情并以前一交易日普通收盘价为基准，正式收盘后切换并锁定报告日普通 `close` 反应；`unknown` 时段不猜测，收盘后缺失反应自动有界补读。EPS 结果标记移回实际 EPS 旁，不用 mock、预期值或估算值冒充真实数据。完整测试 403/403、build、toolchain、audit high、RLS REST、diff check 与 iPhone 17 Pro Simulator 只读截图均已通过；runtime `ae9eb03e725d4a3407eca020b7832e94edd972c4`、Actions `29417637784`、Vercel `BmcXTYQaBfw7oZq5oSovXjqfXQ1W`、入口 `/assets/index-DAkXD4bD.js`、7 个关键生产产物一致性、新版 marker、开发 fixture 缺席和未登录 401 均已通过并上线。
...
- `v10.7.9.337`:收益比赛“我的排名”卡把头像下本人昵称由 10.5px 放大为 15px 半粗体高对比文字,“最后更新”相对放大昵称后的初版稿上移约 8px,卡片底部留白收紧 4px,相对上一张本地效果卡片整体降低约 12px。iPhone 17 Pro Simulator、47/47 定向测试、build、Actions/Vercel、7 个生产产物一致性和未授权边界均通过;头像、排名、三项收益指标、真实日期、缓存、请求、排名、收益、快照、交易账本、API 和安全边界不变。
- `v10.7.9.336`:交易页持仓表把“名称/代码”列由 100px 缩为 92px,把“市值/数量”列由 80px 增为 88px;总宽和其余四列不变,千万级人民币市值完整显示。iPhone 17 Pro Simulator 已用 `10,026,370.00` 边界样例验收,47/47 定向测试、build、Actions/Vercel、7 个生产产物一致性和未授权边界均通过;金额计算、汇率、持仓数据、交易账本、API 和安全边界不变。
- `v10.7.9.335`:资产页走势图/账户分组卡与收益比赛状态卡、排名头卡、统计卡、排行榜和 QQQ 基准卡统一为首页标准黑色/灰边;保留本人排行高亮、资料弹卡和参赛弹窗状态色。只改展示,资产数据、比赛缓存、请求、排名、收益、交易账本、API 和安全边界不变;已通过 iPhone 17 Pro Simulator、UI-fast、Actions/Vercel、生产产物一致性和未授权边界验证并上线。
...
- 收益比赛 D1 只能为 0 snapshots + 0 ranking 成员向前重建 eligibility,不得写收益;SPY 和所有相关个股都必须有精确正数 D1 EOD,D1 当日交易必须由数据库时间证明为同一纽约日期且在 16:00 前创建。内部比赛接受正式账本正数成交价，不再校验 raw high/low；缺行、旧 close、malformed、空/非 USD、超卖或无效 close 绝不 RPC、忽略或估算。D2 下一真实收盘才可能初始化排名。已有锁定快照绝不覆盖,显式日期绝不 rebaseline。
- 所有自动快照无显式日期时使用美东日历;工作日 17:00 前只 defer。UTC 21/22/23 是冗余触发而不是手机时区规则；待发布 v346 的三个自动 path 必须统一到同一 `CRON_SECRET` 保护 scheduler，同次只解析一次目标日，再启动仍保持数据、计算与结果独立的个人/比赛 runner。原个人/比赛 endpoint 只作受保护人工修复路径。
- 比赛发布 marker 只能在完整批次结束后由 service role 写入；不得包含 user id、昵称、排名、收益、交易、持仓或其他用户数据，不得授予浏览器角色直接表权限，也不得加入 Realtime publication。已登录状态 API 只返回日期、不透明版本和数据库完成时间；部分批次、失败批次或 batch-limited 结果不得推进 marker。浏览器缓存必须保持 publication 单调，跨标签页旧响应不得覆盖新 marker；QQQ/完整榜不完整时保留旧完整榜并有界重试，不能缓存半成品。
...
当前优先事项:
1. v349 PWA cache v5、生产部署、73 个文件一致性和未授权边界均已完成；下一步用已登录 iOS 主屏 PWA 完全退出再打开，确认旧 waiting cache 被淘汰并显示 `2026-07-13` 真实榜单。Mac 锁屏期间不得用桌面或内置浏览器替代这一视觉验收。v348 database-first bootstrap、生产 marker/快照不变回读和 `22 tables + 2 RPCs` 门禁继续有效；正式美东 17:00 后 scheduler 仍应按真实交易日追赶 2026-07-14、2026-07-15 及后续缺口，只有 exact complete batch 才推进 marker，绝不得覆盖现有锁定行。
2. Competition rebaseline SQL 源、上一轮生产迁移/回读、revision row→membership row 双事务并发 smoke、21 tables + 2 RPCs RLS gate、v343 runtime SHA/Actions/Vercel 和未登录 401 均已验证;当前仍等待真实 scheduled D1/D2 收盘观察。
3. 观察 scheduled D1 只前移基线且不写收益,再在下一真实收盘 D2 验证账本未变成员才获得首张锁定快照;只报告聚合,不得暴露 user id 或交易明细。
4. 完成其余用户表 RLS metadata 审计和登录隔离 smoke;`swing_waves` 已完成 metadata 13/13、双真实 Auth 用户 14/14 且无残留。
5. 保持财报日历、收益报表、个股详情、正式交易账本、波段、摊薄工具和行情 relay 的独立边界，并继续拆 `src/App.jsx`、`src/lib/db.js` 和 `server/quote/providers/eodhd.js`。
```
