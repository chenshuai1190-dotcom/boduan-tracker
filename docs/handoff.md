...
| 生产运行时代码 | `0246bf0336b21fad40d7d37199ff55f292863401` |
| 设置页版本 | `v10.7.9.361` |
| 生产入口 | `/assets/index-IqK5bLo1.js` |
| Runtime Actions | `29692007690` success |
| Runtime Vercel | `3mhNMVPWUwhkXPYTPBJUo7HfiHrg` success |

已验证：

- `npm run verify:deploy-status -- 0246bf0`：PASS。
...
- 详情页中文标题为“股票趋势”，保留五栏底部导航和安全留白；目标价整卡按压缩放已取消，点击编辑不变。
...
- 生产详情页分包 `/assets/WatchlistStockDetailPage-BityBpi9.js` 返回 `200`，包含 `weeklyHistory`、`ma200WeeklyStatus`、周线 SVG 与细线标记，且旧末端价格气泡填充标记已不存在；Settings/changelog 为 `v10.7.9.361`。
