...
- 最新接手补充: `v10.7.9.235` 股票代码输入严格校验已上线。本轮修复已注册用户历史交易中 `N VDA` 这类内部空格 ticker 打爆整批 Quote API 的问题:旧数据读取会把短 ticker 容错修复为 `NVDA`,新建/编辑交易和自选写入严格拒绝含空格或特殊字符的股票代码。登录权限、邀请码规则、交易收益计算、收益快照生成、行情 relay、RLS 和 `/api/quote` 鉴权不变。
- 当前 GitHub `main`: 本文件所在最新提交为准;`v10.7.9.235` 应用代码已通过项目 SSH key 推送并完成生产 marker 验证。
- 当前生产运行时代码提交: `24172f86abfad32549765b737b5cc43b6659a610`;`v10.7.9.235` production marker verified。
- 设置页版本: `v10.7.9.235`。
- 当前生产地址: `https://boduan-tracker.vercel.app`。
- 最新 Vercel 状态: `24172f8` production marker verified at `https://boduan-tracker.vercel.app`;production alias serving `v10.7.9.235` runtime assets。
- 最近交接文档刷新部署: 本文件所在最新提交为准;`v10.7.9.235` 运行时代码已部署成功,本轮文档回写为生产验证补充。
- 线上关键验证: 当前线上为 `v10.7.9.235`,生产入口 `/assets/index-DGPfGNHJ.js`;runtime chunks include `App-DYwj-VMj.js`,`HomeTab-BUKTqZAG.js`,`TradesTab-BbEKShMl.js`,`StockDetailPage-BaGWfmXc.js`,`SettingsTab-Cli_PHce.js`,`settingsChangelog-Np_TrEFT.js`,`i18n-D6gQC1Ov.js`,`symbols-BLHitJ3E.js`;production assets contain `v10.7.9.235`,`股票代码输入严格校验`,`股票代码格式不正确`,`不要包含空格`,`Invalid ticker format`;未登录 `GET /api/quote?symbols=VIX` 返回 `401`;HTTPS 普通非 WebSocket `GET /api/stocks-realtime`、`GET /api/indices-realtime`、`GET /api/btc-realtime` 返回 `426`。
...
- 当前生产运行时基准提交: `24172f86abfad32549765b737b5cc43b6659a610`;`v10.7.9.235` production marker verified。
- 最近应用代码提交: `24172f86abfad32549765b737b5cc43b6659a610`,本轮股票代码输入严格校验已推送部署。上一轮个股详情字段标题和普通数值层级回调到交易页同款提交为 `244571fc80586b6c36c0156e14713eae4d07b27d`,已推送部署。上一轮个股详情文字层级微调提交为 `b8fc9a62f1802460ff71ff28865ec6d6ee55f1d6`;上一轮个股收益线滑动查看提交为 `630a738d1ca716d04f3d8930e37b5294b7347dbe`;上一轮个股详情视觉优化提交为 `6100a9a7c4303e65bb4fc30d598d2ee93c927c30`;上一轮个股详情只读页提交为 `3f448c805d2270c5931592f681ea907b62e5fb27`;上一轮自动收盘快照提交为 `ebb8090912ba373a48efb756bb85886e1d9f07c6`,环境变量配置记录/重部署提交为 `a066f6beda9e970f1efcbb14a1175e798015d864`,上一轮报表真实快照读取提交为 `1ca0a9684788b8b915ba4b035d1ac076987e7c1f`,上一轮报表快照基础提交为 `f8329b0f5bbad6837c7925d1ba8b19a1335dbe8a`,上一轮 BTC 卡位提交为 `95e8cc5ee25486e9ebee5e0a1d3e44b18d3d2133`。
- 最近文档/配置记录提交: 本文件所在最新提交。
- 设置页版本: `v10.7.9.235`。
- Vercel 最新运行时部署: 当前线上 success,`v10.7.9.235` production marker verified,production alias `https://boduan-tracker.vercel.app` serving runtime assets from production entry `/assets/index-DGPfGNHJ.js`。
- 最近交接文档刷新部署: 本文件所在最新提交为准;`v10.7.9.235` 运行时代码已部署成功,本轮文档回写为生产验证补充。
