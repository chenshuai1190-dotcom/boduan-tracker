...
- 当前本地候选为 `v10.7.9.328`,尚未提交、推送或部署。收益对比现在对个股和 QQQ 并行读取已登录 server-only EODHD `rawClose`,个人快照只核验日期/持仓股数;主卡将“超额金额”和“率差 +X.XX%”分开,图表触摸小浮层的个股/QQQ 行只显示收益金额并保留最终“收益率差 +X.XX%”,分享卡和复制文字继续显示完整收益率。口径明确为我的收益率减 QQQ 收益率,主动投资价值分享卡仅取消最外层白色描边。任一侧缺少必要普通收盘价时整卡不可用,不写账本/快照且生产无 mock 或估算兜底。
- 当前生产已上线 `v10.7.9.327`:个股详情新增基于正式交易/个人收盘快照和已登录 QQQ 普通收盘数据的真实收益对比。同一 runtime 包含独立提交的 `v10.7.9.326` 真实美股收盘涨/跌幅榜与已添加股票减号修复。
...
- 当前已验证 production runtime 为 `55e02c8faa8fdac7b57b44335b6edb5d153494a2`,入口 `/assets/index-DW_Jo82w.js`;GitHub Actions run `29269405728` 与 Vercel target `526DFjY3SKDmd9md2rpx9Cjoq9Ns` success。
...
- 本地候选设置页版本为 `v10.7.9.328`,生产设置页仍为 `v10.7.9.327`;生产运行时基准为 `55e02c8faa8fdac7b57b44335b6edb5d153494a2`,入口 `/assets/index-DW_Jo82w.js`。
...
- 当前 GitHub `main`: 以本文件所在最新交接证据提交为准,接手后执行 `git log -1 --oneline`;最近已上线运行时代码提交为 `55e02c8faa8fdac7b57b44335b6edb5d153494a2`。
- 当前生产运行时基准提交: `55e02c8faa8fdac7b57b44335b6edb5d153494a2`。
- 当前本地候选设置页版本为 `v10.7.9.328`,生产仍为 `v10.7.9.327`。
...
- 最新运行时部署: `npm run verify:deploy-status -- 55e02c8` pass;GitHub Actions run `29269405728` success,Vercel status success,target `https://vercel.com/chenshuai1190-7580s-projects/boduan-tracker/526DFjY3SKDmd9md2rpx9Cjoq9Ns`;production alias 已更新,入口 `/assets/index-DW_Jo82w.js`。
...
- 线上关键验证: 当前生产未登录 quote、market-movers、earnings、pnl-benchmark 均为 `401`;生产 `SettingsTab-DpMW-boE.js`、`StockDetailPage-CuSoiDTP.js`、`HomeTab-RVzG_gIx.js`、`i18n-CvwWM-GG.js`、`settingsChangelog-hc9z3NJB.js` 命中 v327、收益对比/首笔买入、涨幅榜/跌幅榜和中英文更新标题,运行时 chunks 不含本地财务 fixture 标识。真实 30/30 榜单与 QQQ `rawClose` 为发布前同代码 server-only 探针证据,不是伪称的生产登录态人工检查。
...
- 当前本地候选设置页版本为 `v10.7.9.328`,生产仍为 `v10.7.9.327`。v328 统一个股/QQQ 普通收盘价口径,补齐紧凑的收益率差百分比展示并取消分享卡外层白框;v327 新增个股收益与 QQQ 真实同期现金流对比,同一生产 runtime 包含 v326 真实收盘涨跌榜和已添加减号修复。
- 当前 GitHub source 基准提交: 以本文件所在最新交接证据提交为准,接手后执行 `git log -1 --oneline`;当前已验证生产运行时代码提交为 `55e02c8faa8fdac7b57b44335b6edb5d153494a2`。
- 当前生产运行时基准提交: `55e02c8faa8fdac7b57b44335b6edb5d153494a2`。
- 最近应用代码提交: `55e02c8faa8fdac7b57b44335b6edb5d153494a2` 包含 `v10.7.9.327` 个股收益对比,其上一独立提交 `4e56573afaf06680ed22bbbf90ea081e9b559479` 包含 `v10.7.9.326` 真实收盘涨跌榜。
...
- 当前本地候选设置页版本: `v10.7.9.328`;生产设置页版本: `v10.7.9.327`。
- Vercel 最新部署: `v10.7.9.327` runtime commit `55e02c8faa8fdac7b57b44335b6edb5d153494a2` 已 success,target `https://vercel.com/chenshuai1190-7580s-projects/boduan-tracker/526DFjY3SKDmd9md2rpx9Cjoq9Ns`,production 入口 `/assets/index-DW_Jo82w.js`,v327/v326 marker 与未登录 API 边界已验证。
...
- 本地候选设置页版本为 `v10.7.9.328`,生产设置页版本为 `v10.7.9.327`。
- 当前已验证生产运行时为 `55e02c8faa8fdac7b57b44335b6edb5d153494a2`,入口 `/assets/index-DW_Jo82w.js`;接手后以最新部署证据为准。
...
当前生产基准:
- 运行时代码: `55e02c8faa8fdac7b57b44335b6edb5d153494a2`
- 生产设置页版本: `v10.7.9.327`
- 生产入口: `/assets/index-DW_Jo82w.js`
- Runtime Actions: `29269405728` success
- Runtime Vercel: `https://vercel.com/chenshuai1190-7580s-projects/boduan-tracker/526DFjY3SKDmd9md2rpx9Cjoq9Ns` success
...
最近完成:
- `v10.7.9.328` 本地候选: 收益对比的个股和 QQQ 现在都经已登录 `/api/pnl-benchmark` 读取 EODHD 普通收盘价,个人收益快照只核验日期和持仓股数;任一侧缺数据时整卡 fail unavailable,不混用调整价/快照价。主卡保留金额+收益率并将超额金额与“率差 +X.XX%”分开;图表触摸小浮层隐藏个股/QQQ 各自百分比,保留超额金额与最终收益率差;分享卡继续显示完整收益率且仅取消最外层白色描边。尚未提交、推送或部署,生产仍是 v327。
