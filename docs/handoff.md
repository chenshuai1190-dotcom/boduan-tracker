...
- 当前发布候选为 `v10.7.9.338`:收益比赛本人昵称与“最后更新”改为同一水平行;昵称缩为 12px 并使用排行榜普通用户名同款灰白色,头像居中的安全宽度由 72px 增至 80px。iPhone 17 Pro Simulator 已确认“波段玩家1836”完整显示且右侧指标无挤压,47/47 定向测试、build、docs consistency 和 diff check 均通过;runtime、Actions、Vercel 与生产入口待部署后回填。
- 当前生产已上线 `v10.7.9.337`:收益比赛“我的排名”卡把头像下本人昵称放大为 15px 半粗体高对比文字,“最后更新”相对放大昵称后的初版稿上移约 8px,卡片底部留白收紧 4px。iPhone 17 Pro Simulator、47/47 定向测试、build、runtime `f3b354f06914ffede57b08c18d81bb95aa1815a7`、Actions `29362348502`、Vercel `EcTkB5yWLNTDZ1qvoSiiyHnsXpZe` 和 7 个关键生产产物均已验证。
...
- 当前已验证 production runtime 为 `f3b354f06914ffede57b08c18d81bb95aa1815a7`,入口 `/assets/index-DIcYEBHf.js`;GitHub Actions run `29362348502` success,Vercel target `EcTkB5yWLNTDZ1qvoSiiyHnsXpZe` success。
...
- 当前本地设置页版本为 `v10.7.9.338` 发布候选,生产仍为 `v10.7.9.337`;当前已验证生产运行时基准为 `f3b354f06914ffede57b08c18d81bb95aa1815a7`,入口 `/assets/index-DIcYEBHf.js`。
...
- 当前 GitHub `main`: 数据库源提交为 `0f52700761beab0d4488e067ca9e968aea9a9bc1`;最新已上线运行时代码提交为 `f3b354f06914ffede57b08c18d81bb95aa1815a7`。v337、v336、v335、v334、v333、v332 和此前 ET gate/rebaseline 的生产 SQL/runtime 均已部署并通过门禁。
- 当前生产运行时基准提交: `f3b354f06914ffede57b08c18d81bb95aa1815a7`。
- 当前本地设置页版本为 `v10.7.9.338` 发布候选,生产仍为 `v10.7.9.337`。
...
- 最新运行时部署: `npm run verify:deploy-status -- f3b354f` pass;GitHub Actions run `29362348502` success,Vercel status success,target `https://vercel.com/chenshuai1190-7580s-projects/boduan-tracker/EcTkB5yWLNTDZ1qvoSiiyHnsXpZe`;production alias 已更新,入口 `/assets/index-DIcYEBHf.js`。
...
- 线上关键验证: 当前生产未登录 quote、market-movers、earnings、pnl-benchmark、competition GET/POST、competition snapshot GET 和 P&L snapshot GET 均为 `401`;生产入口为 `/assets/index-DIcYEBHf.js`,v337 的 7 个关键产物与本地 production build SHA-256 一致。`2026-07-13` 的上一 runtime 历史快照聚合为个人 12 users / 12 portfolio rows、54 symbol rows / 18 symbols;比赛 8 users / 8 locked rows,active 9、eligible 9、initialized 8、invalid 0,另 1 名尚未排名成员 `eligible_ledger_hash_mismatch`。新 revision/CAS SQL、metadata、RLS、并发 smoke 和 ET-gated runtime 已有生产证据;真实 D1/D2 尚待收盘观察。
...
- 当前本地设置页版本为 `v10.7.9.338` 发布候选,生产仍为 `v10.7.9.337`。v338 优化比赛本人信息同行与昵称完整显示;v337 放大比赛本人昵称并收紧头卡;v336 完整显示千万级持仓市值;v335 统一资产页和比赛页主卡黑色。
- 当前 GitHub source 基准提交: 以本文件所在最新交接证据提交为准,接手后执行 `git log -1 --oneline`;当前已验证生产运行时代码提交为 `f3b354f06914ffede57b08c18d81bb95aa1815a7`。
- 当前生产运行时基准提交: `f3b354f06914ffede57b08c18d81bb95aa1815a7`。
- 最近已部署应用代码提交: `f3b354f06914ffede57b08c18d81bb95aa1815a7` 包含 v337 比赛本人昵称和头卡紧凑化;上一 runtime `76bf439b908f886af76487a2af1411a0f18f4ac6` 包含 v336 持仓市值长金额显示修复;更早 runtime `154daab23ea2712b1bd0717e2bc7696a62799b86` 包含 v335 资产页和比赛页主卡颜色统一。
...
- 当前本地设置页版本: `v10.7.9.338`（发布候选）；当前生产设置页版本仍为 `v10.7.9.337`。
- Vercel 最新部署: runtime commit `f3b354f06914ffede57b08c18d81bb95aa1815a7` 已 success,target `https://vercel.com/chenshuai1190-7580s-projects/boduan-tracker/EcTkB5yWLNTDZ1qvoSiiyHnsXpZe`,production 入口 `/assets/index-DIcYEBHf.js`;GitHub Actions run `29362348502` success。7 个关键产物与本地 build SHA-256 一致,quote/earnings 未授权边界均为 `401`。
...
- 本地设置页版本为 `v10.7.9.338` 发布候选,生产设置页版本仍为 `v10.7.9.337`。
- 当前已验证生产运行时为 `f3b354f06914ffede57b08c18d81bb95aa1815a7`,入口 `/assets/index-DIcYEBHf.js`;接手后以最新部署证据为准。
...
当前生产基准:
- 运行时代码: `f3b354f06914ffede57b08c18d81bb95aa1815a7`
- 设置页版本: `v10.7.9.338`（发布候选；当前生产仍为 `v10.7.9.337`）
- 生产入口: `/assets/index-DIcYEBHf.js`
- Runtime Actions: `29362348502` success
- Runtime Vercel: `https://vercel.com/chenshuai1190-7580s-projects/boduan-tracker/EcTkB5yWLNTDZ1qvoSiiyHnsXpZe` success
- 最新 docs evidence: 本文件所在提交（已回填 v337 runtime、Actions、Vercel、生产入口和产物一致性；接手后以 `git log -1` 核验）
...
最近完成:
- `v10.7.9.338` 发布候选:收益比赛本人昵称与“最后更新”改为同一水平行;昵称由 15px 缩为 12px,颜色与排行榜普通用户名一致,头像居中的安全宽度由 72px 增至 80px。“波段玩家1836”已在 iPhone 17 Pro Simulator 完整显示且右侧指标无挤压;更长昵称继续安全省略。47/47 定向测试、build、docs consistency 和 diff check 均通过,比赛数据、缓存、请求、排名、收益、快照、交易账本、API 和安全边界不变,待部署验证。
