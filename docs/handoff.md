...
- 当前 GitHub `main`: 本文件所在最新提交为准;本轮运行时代码提交为 `a2848a30bc12a8b97f0904bcaedeb6b886a526db`。
- 当前运行时代码提交: `a2848a30bc12a8b97f0904bcaedeb6b886a526db`。
- 设置页版本: `v10.7.9.148`。
- 当前生产地址: `https://boduan-tracker.vercel.app`。
- 最新运行时 Vercel 部署: success, production marker verified for runtime commit `a2848a30bc12a8b97f0904bcaedeb6b886a526db`。
- 最近交接文档刷新部署: 本文件所在提交推送后以 GitHub/Vercel 最新状态为准。
- 线上关键验证: 生产 active runtime assets 包含 `/api/stocks-realtime`、`/api/indices-realtime`、`/api/btc-realtime` 和 `stock_tick`,不含 `ws.eodhistoricaldata.com`、`VITE_EODHD_TOKEN` 或 `VITE_ALLOW_BROWSER_EODHD_WS`;Settings chunk 包含 `v10.7.9.148`;`AnalysisTab-Dye1C266.js` 包含首页/交易页同款头卡外壳、`mt-3` `#ffd18a` 金额和三列指标 marker;五个生产 PNG 图标仍为正确尺寸、RGB 不透明、四角深色;未登录 `GET /api/quote?symbols=VIX` 返回 `401`;普通 `GET /api/stocks-realtime` 返回 `426`。
- 当前产品焦点: 首页三大指数继续秒级更新价格和曲线但不再显示连接态,只有 BTC 卡显示实时连接状态;股票 WebSocket tick 写入 `quoteCache` 后统一驱动首页自选/持仓、交易页头部/主持仓、波段记录和摊薄工具现价;资产页家庭总资产头卡已在 `v10.7.9.148` 对齐首页/交易页的外壳尺寸、金额颜色和金额位置;自动启动、定时和回到前台触发的 `auto-silent + browser-network` 行情抖动只写 console,不再进入设置页行情诊断报警列表;手动刷新/下拉刷新和真实服务端/鉴权/限流/第三方错误仍记录并提示;`v10.7.9.147` 起 PWA/浏览器/iOS 图标改为不透明深色填充 PNG,避免 iOS 主屏用白色填充透明边缘;历史 `public/sw.js` 已删除且入口继续注销旧 SW;后续视觉任务务必先用本地 390×844 视口验证。
...
- 资产/分析页: 深色家庭总资产卡、12 个月走势、我/老婆账户分组、月度余额填报和新增账户。`v10.7.9.116` 起家庭总资产主数字改为完整金额 + 小号两位小数,其它走势图和账户列表仍保留 `万` 简写;`v10.7.9.148` 起资产页家庭总资产头卡尺寸、外壳、金额颜色和金额位置与首页/交易页头卡对齐。
...
仓库: `chenshuai1190-dotcom/boduan-tracker`
生产地址: https://boduan-tracker.vercel.app
当前 GitHub main: 以本文件所在最新提交为准;本轮运行时代码提交为 `a2848a30bc12a8b97f0904bcaedeb6b886a526db`
当前产品基准提交: `a2848a30bc12a8b97f0904bcaedeb6b886a526db` (`v10.7.9.148`,资产头卡对齐首页)
最近应用代码提交: `a2848a30bc12a8b97f0904bcaedeb6b886a526db`
设置页版本: `v10.7.9.148`
Vercel 最新运行时部署: success, `v10.7.9.148` production marker verified for runtime commit `a2848a30bc12a8b97f0904bcaedeb6b886a526db`
最近交接文档刷新部署: 本文件所在提交推送后以 GitHub/Vercel 最新状态为准
部署记录: `v10.7.9.148` runtime commit `a2848a30bc12a8b97f0904bcaedeb6b886a526db`;GitHub Actions run `28767430672` pass;production `GET https://boduan-tracker.vercel.app/?v=a2848a3-asset-header-*` HTTP 200 with `last-modified: Mon, 06 Jul 2026 04:18:42 GMT`;production marker check confirms loaded `App-CDi4ytOG.js`, `AnalysisTab-Dye1C266.js`, `SettingsTab-CjEZrIz3.js`, `settingsChangelog-xXhOm-_J.js`, `HomeTab-BtNukBtx.js`, `TradesTab-DR-WtnYb.js` 和相关 chunks;Settings chunk contains `v10.7.9.148` and changelog contains `资产头卡对齐首页`;`AnalysisTab-Dye1C266.js` contains the shared home/trade header shell marker, the `mt-3` `#ffd18a` amount marker and the shared metric grid marker;active runtime assets contain `/api/stocks-realtime`, `/api/indices-realtime`, `/api/btc-realtime` and `stock_tick`,and active runtime assets outside historical changelog do not contain `ws.eodhistoricaldata.com`, `VITE_EODHD_TOKEN` or `VITE_ALLOW_BROWSER_EODHD_WS`;unauthenticated `/api/quote?symbols=VIX` returns `401`;plain `/api/stocks-realtime` returns `426`
