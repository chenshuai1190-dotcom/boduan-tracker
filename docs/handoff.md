...
- 当前本地待发布版本: `v10.7.9.329`;生产仍为 `v10.7.9.328`。
- `v10.7.9.329` 修复个股详情收益走势/收益对比在 iOS 连续滑动时的日期重绘与指针丢失;收益对比小浮层点图外立即关闭、12 秒自动关闭;主动投资价值分享大卡恢复系统标准 10% 灰色外边框。不改收益计算、普通收盘价、现金流、API、账本或鉴权。
- 当前生产已上线 `v10.7.9.328`。收益对比现在对个股和 QQQ 并行读取已登录 server-only EODHD `rawClose`,个人快照只核验日期/持仓股数;主卡将“超额金额”和“率差 +X.XX%”分开,图表触摸小浮层的个股/QQQ 行只显示收益金额并保留最终“收益率差 +X.XX%”,分享卡和复制文字继续显示完整收益率。口径明确为我的收益率减 QQQ 收益率,主动投资价值分享卡仅取消最外层白色描边。任一侧缺少必要普通收盘价时整卡不可用,不写账本/快照且生产无 mock 或估算兜底。
...
- 当前已验证 production runtime 为 `5ec2c43d4bfe166b2b475fdf294f4cd3c1eb2399`,入口 `/assets/index-DDCYvKoc.js`;GitHub Actions run `29283333713` 与 Vercel target `PwraAndzodwZojYxiZ4bB5H4KLZh` success。
...
- 本地设置页版本为 `v10.7.9.329`,生产仍为 `v10.7.9.328`;生产运行时基准为 `5ec2c43d4bfe166b2b475fdf294f4cd3c1eb2399`,入口 `/assets/index-DDCYvKoc.js`。
- `v10.7.9.329` 本地验证:定向边界测试 51/51、build、docs consistency、diff check 和 Xcode iOS 26.5 `iPhone 17 Pro` Simulator 均 pass;两张图连续滑动日期/金额同步,收益对比浮层点外立即关闭、12 秒自动关闭并可再打开,分享大卡标准灰边实测为 `1px rgba(255,255,255,0.10)`。按 `ui-fast` 未运行完整测试、audit 或旧 frontend smoke。
...
- 当前 GitHub `main`: 以本文件所在最新交接证据提交为准,接手后执行 `git log -1 --oneline`;最近已上线运行时代码提交为 `5ec2c43d4bfe166b2b475fdf294f4cd3c1eb2399`。
- 当前生产运行时基准提交: `5ec2c43d4bfe166b2b475fdf294f4cd3c1eb2399`。
- 当前本地和生产设置页版本均为 `v10.7.9.328`。
...
- 最新运行时部署: `npm run verify:deploy-status -- 5ec2c43` pass;GitHub Actions run `29283333713` success,Vercel status success,target `https://vercel.com/chenshuai1190-7580s-projects/boduan-tracker/PwraAndzodwZojYxiZ4bB5H4KLZh`;production alias 已更新,入口 `/assets/index-DDCYvKoc.js`。
...
- 线上关键验证: 当前生产未登录 quote、market-movers、earnings、pnl-benchmark 均为 `401`;生产 `StockDetailPage-qTgudR6o.js`、`SettingsTab-_KN8Ev1a.js`、`settingsChangelog-CC2Mq6JN.js`、`i18n-WWHWRkQG.js`、`App-oE1DNsBU.js` 与本地 production build 字节一致,命中 v328、收益率差和双侧普通收盘价 marker。真实 30/30 榜单与 NVDA/QQQ `rawClose` 为发布前同代码 server-only 探针证据,不是伪称的生产登录态人工检查。
...
- 当前本地设置页版本为 `v10.7.9.329`,生产仍为 `v10.7.9.328`。v329 修复收益图表连续滑动、浮层关闭与分享卡灰边;v328 统一个股/QQQ 普通收盘价口径,补齐收益率差百分比展示。
- 当前 GitHub source 基准提交: 以本文件所在最新交接证据提交为准,接手后执行 `git log -1 --oneline`;当前已验证生产运行时代码提交为 `5ec2c43d4bfe166b2b475fdf294f4cd3c1eb2399`。
- 当前生产运行时基准提交: `5ec2c43d4bfe166b2b475fdf294f4cd3c1eb2399`。
- 最近应用代码提交: `5ec2c43d4bfe166b2b475fdf294f4cd3c1eb2399` 包含 `v10.7.9.328` 普通收盘价统一与收益率表达；其前一提交 `55e02c8faa8fdac7b57b44335b6edb5d153494a2` 包含 `v10.7.9.327` 个股收益对比。
...
- 当前本地设置页版本: `v10.7.9.329`;生产仍为 `v10.7.9.328`。
- Vercel 最新部署: `v10.7.9.328` runtime commit `5ec2c43d4bfe166b2b475fdf294f4cd3c1eb2399` 已 success,target `https://vercel.com/chenshuai1190-7580s-projects/boduan-tracker/PwraAndzodwZojYxiZ4bB5H4KLZh`,production 入口 `/assets/index-DDCYvKoc.js`,v328 marker、关键 chunks 字节一致性与未登录 API 边界已验证。
...
- 本地设置页版本为 `v10.7.9.329`,生产仍为 `v10.7.9.328`。
- 当前已验证生产运行时为 `5ec2c43d4bfe166b2b475fdf294f4cd3c1eb2399`,入口 `/assets/index-DDCYvKoc.js`;接手后以最新部署证据为准。
...
当前生产基准:
- 运行时代码: `5ec2c43d4bfe166b2b475fdf294f4cd3c1eb2399`
- 设置页版本: `v10.7.9.329`
- 生产入口: `/assets/index-DDCYvKoc.js`
- Runtime Actions: `29283333713` success
- Runtime Vercel: `https://vercel.com/chenshuai1190-7580s-projects/boduan-tracker/PwraAndzodwZojYxiZ4bB5H4KLZh` success
- 上一条已验证 docs-only commit: `a54df762042000444668683bd1771efc72296885`；Actions `29199119074`、Vercel `Dp55pVfvjaKTQzw855Br2gC7Ybsd` success
...
最近完成:
- `v10.7.9.329`: 个股详情的收益走势与收益对比图在 iOS 连续滑动时保持日期、十字线和金额同步;收益对比小浮层点图外立即关闭、12 秒自动关闭并可再打开;主动投资价值分享大卡恢复系统标准灰边。不改收益计算、收盘价、现金流、API、账本或鉴权;待推送部署。
