...
| 运行时代码 | `v10.7.9.424`：首页与交易页现金短金额继续和融资负债第三列对齐，长金额以右侧为锚向左自适应完整显示，不换行、不省略且不挤出卡片；保留 v423 双向同步、v420 波段部分卖出、v417 股票实时稳定版 v10 及全部既有正式行情边界；精确发布提交以 GitHub `main` HEAD 为准 |
...
- v424 只调整首页和交易页现金长金额的自适应显示；短金额仍与融资负债第三列对齐，长金额以右侧为锚向左扩展，不修改现金保存、资产汇总或个人收益口径。交易页统一资产汇总本就包含现金，不得再次相加。v421 的现金资产与个人收益完成收盘快照口径不变，也不改变正式交易、股票收益率、QQQ、比赛或 EODHD 行情。其余继续保留波段部分卖出、收益比赛共享 EODHD 缓存与熔断、交易持仓 EODHD 收盘估值、主 `/api/quote` 的 15/30/60 分钟客户端门控、个人收益只读页面和股票实时稳定版 v10。TSM 业绩趋势在每个冷实例首次读取时仍会产生一笔 Fundamentals 与一笔完整历史 FX 请求；6 小时实例缓存仍不是跨实例全局缓存。
