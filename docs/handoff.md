...
- 线上关键验证: `npm run verify:deploy-status -- ca93291` pass;GitHub Actions run `29104581387` success;Vercel status success;生产入口 `/assets/index-Cbh2QSg0.js`;未登录 `/api/quote?symbols=VIX` 和 `/api/earnings-calendar?symbols=NVDA` 均返回 `401`;线上 `App-euolW0Tp.js`、`SettingsTab-VvuqJL1V.js` 和 `settingsChangelog-Bm14v7kO.js` 与本地构建 SHA-256 一致,生产 marker 确认 `v10.7.9.287`、`首页行情超限分批修复`、`超过 30 个 symbols` 和 `不放宽 /api/quote 服务端上限` 存在。
...
- 当前 GitHub source 基准提交: 最新运行时代码提交 `ca932917d893ce966a05a999d4ead2d415291724` 包含 `v10.7.9.287` 首页行情超限分批热修;本文件所在最新提交为该 runtime 部署证据回填。
- 当前生产运行时基准提交: `ca932917d893ce966a05a999d4ead2d415291724`。
...
- 最新补充部署记录: `v10.7.9.287` runtime code commit `ca932917d893ce966a05a999d4ead2d415291724` pushed via project SSH key;GitHub Actions `CI` run `29104581387` success;Vercel status success,target `https://vercel.com/chenshuai1190-7580s-projects/boduan-tracker/FPGATbpobwY9ScZokNWLw8QTXgZM`;production alias updated;active runtime assets and auth boundaries verified。生产入口 `/assets/index-Cbh2QSg0.js`;关键 chunks 包括 `/assets/App-euolW0Tp.js`,`/assets/HomeTab-CzFb8Igz.js`,`/assets/SettingsTab-VvuqJL1V.js`,`/assets/settingsChangelog-Bm14v7kO.js`,`/assets/i18n-DirP5Edj.js`。上一条 `v10.7.9.286` 首页财报日历智能上移为 runtime code commit `aa7fe68429491a170b637889ea4c95cd8670e3c3`;更早运行时代码部署历史见 `docs/development-log.md`。
