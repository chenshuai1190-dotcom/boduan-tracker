...
- 最新本地热修补充: `v10.7.9.287` 首页行情超限分批已完成本地实现,尚未推送或部署;首页主行情超过 30 个 symbols 时按 `30+余数` 顺序分批并合并结果,修复整批 `400` 导致今日盈亏、指数和交易持仓行情无法显示的问题。后端 30-symbol 上限、`/api/quote` 鉴权、provider、交易账本、财报日历、收益快照、数据库和 RLS 不变。
- 最新接手补充: `v10.7.9.286` 首页财报日历智能上移已上线,production runtime commit `aa7fe68429491a170b637889ea4c95cd8670e3c3`;未来 15 天内自选与持仓合计至少 5 家公司有待公布财报,且至少 1 家属于当前持仓时,同一张财报日历卡片上移到自选/持仓模块上方;其余情况保持首页底部。独立 `/api/earnings-calendar`、`/api/quote`、交易账本、收益快照、RLS 和数据库不变。
...
- 当前 GitHub `main`: 最新运行时代码提交 `aa7fe68429491a170b637889ea4c95cd8670e3c3` 包含 `v10.7.9.286` 首页财报日历智能上移;本文件所在最新提交为该 runtime 部署证据回填。
- 当前生产运行时基准提交: `aa7fe68429491a170b637889ea4c95cd8670e3c3`。
- 设置页版本: `v10.7.9.287`（本地热修待部署;生产仍为 `v10.7.9.286`）。
- 当前生产地址: `https://boduan-tracker.vercel.app`。
- 最新 Vercel 状态: `npm run verify:deploy-status -- aa7fe68` pass;GitHub Actions run `29103998696` success,Vercel status success,target `https://vercel.com/chenshuai1190-7580s-projects/boduan-tracker/6fnx58PDzhGjLieAXsokCtJmcQtZ`;production alias 已更新,入口 `/assets/index-Cue4nbNN.js`。
- 最近交接文档刷新部署: 本文件所在最新提交为 `v10.7.9.286` runtime production 验证证据回填。
- 线上关键验证: `npm run verify:deploy-status -- aa7fe68` pass;GitHub Actions run `29103998696` success;Vercel status success;生产入口 `/assets/index-Cue4nbNN.js`;未登录 `/api/quote?symbols=VIX` 和 `/api/earnings-calendar?symbols=NVDA` 均返回 `401`;生产 bundle marker 确认 `data-home-earnings-placement`、`promoted`、`default`、`order-1`、`order-3`、`/api/earnings-calendar`、`v10.7.9.286`、`首页财报日历智能上移`、`未来 15 天内` 和 `独立 /api/earnings-calendar` 存在。
...
- 当前 GitHub source 基准提交: 最新运行时代码提交 `aa7fe68429491a170b637889ea4c95cd8670e3c3` 包含 `v10.7.9.286` 首页财报日历智能上移;本文件所在最新提交为该 runtime 部署证据回填。
- 当前生产运行时基准提交: `aa7fe68429491a170b637889ea4c95cd8670e3c3`。
- 最近应用代码提交: production 最近应用代码提交 `aa7fe68429491a170b637889ea4c95cd8670e3c3` 包含 `v10.7.9.286` 首页财报日历智能上移;此前 `a0832b369a657ca95029da78c727acabbdff36ef` 包含 `v10.7.9.285` 热门股票弹窗实时行情和 `v10.7.9.284` 自选添加股票校验,`d0b63f8f8b3c622b9c84b63b9964a307d442efc3` 包含 `v10.7.9.283` 个股详情持仓时间;更早收益报表、个股详情、财报日历、BTC/指数拆分和 iOS 主屏实时链路历史见开发日志。
- 最近文档/配置记录提交: 本文件所在最新提交;流程工具链运行提交为 `c47b6e0b78115ea0e004c8cc5b498a2505527fc4`。
- 设置页版本: `v10.7.9.287`（本地热修待部署;生产仍为 `v10.7.9.286`）。
- Vercel 最新部署: `v10.7.9.286` runtime commit `aa7fe68429491a170b637889ea4c95cd8670e3c3` 已 success,target `https://vercel.com/chenshuai1190-7580s-projects/boduan-tracker/6fnx58PDzhGjLieAXsokCtJmcQtZ`,production 入口 `/assets/index-Cue4nbNN.js`,关键 chunks 包括 `/assets/HomeTab-CzFb8Igz.js`,`/assets/App-Di4LNbkX.js`,`/assets/SettingsTab-D52JO6_h.js`,`/assets/settingsChangelog-C81hDGn6.js`,`/assets/i18n-DirP5Edj.js`。
- 最近交接文档刷新部署: 本文件所在最新提交只同步 `v10.7.9.286` runtime production 验证证据。
...
- 最新补充部署记录: `v10.7.9.286` runtime code commit `aa7fe68429491a170b637889ea4c95cd8670e3c3` pushed via project SSH key;GitHub Actions `CI` run `29103998696` success;Vercel status success,target `https://vercel.com/chenshuai1190-7580s-projects/boduan-tracker/6fnx58PDzhGjLieAXsokCtJmcQtZ`;production alias updated;active runtime assets and auth boundaries verified。生产入口 `/assets/index-Cue4nbNN.js`;关键 chunks 包括 `/assets/HomeTab-CzFb8Igz.js`,`/assets/App-Di4LNbkX.js`,`/assets/SettingsTab-D52JO6_h.js`,`/assets/settingsChangelog-C81hDGn6.js`,`/assets/i18n-DirP5Edj.js`。上一条 `v10.7.9.285` 热门股票弹窗实时行情为 runtime code commit `a0832b369a657ca95029da78c727acabbdff36ef`;更早运行时代码部署历史见 `docs/development-log.md`。
...
当前 GitHub main: 最新运行时代码提交 `aa7fe68429491a170b637889ea4c95cd8670e3c3` 包含 `v10.7.9.286` 首页财报日历智能上移;`v10.7.9.287` 首页行情超限分批热修当前仅在本地待部署
当前运行时基准提交: `aa7fe68429491a170b637889ea4c95cd8670e3c3`
设置页版本: `v10.7.9.287`（本地热修待部署;生产仍为 `v10.7.9.286`）
最新 Vercel 部署: `aa7fe68429491a170b637889ea4c95cd8670e3c3` success,target `https://vercel.com/chenshuai1190-7580s-projects/boduan-tracker/6fnx58PDzhGjLieAXsokCtJmcQtZ`
最新生产入口: `/assets/index-Cue4nbNN.js`

关键线上验证:
- `v10.7.9.287` 本地热修待部署:首页主行情超过 30 个 symbols 时按 30 个一批顺序读取并合并,服务端上限和 `/api/quote` 鉴权不变
- `npm run verify:deploy-status -- aa7fe68` pass: GitHub Actions run `29103998696` success,Vercel status success,生产入口 `/assets/index-Cue4nbNN.js`,未登录 quote/earnings 均为 `401`
