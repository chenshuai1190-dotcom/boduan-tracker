...
- 当前 GitHub `main`: 本文件所在最新提交为准;`v10.7.9.170` 运行时代码提交待推送后回填。
- 当前运行时代码提交: 待 `v10.7.9.170` 推送后回填。
- 设置页版本: `v10.7.9.170`。
- 当前生产地址: `https://boduan-tracker.vercel.app`。
- 最新运行时 Vercel 部署: `v10.7.9.170` 待推送并等待 Vercel 自动部署。
- 最近交接文档刷新部署: 本文件所在提交推送后以 GitHub/Vercel 最新状态为准。
- 线上关键验证: `v10.7.9.170` 待部署后回填生产 active assets 和 marker;上一轮 `v10.7.9.169` 已验证未登录 `GET /api/quote?symbols=VIX` 返回 `401`,普通 HTTPS `GET /api/stocks-realtime` 返回 `426`。
...
- 当前产品基准提交: 待 `v10.7.9.170` 推送后回填。
- 最近应用代码提交: 待 `v10.7.9.170` 推送后回填。
- 最近文档/配置记录提交: 本文件所在最新提交。
- 设置页版本: `v10.7.9.170`
- Vercel 最新运行时部署: `v10.7.9.170` 待推送并等待 Vercel 自动部署。
- 最近交接文档刷新部署: 本文件所在提交推送后以 GitHub/Vercel 最新状态为准。
- Vercel 部署记录: `v10.7.9.170` 待推送后由 Vercel 自动部署;生产 active assets 和 marker 待回填。
...
仓库: `chenshuai1190-dotcom/boduan-tracker`
生产地址: https://boduan-tracker.vercel.app
当前 GitHub main: 以本文件所在最新提交为准;`v10.7.9.170` 运行时代码提交待推送后回填
当前产品基准提交: 待 `v10.7.9.170` 推送后回填
最近应用代码提交: 待 `v10.7.9.170` 推送后回填
设置页版本: `v10.7.9.170`
Vercel 最新运行时部署: `v10.7.9.170` 待推送并等待 Vercel 自动部署
最近交接文档刷新部署: 本文件所在提交推送后以 GitHub/Vercel 最新状态为准
部署记录: `v10.7.9.170` 待推送后由 Vercel 自动部署;active runtime assets 和生产 marker 待回填;上一轮 `v10.7.9.169` 已验证 unauthenticated `/api/quote?symbols=VIX` returns `401`,plain HTTPS `/api/stocks-realtime` returns `426`
...
当前已完成:
- `v10.7.9.170` 交易录入弹窗细节修正已完成本地验证,待推送和 Vercel 生产验证:交易录入弹窗输入框/标签字体适量放大,系统自动识别提示增强,移除股票代码/价格股数/日期前面的数字标记,底部不再显示“操作”标题;二次确认弹窗改为居中深色卡片,不再显示白色老版底部抽屉;不改主交易账本、波段记录边界、持仓/盈亏计算、行情 relay、RLS 或 `/api/quote` 鉴权。
