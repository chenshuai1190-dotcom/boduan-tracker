...
- 最新接手补充: `v10.7.9.232` 新增个股收益详情页收益线滑动查看。收益走势支持手指按住或滑动吸附到最近真实快照点,显示对应日期、收益金额、竖向辅助线和节点圆点;点位仍只来自已有单股票收盘快照,不插值、不补假数据。同时交易/首页持仓收益展示口径保留同一持仓周期内部分卖出的已实现收益,完全清仓后新买入周期重置。交易录入/编辑、收益快照生成、行情 relay、RLS 和 `/api/quote` 鉴权不变。
- 当前 GitHub `main`: 本文件所在最新提交为准;`v10.7.9.232` 应用代码已通过项目 SSH key 推送并完成生产 marker 验证。
- 当前生产运行时代码提交: `630a738d1ca716d04f3d8930e37b5294b7347dbe`;`v10.7.9.232` production marker verified。
- 设置页版本: `v10.7.9.232`。
- 当前生产地址: `https://boduan-tracker.vercel.app`。
- 最新 Vercel 状态: `630a738` production marker verified at `https://boduan-tracker.vercel.app`;production alias serving `v10.7.9.232` runtime assets。
- 最近交接文档刷新部署: 本文件所在最新提交为准;`v10.7.9.232` 运行时代码已部署成功,本轮文档回写为生产验证补充。
- 线上关键验证: 当前线上为 `v10.7.9.232`,生产入口 `/assets/index-BV_WwH7C.js`;runtime chunks include `App--TKAmDct.js`,`HomeTab-BEBfMS18.js`,`TradesTab-DAX6AzUD.js`,`StockDetailPage-C3SNAms6.js`,`SettingsTab-nqwO9CwL.js`,`settingsChangelog-CQ7xohdU.js`,`i18n-C2nshi8Z.js`;production assets contain `v10.7.9.232`,`个股收益线滑动查看`,`touchAction` and `holdingPnl`;未登录 `GET /api/quote?symbols=VIX` 返回 `401`;HTTPS 普通非 WebSocket `GET /api/stocks-realtime`、`GET /api/indices-realtime`、`GET /api/btc-realtime` 返回 `426`。
...
- 当前生产运行时基准提交: `630a738d1ca716d04f3d8930e37b5294b7347dbe`;`v10.7.9.232` production marker verified。
- 最近应用代码提交: `630a738d1ca716d04f3d8930e37b5294b7347dbe`,本轮新增个股收益线滑动查看和持仓收益口径调整,已推送部署。上一轮个股详情视觉优化提交为 `6100a9a7c4303e65bb4fc30d598d2ee93c927c30`;上一轮个股详情只读页提交为 `3f448c805d2270c5931592f681ea907b62e5fb27`;上一轮自动收盘快照提交为 `ebb8090912ba373a48efb756bb85886e1d9f07c6`,环境变量配置记录/重部署提交为 `a066f6beda9e970f1efcbb14a1175e798015d864`,上一轮报表真实快照读取提交为 `1ca0a9684788b8b915ba4b035d1ac076987e7c1f`,上一轮报表快照基础提交为 `f8329b0f5bbad6837c7925d1ba8b19a1335dbe8a`,上一轮 BTC 卡位提交为 `95e8cc5ee25486e9ebee5e0a1d3e44b18d3d2133`。
- 最近文档/配置记录提交: 本文件所在最新提交。
- 设置页版本: `v10.7.9.232`。
- Vercel 最新运行时部署: 当前线上 success,`v10.7.9.232` production marker verified,production alias `https://boduan-tracker.vercel.app` serving runtime assets from production entry `/assets/index-BV_WwH7C.js`。
- 最近交接文档刷新部署: 本文件所在最新提交为准;`v10.7.9.232` 运行时代码已部署成功,本轮文档回写为生产验证补充。
