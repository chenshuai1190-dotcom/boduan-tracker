...
- 当前生产已上线 `v10.7.9.342`：资产页点击精确账户的名称/类型后，读取该 `accountId` 在 `balance_snapshots` 中的真实原币月末余额并显示近 12 个自然月走势；缺月不补零、不插值、不沿用旧余额，也不合并同名或同类型账户。右侧金额仍进入原有账户修改/删除弹窗。定向测试、完整 410/410 测试、build、toolchain、audit high、diff check 和 iPhone 17 Pro Simulator 只读截图均已通过；runtime `f6b53a834df5550bcdeef18e1181ddd8b382a9da`、Actions `29438656651`、Vercel `FvseyF5Gu2Dh3gJm8C7M4twbxRuE`、入口 `/assets/index-DN3icOHQ.js`、`index.html` 与 30 个生产 assets 字节一致、功能 marker、开发 fixture 缺席和未登录 401 均已验证。
...
- 当前本地与生产设置页版本均为 `v10.7.9.342`；当前已验证生产运行时基准为 `f6b53a834df5550bcdeef18e1181ddd8b382a9da`,入口 `/assets/index-DN3icOHQ.js`。
...
- 当前本地与生产设置页版本均为 `v10.7.9.342`。v342 新增精确单账户真实原币月度资产走势，不补零、不插值或沿用旧余额；v341 的真实财报结果与盘前/正式收盘反应继续保留。
...
- 本地与生产设置页版本均为 `v10.7.9.342`；以最新提交和部署证据为准。
...
当前生产基准:
- 运行时代码: `f6b53a834df5550bcdeef18e1181ddd8b382a9da`
- 设置页版本: `v10.7.9.342`
- 生产入口: `/assets/index-DN3icOHQ.js`
- Runtime Actions: `29438656651` success
- Runtime Vercel: `https://vercel.com/chenshuai1190-7580s-projects/boduan-tracker/FvseyF5Gu2Dh3gJm8C7M4twbxRuE` success
- 最新 docs evidence: 本文件所在提交（已回填 v342 runtime、Actions、Vercel、生产入口、`index.html` 与 30 个 assets 一致性、开发 fixture 缺席和未授权边界；接手后以 `git log -1` 核验）
