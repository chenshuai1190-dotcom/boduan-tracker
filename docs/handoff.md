...
- 最新接手补充: `v10.7.9.235` 股票代码输入严格校验待推送部署。本轮修复已注册用户历史交易中 `N VDA` 这类内部空格 ticker 打爆整批 Quote API 的问题:旧数据读取会把短 ticker 容错修复为 `NVDA`,新建/编辑交易和自选写入严格拒绝含空格或特殊字符的股票代码。登录权限、邀请码规则、交易收益计算、收益快照生成、行情 relay、RLS 和 `/api/quote` 鉴权不变。
- 当前 GitHub `main`: 本文件所在最新提交为准;`v10.7.9.235` 应用代码待通过项目 SSH key 推送并完成生产 marker 验证。
- 当前生产运行时代码提交: `244571fc80586b6c36c0156e14713eae4d07b27d`;线上仍为 `v10.7.9.234` production marker verified,`v10.7.9.235` 待部署验证。
- 设置页版本: `v10.7.9.235`。
- 当前生产地址: `https://boduan-tracker.vercel.app`。
- 最新 Vercel 状态: `244571f` production marker verified at `https://boduan-tracker.vercel.app`;production alias currently serving `v10.7.9.234` runtime assets,`v10.7.9.235` 待推送部署。
- 最近交接文档刷新部署: 本文件所在最新提交为准;`v10.7.9.235` 运行时代码待部署,部署后需回写生产验证补充。
- 线上关键验证: 当前线上为 `v10.7.9.234`,生产入口 `/assets/index-Hm4YhSOo.js`;runtime chunks include `App-DKMStrOI.js`,`HomeTab-BEBfMS18.js`,`TradesTab-DAX6AzUD.js`,`StockDetailPage-NlxUfSyh.js`,`SettingsTab-C-3ET74D.js`,`settingsChangelog-nnQ4DkMJ.js`,`i18n-C2nshi8Z.js`;production assets contain `v10.7.9.234`,`个股详情文字层级回调`,`StockDetailPage`,`text-white/40` and `text-white/86`;未登录 `GET /api/quote?symbols=VIX` 返回 `401`;HTTPS 普通非 WebSocket `GET /api/stocks-realtime`、`GET /api/indices-realtime`、`GET /api/btc-realtime` 返回 `426`。
- 当前产品焦点: 英文模式已分阶段覆盖设置页、底部导航、首页、交易页、资产页和目标页。`v10.7.9.176` 起股票涨跌幅按现价和昨收重算;`v10.7.9.177` 到 `v10.7.9.207` 主要处理股票 realtime、iOS 主屏 snapshot、BTC/指数拆分和卡位稳定;`v10.7.9.208` 到 `v10.7.9.211` 主要处理三大指数去 Yahoo 图源、固定卡位和分时曲线锁定;`v10.7.9.212` 到 `v10.7.9.214` 建立并精简收益报表独立页面前端;`v10.7.9.215` 建立收益报表快照 SQL/RLS/dirty 标记基础;`v10.7.9.216` 到 `v10.7.9.228` 接入真实快照读取、手动收盘快照回填、收益日历和周期统计;`v10.7.9.229` 起新增全账户自动收盘快照;`v10.7.9.230` 起交易页支持进入只读个股收益详情页;`v10.7.9.231` 起详情页默认全部周期并收紧设计稿视觉层级;`v10.7.9.232` 起详情页收益线可滑动查看真实快照点,且持仓收益口径保留同一持仓周期内部分卖出的已实现收益;`v10.7.9.233` 起详情页文字层级进一步压灰并延长收益线点位提示;`v10.7.9.234` 起详情页字段标题和普通数值层级回调到交易页同款;`v10.7.9.235` 起股票代码写入严格拒绝空格/特殊字符,历史短 ticker 空格脏数据读取时自动修复。用户自写内容、中文显示、主交易账本、摊薄工具、行情鉴权和 `/api/quote` 鉴权保持不变。
...
- 当前生产运行时基准提交: `244571fc80586b6c36c0156e14713eae4d07b27d`;线上仍为 `v10.7.9.234` production marker verified,`v10.7.9.235` 待部署验证。
- 最近应用代码提交: `v10.7.9.235` 股票代码输入严格校验待推送部署;上一轮个股详情字段标题和普通数值层级回调到交易页同款提交为 `244571fc80586b6c36c0156e14713eae4d07b27d`,已推送部署。上一轮个股详情文字层级微调提交为 `b8fc9a62f1802460ff71ff28865ec6d6ee55f1d6`;上一轮个股收益线滑动查看提交为 `630a738d1ca716d04f3d8930e37b5294b7347dbe`;上一轮个股详情视觉优化提交为 `6100a9a7c4303e65bb4fc30d598d2ee93c927c30`;上一轮个股详情只读页提交为 `3f448c805d2270c5931592f681ea907b62e5fb27`;上一轮自动收盘快照提交为 `ebb8090912ba373a48efb756bb85886e1d9f07c6`,环境变量配置记录/重部署提交为 `a066f6beda9e970f1efcbb14a1175e798015d864`,上一轮报表真实快照读取提交为 `1ca0a9684788b8b915ba4b035d1ac076987e7c1f`,上一轮报表快照基础提交为 `f8329b0f5bbad6837c7925d1ba8b19a1335dbe8a`,上一轮 BTC 卡位提交为 `95e8cc5ee25486e9ebee5e0a1d3e44b18d3d2133`。
- 最近文档/配置记录提交: 本文件所在最新提交。
- 设置页版本: `v10.7.9.235`。
- Vercel 最新运行时部署: 当前线上 success,`v10.7.9.234` production marker verified,production alias `https://boduan-tracker.vercel.app` serving runtime assets from production entry `/assets/index-Hm4YhSOo.js`;`v10.7.9.235` 待推送部署。
- 最近交接文档刷新部署: 本文件所在最新提交为准;`v10.7.9.235` 运行时代码待部署,部署后需回写生产验证补充。
