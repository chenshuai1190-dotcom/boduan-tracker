...
- 当前 GitHub `main`: 本文件所在最新提交为准;运行时代码提交 `d6ef1695bb0fb9c3cb2206a50f61756e85789eb2` 已通过项目 SSH key 推送并上线,当前运行时为 `v10.7.9.204`。
- 当前生产运行时代码提交: `d6ef1695bb0fb9c3cb2206a50f61756e85789eb2`;最近已验证的 runtime marker 为 `v10.7.9.204`。
- 设置页版本: `v10.7.9.204`。
- 当前生产地址: `https://boduan-tracker.vercel.app`。
- 最新 Vercel 状态: `d6ef169` production deployment success,`v10.7.9.204` production marker verified,`v10.7.9.202` marker absent。
- 最近交接文档刷新部署: 本文件所在最新提交为准;`v10.7.9.204` 运行时代码已部署成功。
- 线上关键验证: 生产入口 `/assets/index-DnB_Z168.js`;entry imports `App-BSWC9NlH.js`;`App-BSWC9NlH.js` contains `/api/btc-realtime`,`/api/stocks-realtime`,`/api/indices-realtime`,`America/New_York`,the faster iOS snapshot burst `0/0.8/1.6/3/5` and `stockFreshnessStartedAt`,does not contain BTC snapshot fetch/minified BTC snapshot call or `setBtcRealtimeStatus('warming')`;`HomeTab-BzDNIrHi.js` and `TradesTab-DvTLX5c4.js` still render masked current prices as `--`;`i18n-QTvefRC5.js` remains active;`SettingsTab-xej1q5lA.js` contains `v10.7.9.204`;`settingsChangelog-BHgcb57S.js` contains `v10.7.9.204`,`iOS 主屏股票秒级刷新` and retained `BTC 主屏连接恢复`;production assets no longer contain `v10.7.9.202` or `首屏当日盈亏兜底`;未登录 `GET /api/quote?symbols=VIX` 返回 `401`;HTTPS 普通 `GET /api/stocks-realtime` 返回 `426`;未登录 `GET /api/stocks-realtime?snapshot=1&symbols=NVDA`、`GET /api/btc-realtime?snapshot=1`、`GET /api/indices-realtime?snapshot=1` 均返回 `401`。
- 当前产品焦点: 英文模式已分阶段覆盖设置页、底部导航、首页、交易页、资产页和目标页。`v10.7.9.176` 起股票涨跌幅按现价和昨收重算;`v10.7.9.177` 到 `v10.7.9.182` 主要处理启动、iOS PWA 回前台和三套 realtime 恢复重连;`v10.7.9.183` 起保护实时 tick 缺少有效昨收时不覆盖完整行情行;`v10.7.9.185` 起按券商口径优先使用日线基准;`v10.7.9.189` 起正式拆分“展示当前价”和“今日盈亏计算价”:盘前/盘中今日盈亏随实时价更新,盘后/夜盘今日盈亏锁定到常规盘收盘价,盘后现价仍可显示并影响市值/持仓盈亏;`v10.7.9.190` 起首页/交易页“收盘锁定”状态标签改为中性灰色;`v10.7.9.191` 起英文首页/交易页头部持仓数量只显示 holdings 并恢复中文同款三列间距;`v10.7.9.192` 起股票 realtime relay 保留 `/ws/us` 成交价为主源,新增 `/ws/us-quote` bid/ask 中间价作为盘前成交稀疏时的兜底;`v10.7.9.195` 起 warm upstream 新客户端接入会补发订阅;`v10.7.9.196` 起客户端不再因首轮 symbol 覆盖不足反复断开股票 WebSocket;`v10.7.9.197` 起 iOS 主屏 standalone Web App 自动切换为认证 HTTP snapshot 轮询,BTC、指数和股票不再使用浏览器 WebSocket,普通 Safari/桌面仍走原有 WebSocket;`v10.7.9.198` 起 iOS 主屏首次进入/回前台会先跑 realtime snapshot burst,并避免启动恢复链路先用 `/api/quote` REST 快照覆盖实时价格;`v10.7.9.199` 起 iOS 主屏预热阶段未拿到本轮新 tick 的首页/交易持仓现价只显示 `----`,拿到新 tick 后自动恢复;`v10.7.9.200` 起 iOS 主屏普通 focus/触摸滑动触发的 snapshot 不再刷新 freshness 遮罩,只有真正恢复可见、联网、云数据加载和 BFCache 恢复才重开遮罩周期;`v10.7.9.201` 起首页/交易持仓现价遮罩占位从 `----` 改为更轻的 `--`;`v10.7.9.202` 首屏当日盈亏兜底实验已回退;`v10.7.9.203` 起 BTC 在 iOS 主屏恢复独立 WebSocket,不再被股票/指数 snapshot burst 标成同步中;`v10.7.9.204` 起 iOS 主屏股票/指数 snapshot 在盘前、盘中和盘后使用 1.25 秒活跃轮询,启动/回前台 burst 前移到 0/0.8/1.6/3/5 秒,BTC 继续保持独立 WebSocket。用户自写内容、中文显示、持仓/交易计算、交易账本、摊薄工具、行情鉴权和数据库结构不变。
...
- 当前生产运行时基准提交: `d6ef1695bb0fb9c3cb2206a50f61756e85789eb2`;最近已验证的 runtime marker 为 `v10.7.9.204`。
- 最近应用代码提交: `d6ef1695bb0fb9c3cb2206a50f61756e85789eb2`,提升 iOS 主屏股票/指数 snapshot 活跃时段刷新频率到 1.25 秒并前移启动/回前台 burst,同时保持 BTC 独立 WebSocket 不变。上一轮 BTC 恢复提交为 `1d414466c4443bcc5e552019d1cff22440f0e847`,上一轮回退提交为 `6797bce8c49a2844ed21ff0252b4fc519778771c`,上一轮实验代码提交为 `29f596bf7ffacacbc6b580fb5be04c34f019a5b5`,上一轮稳定生产 `v10.7.9.201` 应用代码提交为 `1b70bf5717d7701e432244a5f622f17eee87cfcb`。
- 最近文档/配置记录提交: 本文件所在最新提交。
- 设置页版本: `v10.7.9.204`。
- Vercel 最新运行时部署: success,`v10.7.9.204` production marker verified,`v10.7.9.202` marker absent。
- 最近交接文档刷新部署: 本文件所在最新提交为准;`v10.7.9.204` 运行时代码已部署成功。
