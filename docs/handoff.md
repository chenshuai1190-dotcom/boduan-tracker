...
- 最新接手补充: `v10.7.9.231` 优化个股收益详情页视觉层级。交易页持仓列表点击股票名称/代码进入独立详情页,默认展示全部周期;页面删除右上角编辑按钮和股票装饰标识,股票标题/副标题居中,收益走势按真实周期起止日期绘制并显示左侧真实金额刻度,统计日期/小标题/交易记录灰度和字号按设计稿下调。页面仍只读,只读取主交易账本 `stock_trades` 和单股票收盘快照 `pnl_report_symbol_snapshots`;交易录入/编辑、交易页实时持仓/盈亏、自动快照任务、行情 relay、RLS 和 `/api/quote` 鉴权不变。
- 当前 GitHub `main`: 本文件所在最新提交为准;应用代码提交 `6100a9a7c4303e65bb4fc30d598d2ee93c927c30` 已通过项目 SSH key 推送并上线。
- 当前生产运行时代码提交: `6100a9a7c4303e65bb4fc30d598d2ee93c927c30`;最近已验证 runtime marker 为 `v10.7.9.231`。
- 设置页版本: `v10.7.9.231`。
- 当前生产地址: `https://boduan-tracker.vercel.app`。
- 最新 Vercel 状态: `6100a9a` production deployment success,target `https://boduan-tracker-d7tpw9kih-chenshuai1190-7580s-projects.vercel.app`;production alias `https://boduan-tracker.vercel.app` updated。
- 最近交接文档刷新部署: 本文件所在最新提交为准;`v10.7.9.231` 运行时代码已部署成功。
- 线上关键验证: 生产入口 `/assets/index-D-yQok0K.js`;runtime chunks include `App-BK-mGSKy.js`,`TradesTab-B3aPjGuD.js`,`StockDetailPage-BhrIEDqR.js`,`i18n-C2nshi8Z.js`,`SettingsTab-CoMrdoXk.js`,`settingsChangelog-B7cimjRW.js`;production assets contain `v10.7.9.231`,`个股详情视觉优化`,`StockDetailPage`,`axisStartDate`,`stockDetail.openAria` and `个股收益详情`;未登录 `GET /api/quote?symbols=VIX` 返回 `401`;HTTPS 普通非 WebSocket `GET /api/stocks-realtime`、`GET /api/indices-realtime`、`GET /api/btc-realtime` 返回 `426`;纯 HTTP 首先被 Vercel edge `308` 跳转到 HTTPS。
- 当前产品焦点: 英文模式已分阶段覆盖设置页、底部导航、首页、交易页、资产页和目标页。`v10.7.9.176` 起股票涨跌幅按现价和昨收重算;`v10.7.9.177` 到 `v10.7.9.207` 主要处理股票 realtime、iOS 主屏 snapshot、BTC/指数拆分和卡位稳定;`v10.7.9.208` 到 `v10.7.9.211` 主要处理三大指数去 Yahoo 图源、固定卡位和分时曲线锁定;`v10.7.9.212` 到 `v10.7.9.214` 建立并精简收益报表独立页面前端;`v10.7.9.215` 建立收益报表快照 SQL/RLS/dirty 标记基础;`v10.7.9.216` 到 `v10.7.9.228` 接入真实快照读取、手动收盘快照回填、收益日历和周期统计;`v10.7.9.229` 起新增全账户自动收盘快照;`v10.7.9.230` 起交易页支持进入只读个股收益详情页;`v10.7.9.231` 起详情页默认全部周期并收紧设计稿视觉层级。用户自写内容、中文显示、交易页实时持仓/盈亏、主交易账本、摊薄工具、行情鉴权和 `/api/quote` 鉴权保持不变。
...
- 当前生产运行时基准提交: `6100a9a7c4303e65bb4fc30d598d2ee93c927c30`;最近已验证 runtime marker 为 `v10.7.9.231`。
- 最近应用代码提交: `6100a9a7c4303e65bb4fc30d598d2ee93c927c30`,个股详情视觉优化:默认全部周期,删除右上角编辑按钮和股票装饰标识,标题居中,收益走势图显示真实周期日期和左侧真实金额刻度,并下调统计/交易记录灰度字号。上一轮个股详情只读页提交为 `3f448c805d2270c5931592f681ea907b62e5fb27`;上一轮自动收盘快照提交为 `ebb8090912ba373a48efb756bb85886e1d9f07c6`,环境变量配置记录/重部署提交为 `a066f6beda9e970f1efcbb14a1175e798015d864`,上一轮报表真实快照读取提交为 `1ca0a9684788b8b915ba4b035d1ac076987e7c1f`,上一轮报表快照基础提交为 `f8329b0f5bbad6837c7925d1ba8b19a1335dbe8a`,上一轮 BTC 卡位提交为 `95e8cc5ee25486e9ebee5e0a1d3e44b18d3d2133`。
- 最近文档/配置记录提交: 本文件所在最新提交。
- 设置页版本: `v10.7.9.231`。
- Vercel 最新运行时部署: success,`v10.7.9.231` production marker verified,target `https://boduan-tracker-d7tpw9kih-chenshuai1190-7580s-projects.vercel.app`,production alias `https://boduan-tracker.vercel.app` updated。
- 最近交接文档刷新部署: 本文件所在最新提交为准;`v10.7.9.231` 运行时代码已部署成功。
