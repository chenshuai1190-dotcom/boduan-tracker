...
- 当前本地版本: `v10.7.9.319`;当前年度目标右侧摘要已增加目标/实现/落后或超额百分比,摘要列加宽到 `160px`,不改整卡宽度或年度目标计算;生产仍为 `v10.7.9.318`,待本轮验证后部署。
...
- 当前本地设置页版本为 `v10.7.9.319`,生产仍为 `v10.7.9.318`。生产运行时基准提交为 `3e384856646901c0f6884ec87e4b95d60f24c0fe`,入口 `/assets/index-C5csRlAa.js`。
...
最新已上线:
- `v10.7.9.319` 已接入:当前年度目标摘要显示目标 100%、实现百分比与落后/超额百分比,摘要列加宽到 160px,不改整卡宽度或原计算;完整测试 255/255、build、5/5 smoke和 high audit 已 pass,部署证据待本轮回填。
