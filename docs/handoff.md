...
- 最新接手补充: `v10.7.9.233` 个股收益详情页文字层级微调待部署。个股详情页日期、小标题、统计标签、交易统计和普通白字统一压低灰度层级,并把收益走势点位提示停留延长到至少 10 秒;本次只改个股详情只读展示和图表提示停留时间。交易录入/编辑、收益快照生成、行情 relay、RLS 和 `/api/quote` 鉴权不变。
- 当前 GitHub `main`: 本文件所在最新提交为准;`v10.7.9.233` 应用代码准备通过项目 SSH key 推送并等待生产 marker 验证。
- 当前生产运行时代码提交: `630a738d1ca716d04f3d8930e37b5294b7347dbe`;线上当前仍为 `v10.7.9.232`,等待本轮部署。
- 设置页版本: `v10.7.9.233`。
- 当前生产地址: `https://boduan-tracker.vercel.app`。
- 最新 Vercel 状态: 线上当前仍为 `630a738` / `v10.7.9.232`;本轮 `v10.7.9.233` 等待推送后验证。
- 最近交接文档刷新部署: 本文件所在最新提交为准;`v10.7.9.233` 本轮部署完成后需回写生产入口和 marker。
- 线上关键验证: 等待本轮部署后回写。上一轮线上为 `v10.7.9.232`,生产入口 `/assets/index-BV_WwH7C.js`;runtime chunks include `App--TKAmDct.js`,`HomeTab-BEBfMS18.js`,`TradesTab-DAX6AzUD.js`,`StockDetailPage-C3SNAms6.js`,`SettingsTab-nqwO9CwL.js`,`settingsChangelog-CQ7xohdU.js`,`i18n-C2nshi8Z.js`;未登录 `GET /api/quote?symbols=VIX` 返回 `401`;HTTPS 普通非 WebSocket `GET /api/stocks-realtime`、`GET /api/indices-realtime`、`GET /api/btc-realtime` 返回 `426`。
...
- 当前生产运行时基准提交: `630a738d1ca716d04f3d8930e37b5294b7347dbe`;线上当前仍为 `v10.7.9.232`,等待本轮 `v10.7.9.233` 部署验证。
- 最近应用代码提交: 等待本轮 `v10.7.9.233` 推送。上一轮个股收益线滑动查看提交为 `630a738d1ca716d04f3d8930e37b5294b7347dbe`;上一轮个股详情视觉优化提交为 `6100a9a7c4303e65bb4fc30d598d2ee93c927c30`;上一轮个股详情只读页提交为 `3f448c805d2270c5931592f681ea907b62e5fb27`;上一轮自动收盘快照提交为 `ebb8090912ba373a48efb756bb85886e1d9f07c6`,环境变量配置记录/重部署提交为 `a066f6beda9e970f1efcbb14a1175e798015d864`,上一轮报表真实快照读取提交为 `1ca0a9684788b8b915ba4b035d1ac076987e7c1f`,上一轮报表快照基础提交为 `f8329b0f5bbad6837c7925d1ba8b19a1335dbe8a`,上一轮 BTC 卡位提交为 `95e8cc5ee25486e9ebee5e0a1d3e44b18d3d2133`。
- 最近文档/配置记录提交: 本文件所在最新提交。
- 设置页版本: `v10.7.9.233`。
- Vercel 最新运行时部署: 线上当前 success,`v10.7.9.232` production marker verified;本轮 `v10.7.9.233` 等待推送后重新验证。
- 最近交接文档刷新部署: 本文件所在最新提交为准;`v10.7.9.233` 本轮部署完成后需回写生产验证补充。
