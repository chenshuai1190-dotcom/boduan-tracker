...
- 最新接手补充: `v10.7.9.255` 财报日历券商式同比对比口径已部署上线。已公布财报详情改为“公布值同比 / 预测值同比”并列表格;列表视图已公布 EPS/营收百分比也同步改为同比口径,不再显示实际值相对预测值的 surprise;EODHD trends 同一 fiscal date 同时返回 `+1q` 和 `0q` 时优先使用 `0q`,避免误拿下一季预测。NVDA 真实 key smoke 已确认 `revenueEstimate=79115709670`,`revenueEstimateYoyPercent=79.56`,`revenueActualUsd=81615000000`,`revenueActualYoyPercent=85.2276`。交易账本、收益快照、股票/指数/BTC realtime relay、RLS、独立 `/api/earnings-calendar` 鉴权和 `/api/quote` 鉴权不变。
- 当前 GitHub `main`: 本文件所在最新提交为准;运行时代码提交 `04248d08c22c61b04a83cd09def2b9c97ce74277` 已由 GitHub `main` 推送触发 Vercel production 部署。
- 当前生产运行时代码提交: `04248d08c22c61b04a83cd09def2b9c97ce74277`。
- 设置页版本: `v10.7.9.255`。
- 当前生产地址: `https://boduan-tracker.vercel.app`。
- 最新 Vercel 状态: `v10.7.9.255` production `success`,target `https://vercel.com/chenshuai1190-7580s-projects/boduan-tracker/GwePcHBLeezeTXRK4KHQuw3cwQwc`;production alias 已更新,入口 `/assets/index-BvGLlodr.js`。
- 最近交接文档刷新部署: 本文件所在最新提交为准;本次 docs-only follow-up 只回填 `v10.7.9.255` 部署证据,运行时代码仍是 `04248d08c22c61b04a83cd09def2b9c97ce74277`。
- 线上关键验证: `HomeTab-CQoyCHyU.js` 包含 `revenueActualYoyPercent`,`epsActualYoyPercent`,`revenueEstimateYoyPercent`,`epsEstimateYoyPercent`,`公布值`,`预测值`;`SettingsTab-ByCQZjZF.js` 包含 `v10.7.9.255`;`settingsChangelog-DQ86VTuA.js` 包含 `v10.7.9.255` 和 `财报日历券商式对比口径`;未登录 `/api/quote?symbols=VIX` 和 `/api/earnings-calendar?symbols=NVDA` 均返回 `401`。
...
- 当前生产运行时基准提交: `04248d08c22c61b04a83cd09def2b9c97ce74277`。
- 最近应用代码提交: `04248d08c22c61b04a83cd09def2b9c97ce74277` 包含 `v10.7.9.255` 财报日历券商式同比对比口径。此前财报日历营收字段修复提交为 `1a5ae2605e51838514f3b508eb9ec6c769a7268a`;首页财报日历视觉压缩提交为 `b7422bd96b886952cc6233d218dd2c89eb89cf83`;首页财报日历独立重构提交为 `8cc8194edabcacd15a5cd49b142dff946f765298`;收益报表、个股详情、BTC/指数拆分和 iOS 主屏实时链路历史见开发日志。
- 最近文档/配置记录提交: 本文件所在最新提交。
- 设置页版本: `v10.7.9.255`。
- Vercel 最新运行时部署: `v10.7.9.255` success;production 入口 `/assets/index-BvGLlodr.js`,关键 chunks `/assets/App-DpU85JYz.js`,`/assets/HomeTab-CQoyCHyU.js`,`/assets/SettingsTab-ByCQZjZF.js`,`/assets/settingsChangelog-DQ86VTuA.js`。
...
当前运行时代码提交: `04248d08c22c61b04a83cd09def2b9c97ce74277`
设置页版本: `v10.7.9.255`
最新运行时 Vercel 部署: success,`v10.7.9.255` production marker verified
最新生产入口: `/assets/index-BvGLlodr.js`

关键线上验证:
- `HomeTab-CQoyCHyU.js` 包含 `revenueActualYoyPercent`,`epsActualYoyPercent`,`revenueEstimateYoyPercent`,`epsEstimateYoyPercent`,`公布值`,`预测值`
- `SettingsTab-ByCQZjZF.js` 包含 `v10.7.9.255`
- `settingsChangelog-DQ86VTuA.js` 包含 `v10.7.9.255` 和 `财报日历券商式对比口径`
