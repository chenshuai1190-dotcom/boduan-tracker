...
- 当前 GitHub `main`: 本文件所在最新提交为准;`v10.7.9.232` 应用代码待通过项目 SSH key 推送并等待 Vercel 上线。
- 当前生产运行时代码提交: `6100a9a7c4303e65bb4fc30d598d2ee93c927c30`;最近已验证 runtime marker 为 `v10.7.9.231`。`v10.7.9.232` 本地验证通过后待部署。
- 设置页版本: `v10.7.9.232`。
- 当前生产地址: `https://boduan-tracker.vercel.app`。
- 最新 Vercel 状态: `6100a9a` production deployment success,target `https://boduan-tracker-d7tpw9kih-chenshuai1190-7580s-projects.vercel.app`;production alias `https://boduan-tracker.vercel.app` updated。`v10.7.9.232` 待部署。
- 最近交接文档刷新部署: 本文件所在最新提交为准;`v10.7.9.231` 运行时代码已部署成功,`v10.7.9.232` 待部署回写。
- 线上关键验证: 当前线上仍为 `v10.7.9.231`,生产入口 `/assets/index-D-yQok0K.js`;runtime chunks include `App-BK-mGSKy.js`,`TradesTab-B3aPjGuD.js`,`StockDetailPage-BhrIEDqR.js`,`i18n-C2nshi8Z.js`,`SettingsTab-CoMrdoXk.js`,`settingsChangelog-B7cimjRW.js`;production assets contain `v10.7.9.231`,`个股详情视觉优化`,`StockDetailPage`,`axisStartDate`,`stockDetail.openAria` and `个股收益详情`;未登录 `GET /api/quote?symbols=VIX` 返回 `401`;HTTPS 普通非 WebSocket `GET /api/stocks-realtime`、`GET /api/indices-realtime`、`GET /api/btc-realtime` 返回 `426`;纯 HTTP 首先被 Vercel edge `308` 跳转到 HTTPS。
...
- 当前生产运行时基准提交: `6100a9a7c4303e65bb4fc30d598d2ee93c927c30`;最近已验证 runtime marker 为 `v10.7.9.231`。`v10.7.9.232` 待部署。
- 最近应用代码提交: 本文件所在最新提交,本轮新增个股收益线滑动查看和持仓收益口径调整,待推送部署。上一轮个股详情视觉优化提交为 `6100a9a7c4303e65bb4fc30d598d2ee93c927c30`;上一轮个股详情只读页提交为 `3f448c805d2270c5931592f681ea907b62e5fb27`;上一轮自动收盘快照提交为 `ebb8090912ba373a48efb756bb85886e1d9f07c6`,环境变量配置记录/重部署提交为 `a066f6beda9e970f1efcbb14a1175e798015d864`,上一轮报表真实快照读取提交为 `1ca0a9684788b8b915ba4b035d1ac076987e7c1f`,上一轮报表快照基础提交为 `f8329b0f5bbad6837c7925d1ba8b19a1335dbe8a`,上一轮 BTC 卡位提交为 `95e8cc5ee25486e9ebee5e0a1d3e44b18d3d2133`。
- 最近文档/配置记录提交: 本文件所在最新提交。
- 设置页版本: `v10.7.9.232`。
- Vercel 最新运行时部署: 当前线上 success,`v10.7.9.231` production marker verified,target `https://boduan-tracker-d7tpw9kih-chenshuai1190-7580s-projects.vercel.app`,production alias `https://boduan-tracker.vercel.app` updated。`v10.7.9.232` 待部署。
- 最近交接文档刷新部署: 本文件所在最新提交为准;`v10.7.9.231` 运行时代码已部署成功,`v10.7.9.232` 待部署回写。
