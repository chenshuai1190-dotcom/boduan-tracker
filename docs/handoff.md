...
- 最新待发布补充: `v10.7.9.289` 首页持仓盈亏和自选亮度修复已获用户授权立即部署;持仓盈亏金额和收益率取消粗体并分别跟随系统涨跌颜色,持仓盈亏列参考交易页从 `112px` 扩为 `144px`,金额/收益率固定单行显示;自选股票代码和价格统一为当前信号“等待中”的 `text-white/80`。生产暂仍为 `v10.7.9.288`,部署成功后回填实际提交和入口。
- 最新已上线补充: `v10.7.9.288` 首页财报和股票文字层级降亮已上线,production runtime commit `c3fe394abe7f8ec10f7e14eb535b2fda9377cba9`;财报日历标题/代码、自选/持仓当前标签和股票代码统一参考当前信号“等待中”的 `text-white/80`,“名称”表头与价格/涨跌幅统一为 `text-white/40`,股票代码取消粗体。行情数据、涨跌色、排序、API、交易账本、收益快照、数据库、RLS 和鉴权不变。
...
- 当前 GitHub `main`: 最新运行时代码提交 `c3fe394abe7f8ec10f7e14eb535b2fda9377cba9` 包含 `v10.7.9.288` 首页财报和股票文字层级降亮;本文件所在最新提交为该 runtime 部署证据回填。
- 当前生产运行时基准提交: `c3fe394abe7f8ec10f7e14eb535b2fda9377cba9`。
- 设置页版本: `v10.7.9.289`（待发布;生产暂为 `v10.7.9.288`）。
- 当前生产地址: `https://boduan-tracker.vercel.app`。
- 最新 Vercel 状态: `npm run verify:deploy-status -- c3fe394` pass;GitHub Actions run `29106135911` success,Vercel status success,target `https://vercel.com/chenshuai1190-7580s-projects/boduan-tracker/Hmz9qmLKnbA9Baat2QWLVVNNDK5x`;production alias 已更新,入口 `/assets/index-CxiEJZF5.js`。
- 最近交接文档刷新部署: 本文件所在最新提交为 `v10.7.9.288` runtime production 验证证据回填。
- 线上关键验证: `npm run verify:deploy-status -- c3fe394` pass;GitHub Actions run `29106135911` success;Vercel status success;生产入口 `/assets/index-CxiEJZF5.js`;未登录 `/api/quote?symbols=VIX` 和 `/api/earnings-calendar?symbols=NVDA` 均返回 `401`;线上 `App-BtJDwnkT.js`、`HomeTab-CmMtgaID.js`、`SettingsTab-BVX7YHYH.js` 和 `settingsChangelog-Bp16Mh5K.js` 与本地构建 SHA-256 一致,生产 marker 确认 `v10.7.9.288`、`首页财报与股票文字降亮`、`text-white/80`、`text-white/40` 和正常字重股票代码 class 存在,旧粗体股票代码 class 不存在。
...
- 当前 GitHub source 基准提交: 最新运行时代码提交 `c3fe394abe7f8ec10f7e14eb535b2fda9377cba9` 包含 `v10.7.9.288` 首页财报和股票文字层级降亮;本文件所在最新提交为该 runtime 部署证据回填。
- 当前生产运行时基准提交: `c3fe394abe7f8ec10f7e14eb535b2fda9377cba9`。
- 最近应用代码提交: production 最近应用代码提交 `c3fe394abe7f8ec10f7e14eb535b2fda9377cba9` 包含 `v10.7.9.288` 首页财报和股票文字层级降亮;此前 `ca932917d893ce966a05a999d4ead2d415291724` 包含 `v10.7.9.287` 首页行情超限分批热修,`aa7fe68429491a170b637889ea4c95cd8670e3c3` 包含 `v10.7.9.286` 首页财报日历智能上移,`a0832b369a657ca95029da78c727acabbdff36ef` 包含 `v10.7.9.285` 热门股票弹窗实时行情和 `v10.7.9.284` 自选添加股票校验;更早收益报表、个股详情、财报日历、BTC/指数拆分和 iOS 主屏实时链路历史见开发日志。
- 最近文档/配置记录提交: 本文件所在最新提交;流程工具链运行提交为 `c47b6e0b78115ea0e004c8cc5b498a2505527fc4`。
- 设置页版本: `v10.7.9.289`（待发布;生产暂为 `v10.7.9.288`）。
- Vercel 最新部署: `v10.7.9.288` runtime commit `c3fe394abe7f8ec10f7e14eb535b2fda9377cba9` 已 success,target `https://vercel.com/chenshuai1190-7580s-projects/boduan-tracker/Hmz9qmLKnbA9Baat2QWLVVNNDK5x`,production 入口 `/assets/index-CxiEJZF5.js`,关键 chunks 包括 `/assets/App-BtJDwnkT.js`,`/assets/HomeTab-CmMtgaID.js`,`/assets/SettingsTab-BVX7YHYH.js`,`/assets/settingsChangelog-Bp16Mh5K.js`,`/assets/i18n-DirP5Edj.js`。
- 最近交接文档刷新部署: 本文件所在最新提交只同步 `v10.7.9.288` runtime production 验证证据。
...
- 最新补充部署记录: `v10.7.9.288` runtime code commit `c3fe394abe7f8ec10f7e14eb535b2fda9377cba9` pushed via project SSH key;GitHub Actions `CI` run `29106135911` success;Vercel status success,target `https://vercel.com/chenshuai1190-7580s-projects/boduan-tracker/Hmz9qmLKnbA9Baat2QWLVVNNDK5x`;production alias updated;active runtime assets and auth boundaries verified。生产入口 `/assets/index-CxiEJZF5.js`;关键 chunks 包括 `/assets/App-BtJDwnkT.js`,`/assets/HomeTab-CmMtgaID.js`,`/assets/SettingsTab-BVX7YHYH.js`,`/assets/settingsChangelog-Bp16Mh5K.js`,`/assets/i18n-DirP5Edj.js`。上一条 `v10.7.9.287` 首页行情超限分批热修为 runtime code commit `ca932917d893ce966a05a999d4ead2d415291724`;更早运行时代码部署历史见 `docs/development-log.md`。
...
当前 GitHub main: 最新运行时代码提交 `c3fe394abe7f8ec10f7e14eb535b2fda9377cba9` 包含 `v10.7.9.288` 首页财报和股票文字层级降亮
当前运行时基准提交: `c3fe394abe7f8ec10f7e14eb535b2fda9377cba9`
设置页版本: `v10.7.9.289`（待发布;生产暂为 `v10.7.9.288`）
最新 Vercel 部署: `c3fe394abe7f8ec10f7e14eb535b2fda9377cba9` success,target `https://vercel.com/chenshuai1190-7580s-projects/boduan-tracker/Hmz9qmLKnbA9Baat2QWLVVNNDK5x`
最新生产入口: `/assets/index-CxiEJZF5.js`

关键线上验证:
- `v10.7.9.289` 已获授权立即部署:首页持仓盈亏取消粗体、跟随系统涨跌色并扩大到交易页同款 144px 单行列;自选股票代码和价格统一为“等待中”同款 `text-white/80`;生产暂仍为 `v10.7.9.288`
- `npm run verify:deploy-status -- c3fe394` pass: GitHub Actions run `29106135911` success,Vercel status success,生产入口 `/assets/index-CxiEJZF5.js`,未登录 quote/earnings 均为 `401`
...
- 本轮只改首页财报日历卡片位置判断;不改 `/api/quote`、`/api/earnings-calendar`、RLS、交易账本、收益快照或行情 relay
