...
6. Keep automated P&L report snapshots server-only.
   ...
   - The primary path and `/api/pnl-report-daily-snapshot-retry` alias run in separate Hobby hourly windows and share the same `CRON_SECRET` gate. Per-symbol provider retries are bounded at three attempts; an incomplete required holding or missing target-date SPY calendar row must return 503 instead of 200 or silently choosing an older day.
...
7. Keep community competition snapshots independent and server-only.
   ...
   - `/api/community-competition-daily-snapshot-retry` is only a rewrite to the same protected branch, not a new public function. Scheduled runs read the complete SPY session sequence from the earliest pending anchor through the target and process each member from `eligible_after_snapshot_date` or the latest locked row in ascending order, bounded to 5 trading dates or 250 member-days per invocation. Remaining work resumes on a later primary/retry run. `missing_close`, `snapshot_gap`, and `trade_between_snapshots` are recoverable incomplete states and return 503; ledger-hash mismatch, oversell, late records, and out-of-range prices remain authoritative rejections and can never be forced through by retry.
   ...
   - Leaderboard responses may return only the ticker codes held at the leaderboard close date, and only after the server-computed official ledger hash matches that same-date locked competition snapshot. A missing or mismatched hash must return an unavailable state. Never return user IDs, email, shares, cost, amounts, allocation, positions, or trade rows. Missing locked snapshots or QQQ EOD closes must remain null/`--`, never mock or realtime substitutes. QQQ must be labeled as the ETF benchmark, not as the NDX index.
   - Production recovery verification (2026-07-14): runtime commit `9e1c840e0b336a0352b79f691b7ce3a3b252ff98` passed GitHub Actions run `29313005445` and Vercel deployment `DSGn5mQnzs2o1x6ohQWD6DGrMy2Y` is Ready. For snapshot date `2026-07-13`, personal P&L contains 12 portfolio rows for 12 users and 54 symbol rows across 18 symbols; competition contains 8 locked rows for 8 users, with 9 active, 9 eligible, 8 initialized, and 0 invalid snapshot rows. One member remains authoritatively rejected as `eligible_ledger_hash_mismatch`; no formal trade row was changed and no return was fabricated.
...
- Automated P&L report snapshots use `/api/pnl-report-daily-snapshot` plus its protected retry alias, return non-200 for incomplete work (including a conservative holiday target without a same-date SPY row), and write only the independent P&L report snapshot tables. Neither personal nor competition automation mutates `stock_trades` or creates mock returns.
