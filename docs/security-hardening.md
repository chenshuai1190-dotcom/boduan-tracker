...
8. Keep stock return comparison authenticated, read-only, and fail closed.
   ...
   - QQQ uses provider ordinary closes for the comparison cash-flow ledger. The current position-cycle start and selected range determine the requested baseline; the actual baseline is the first date on or after it with both an existing personal stock snapshot and an ordinary QQQ close, with both sides initialized from the same capital.
   - Later buys mirror the actual executed dollar value into QQQ, and sells reduce QQQ by the same pre-sale holding ratio. Trade/snapshot disagreement, missing exact trade-date QQQ closes, missing final closes, or invalid data must return an unavailable state. Production must never substitute realtime prices, adjusted-return estimates, synthetic rows, or mock returns.
