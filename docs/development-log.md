...
- Changes:
  - 近 12 个自然月槽位和缺月状态保持不变；累计增长的起点改为窗口内首个真实、未冲突的该账户快照，终点仍严格要求当前月真实快照，不回退旧月、不补零、不插值或沿用前值。
  - 只有当前月一个真实快照时，起点与终点相同并显示 `0.0%`；真实起点余额为零时只保留可证明的金额变化，并明确显示“起点为 0 / Started From 0”，不生成无穷或伪造百分比。
  - 月份对比浮层默认关闭，只有点击/滑动柱图后显示；选中后点击图表区域外只清除小浮层选择，不关闭账户走势主弹窗，也不改变柱图内连续选月逻辑。
  - “我”和“老婆”的账户分别在各自分组内，按当前月余额的人民币等值从高到低排列。CNY 使用原值，USD/HKD 复用现有每日汇率；同额保持原数据库顺序，零余额隐藏规则不变，结果不写回 `sort_order`。
...
- Commit: runtime `f6b53a834df5550bcdeef18e1181ddd8b382a9da`；本条部署证据由后续 docs-only 提交回填。
...
- Deployment: completed；runtime `f6b53a834df5550bcdeef18e1181ddd8b382a9da` 已通过项目 SSH key 推送 GitHub `main`，GitHub Actions run `29438656651` success，Vercel `https://vercel.com/chenshuai1190-7580s-projects/boduan-tracker/FvseyF5Gu2Dh3gJm8C7M4twbxRuE` success，production alias 已切换，入口 `/assets/index-DN3icOHQ.js`，CSS `/assets/index-DTG3hddA.css`。
- Production verification: `npm run verify:deploy-status -- f6b53a8` pass；未登录 quote VIX 和 earnings NVDA 均为 `401`。生产 `index.html` 与 30 个 assets 全部和本地最终 production build 逐字节一致；入口、CSS、App、AnalysisTab、ActionModalCard、SettingsTab、settings changelog 和 i18n 的 SHA-256 逐项一致。生产命中 `v10.7.9.342`、“单个账户真实资产走势”、“不补零、不插值或沿用旧余额”、账户趋势空态和账户趋势 DOM marker，且不包含 `dev_me_bank_hkd`、`wingLungTrendPreview`、`accountTrendPreviewId`、`219500` 或 `DevVisualPreview`。
...
- Commit: runtime `ae9eb03e725d4a3407eca020b7832e94edd972c4`；本条部署证据由后续 docs-only 提交回填。
...
- Deployment: completed；runtime `ae9eb03e725d4a3407eca020b7832e94edd972c4` 已通过项目 SSH key 推送 GitHub `main`，GitHub Actions run `29417637784` success，Vercel `https://vercel.com/chenshuai1190-7580s-projects/boduan-tracker/BmcXTYQaBfw7oZq5oSovXjqfXQ1W` success，production alias 已切换，入口 `/assets/index-DAkXD4bD.js`。
- Production verification: `npm run verify:deploy-status -- ae9eb03` pass；未登录 quote VIX 和 earnings NVDA 均为 `401`。生产入口、App、HomeTab、SettingsTab、settings changelog、i18n 和 stockRealtime 共 7 个关键产物与本地 production build SHA-256 逐项一致；生产命中 `v10.7.9.341`、“盘前实时”、等待收盘状态和 `EODHD_WS_QUOTE` marker，且不包含 `earningsLiveSmoke`、`earningsAutoOpen`、`dev-earnings-resume` 或本地 ASML fixture 数值。
...
- Deployment: completed；runtime `0c979d522b79bea7e3c090fa737a07d38d76ace3` 已通过项目 SSH key 推送 GitHub `main`，GitHub Actions run `29395152514` success，Vercel `https://vercel.com/chenshuai1190-7580s-projects/boduan-tracker/F1rynuez82PaqaquAtxwvfAxDjpV` success，production alias 已切换，入口 `/assets/index-DtGGmaQO.js`。
- Production verification: `npm run verify:deploy-status -- 0c979d5` pass；未登录 quote VIX、earnings NVDA 和 community competition day 均为 `401`。生产入口、App、比赛页、比赛缓存、设置页、设置更新日志和 i18n 共 7 个关键 JS 产物与本地 production build SHA-256 逐项一致。
...
- Rollback: 回退 runtime `0c979d522b79bea7e3c090fa737a07d38d76ace3` 中的 `snapshotUpdatedAt` API 字段、缓存 v2 校验、固定美东格式化、PWA 恢复监听、v339 版本/更新日志及对应测试即可恢复 v338；无需数据、SQL 或 RLS 回滚。
...
- Changes:
  - 本人昵称移入与“最后更新”共用的网格行,通过 `items-center` 保持同一水平中心线;更新时间继续位于第三指标列并保持原有左对齐位置。
  - 本人昵称由 15px/20px 行高调整为 12px/16px 行高,颜色改为排行榜普通用户名同款 `text-white/[0.72]`;半粗体、单行和超长截断保护保留。
  - 昵称安全宽度由 72px 增为 80px,左偏移由 8px 调整为 12px,与 56px 头像精确同心;右侧距离指标列仍保留 4px。目标昵称“波段玩家1836”完整显示,更长合法昵称仍按安全边界省略。
...
### 2026-07-15 - v10.7.9.337 部署证据回填

- Commit: `same commit`（docs-only 证据提交；运行时代码提交见下方）。
- Background: v337 收益比赛本人昵称与头卡紧凑化已经通过 UI-fast 门禁并部署到 production,需要把实际 runtime、Actions、Vercel、入口、产物一致性和未授权边界写回 GitHub `main`。
- Workflow tier: `docs-only`。
- Changes:
  - 记录 runtime commit `f3b354f06914ffede57b08c18d81bb95aa1815a7`、GitHub Actions run `29362348502` success 和 Vercel deployment `https://vercel.com/chenshuai1190-7580s-projects/boduan-tracker/EcTkB5yWLNTDZ1qvoSiiyHnsXpZe` success;生产入口更新为 `/assets/index-DIcYEBHf.js`,设置页版本为 `v10.7.9.337`。
  - 生产入口、CSS、App、比赛页、设置页、设置更新日志和 i18n 共 7 个关键产物与本地 production build 的 SHA-256 逐项一致。
  - 未登录 quote VIX 与 earnings NVDA 均为 `401`;没有登录生产账户或执行生产写操作。
- Key files: `docs/development-log.md`,`docs/handoff.md`。
- Validation: `npm run verify:deploy-status -- f3b354f` pass;GitHub/Vercel 均为 success,production alias 已切换。7 个关键产物 hash 和 2 条未授权边界通过;本 docs-only 变更的 `npm run verify:docs-consistency`、`git diff --check` 和 `git diff --stat` pass,不重复运行已完成的 UI-fast 测试、build 或 Simulator 验收。
- Deployment: runtime 已完成;本条 docs-only 证据提交推送后再验证其自身 Actions/Vercel,不改变生产运行时逻辑。
- Boundaries: 只回填真实发布证据,不改 UI、比赛缓存/API、排名、收益、快照、交易账本、数据库、RLS、provider、环境变量或生产数据。
- Rollback: 回退本 docs-only 条目只移除证据记录;恢复 v336 比赛头卡样式需要单独回退 runtime `f3b354f`。

### 2026-07-15 - v10.7.9.337 收益比赛本人昵称与头卡紧凑化

- Commit: `f3b354f06914ffede57b08c18d81bb95aa1815a7`。
...
- Changes:
  ...
  - “最后更新”行由 `mt-2` 改为 `mt-0`,相对放大昵称后的初版稿上移约 8px;头卡底部由 12px 收紧为 8px,相对上一张本地效果卡片整体降低约 12px。
...
- Deployment: completed;runtime commit `f3b354f06914ffede57b08c18d81bb95aa1815a7`,GitHub Actions run `29362348502` success,Vercel `https://vercel.com/chenshuai1190-7580s-projects/boduan-tracker/EcTkB5yWLNTDZ1qvoSiiyHnsXpZe` success,production alias 已更新,入口 `/assets/index-DIcYEBHf.js`。
- Production verification: 入口、CSS、App、比赛页、设置页、更新日志和 i18n 共 7 个关键产物与本地 build SHA-256 一致;未登录 quote 与 earnings 均为 `401`。
...
- Commit: `same commit`（docs-only 证据提交；运行时代码提交见下方）。
- Background: v336 持仓市值长金额显示修复已经通过 UI-fast 门禁并部署到 production,需要把实际 runtime、Actions、Vercel、入口、产物一致性和未授权边界写回 GitHub `main`。
- Workflow tier: `docs-only`。
- Changes:
  - 记录 runtime commit `76bf439b908f886af76487a2af1411a0f18f4ac6`、GitHub Actions run `29358437006` success 和 Vercel deployment `https://vercel.com/chenshuai1190-7580s-projects/boduan-tracker/ATnLaHUdyrUp6h4cq6dpKgDneyUM` success;生产入口更新为 `/assets/index--_5WFpJf.js`,设置页版本为 `v10.7.9.336`。
  - 生产入口、CSS、App、交易页、设置页、设置更新日志和 i18n 共 7 个关键产物与本地 production build 的 SHA-256 逐项一致。
  - 未登录 quote VIX 与 earnings NVDA 均为 `401`;没有登录生产账户或执行生产写操作。
- Key files: `docs/development-log.md`,`docs/handoff.md`。
- Validation: `npm run verify:deploy-status -- 76bf439` pass;GitHub/Vercel 均为 success,production alias 已切换。7 个关键产物 hash 和 2 条未授权边界通过;本 docs-only 变更的 `npm run verify:docs-consistency`、`git diff --check` 和 `git diff --stat` pass,不重复运行已完成的 UI-fast 测试、build 或 Simulator 验收。
- Deployment: runtime 已完成;本条 docs-only 证据提交推送后再验证其自身 Actions/Vercel,不改变生产运行时逻辑。
- Boundaries: 只回填真实发布证据,不改 UI、金额、汇率、持仓数据、交易账本、API、数据库、RLS、provider、环境变量或生产数据。
- Rollback: 回退本 docs-only 条目只移除证据记录;恢复 v335 持仓列宽需要单独回退 runtime `76bf439`。

### 2026-07-15 - v10.7.9.336 持仓市值长金额完整显示

- Commit: `76bf439b908f886af76487a2af1411a0f18f4ac6`。
...
- Deployment: completed;runtime commit `76bf439b908f886af76487a2af1411a0f18f4ac6`,GitHub Actions run `29358437006` success,Vercel `https://vercel.com/chenshuai1190-7580s-projects/boduan-tracker/ATnLaHUdyrUp6h4cq6dpKgDneyUM` success,production alias 已更新,入口 `/assets/index--_5WFpJf.js`。
- Production verification: 入口、CSS、App、交易页、设置页、更新日志和 i18n 共 7 个关键产物与本地 build SHA-256 一致;未登录 quote 与 earnings 均为 `401`。
...
- Commit: `same commit`（docs-only 证据提交；运行时代码提交见下方）。
- Background: v334 比赛头卡昵称、更新时间与页面宽度已经通过 UI-fast 门禁并部署到 production,需要把实际 runtime、Actions、Vercel、入口、产物一致性和未授权边界写回 GitHub `main`。
- Workflow tier: `docs-only`。
- Changes:
  - 记录 runtime commit `4f19c6fc14461ddff1a4c0e9be13a61b440a4ce3`、GitHub Actions run `29346467101` success 和 Vercel deployment `https://vercel.com/chenshuai1190-7580s-projects/boduan-tracker/TNJLaYSR8WAirn8eQhsax6shY91j` success;生产入口更新为 `/assets/index-C5YbGHop.js`,设置页版本为 `v10.7.9.334`。
  - 生产入口、CSS、App、比赛页面、设置页、设置更新日志和 i18n 共 7 个关键产物与本地 production build 的 SHA-256 逐项一致。
  - 未登录 quote VIX 与 earnings NVDA 均为 `401`;没有登录生产账户或执行生产写操作。
- Key files: `docs/development-log.md`,`docs/handoff.md`。
- Validation: `npm run verify:deploy-status -- 4f19c6f` pass;GitHub/Vercel 均为 success,production alias 已切换。7 个关键产物 hash 和 2 条未授权边界通过;本 docs-only 变更的 `npm run verify:docs-consistency`、`git diff --check` 和 `git diff --stat` pass,不重复运行已完成的 UI-fast 测试、build 或 Simulator 验收。
- Deployment: runtime 已完成;本条 docs-only 证据提交推送后再验证其自身 Actions/Vercel,不改变生产运行时逻辑。
- Boundaries: 只回填真实发布证据,不改 UI、日期、缓存、API、账本、快照、数据库、RLS、provider、环境变量或生产数据。
- Rollback: 回退本 docs-only 条目只移除证据记录;恢复 v333 页面样式需要单独回退 runtime `4f19c6f`。

### 2026-07-14 - v10.7.9.334 收益比赛头卡与页面宽度统一

- Commit: `4f19c6fc14461ddff1a4c0e9be13a61b440a4ce3`。
- Background: 用户要求把比赛更新时间精简为“最后更新 MM.DD”,在本人头像下显示社区昵称并稍微放大/下移,同时让比赛所有卡片与首页保持相同宽度;本地检查还发现生产壳层与比赛页叠加边距,且原 `430px` 页面上限会在 Pro Max/宽屏再次缩窄内容。
- Workflow tier: `ui-fast`。本轮只调整系统文案、字号、间距、容器边距和最大展示宽度;不改变日期来源、缓存、请求、排名、收益计算、交易账本、API、数据库、鉴权、RLS、provider 或任何持久化行为。
- Changes:
  - 中文更新时间改为“最后更新 {{date}}”,English 保持 `Last updated {{date}}`;文字与日期之间保留空格,日期继续由 `formatCompactDate(data?.asOfDate)` 动态生成,不写死日期。
  - 本人头像下显示权威 `self.nickname`,字号由 9.5px 微调到 10.5px,头像间距由 4px 调到 6px;72px 单行安全截断不侵入右侧三项指标。
  - 生产 App 壳层在独立比赛页取消重复 `px-4`,比赛内容统一为首页相同的 16px 左右边距;页面上限由 430px 调到 462px,扣除两侧边距后与首页同为 430px 最大卡片宽度。
  - 设置页版本和中英文更新日志同步到 `v10.7.9.334`,静态边界断言覆盖动态文案、昵称来源/间距、重复 padding 和宽屏最大宽度。
- Key files: `src/App.jsx`,`src/pages/CommunityCompetitionPage.jsx`,`src/lib/i18n.js`,`src/tabs/SettingsTab.jsx`,`src/lib/settingsChangelog.js`,`tests/tool-ledger-boundaries.test.js`,`docs/development-log.md`,`docs/handoff.md`。
- Validation: 定向边界测试 47/47 pass;`npm run build`、`npm run verify:docs-consistency` 和 `git diff --check` pass。本机 Xcode iOS 26.5 `iPhone 17 Pro` Simulator Safari 只读 `DevVisualPreview` 已确认昵称间距、动态更新时间和比赛/首页 16px 左右边缘一致;截图 `~/Desktop/boduan-previews/community-competition-home-width-local.png` 与首页参考 `~/Desktop/boduan-previews/home-card-width-reference-local.png`。另用 `iPhone 17 Pro Max` 复核 440pt 宽屏,卡片保持 16px 左右边距且不再受旧 430px 外层上限缩窄;截图 `~/Desktop/boduan-previews/community-competition-home-width-v334-iphone-17-pro-max.png`。按 `ui-fast` 不运行完整 `npm test`、audit、RLS 或旧 `verify:frontend-smoke`。
- Deployment: completed;runtime commit `4f19c6fc14461ddff1a4c0e9be13a61b440a4ce3`,GitHub Actions run `29346467101` success,Vercel `https://vercel.com/chenshuai1190-7580s-projects/boduan-tracker/TNJLaYSR8WAirn8eQhsax6shY91j` success,production alias 已更新,入口 `/assets/index-C5YbGHop.js`。
- Production verification: 入口、CSS、App、比赛页面、设置页、更新日志和 i18n 共 7 个关键产物与本地 build SHA-256 一致;未登录 quote、earnings 均为 `401`。
- Boundaries: 客户端仍不生成、修正或伪造任何比赛收益;服务端真实 `asOfDate`、按账户/周期缓存、美东刷新窗口、独立比赛 API/Cron、正式 `stock_trades`、比赛表、Supabase/RLS 和生产数据全部不变。
- Rollback: 回退本条展示样式、v334 版本/更新日志、静态断言与文档即可恢复 v333;无需数据、SQL、RLS 或环境回滚。
...
- Key files: database source commit contains `supabase/stock_trades.sql`,`supabase/community_competition.sql`,`supabase/rls.sql`,`supabase/community_competition_rebaseline_20260714.sql`,`scripts/verify-rls-rest.mjs`,`tests/community-profiles.test.js`,`docs/development-log.md`. Runtime commit contains the personal/competition API, Cron, model, tests, configuration and release-document changes listed by this entry.
- Validation:
...
- Deployment: complete。Database source `0f52700761beab0d4488e067ca9e968aea9a9bc1` and production SQL completed first;runtime `8f23a471be3cd63b657bf1f7a807c438881a23ea` then passed GitHub Actions run `29321470173` and Vercel target `https://vercel.com/chenshuai1190-7580s-projects/boduan-tracker/CkNECvKe9N3WGSLokcaWFdYkUxvg` success。SQL was applied at about 05:00 America/New_York, outside every scheduled snapshot window, and no manual Cron was triggered. Production entry remains `/assets/index-DrckgGpM.js` because this release has no frontend change.
- Production verification: `npm run verify:deploy-status -- 8f23a47` pass。Unauthenticated quote VIX, market movers, earnings NVDA, P&L benchmark, competition GET/POST, competition snapshot GET and P&L snapshot GET all reject with `401`;POST to the GET-only snapshot routes returns `405` as expected。D1/D2 can only be observed after real closes and must not be prefilled;the `2026-07-13` repair remains only the previous-runtime baseline.
...
- Commit: `same commit`。
- Background: `v10.7.9.326` 真实美股收盘涨跌榜和 `v10.7.9.327` 个股收益与 QQQ 同期现金流对比已通过 sensitive 门禁并部署到 production,需要把最终运行时、流水线、入口和未登录安全边界写回唯一代码源头。
- Workflow tier: `docs-only`。
- Changes:
  - 当前生产 runtime 更新为 `55e02c8faa8fdac7b57b44335b6edb5d153494a2`,设置页版本为 `v10.7.9.327`,入口为 `/assets/index-DW_Jo82w.js`。
  - 记录 GitHub Actions run `29269405728` 与 Vercel target `526DFjY3SKDmd9md2rpx9Cjoq9Ns` success。
  - 更新交接当前状态、首次接手基准和最新可转发块,删除 v326/v327 待发布表述。
- Key files: `docs/development-log.md`,`docs/handoff.md`。
- Validation: `npm run verify:deploy-status -- 55e02c8faa8fdac7b57b44335b6edb5d153494a2` pass;生产入口、Settings、StockDetail、Home、i18n 和更新日志 chunks 命中 `v10.7.9.327`、“收益对比”/`First buy`、“涨幅榜”/“跌幅榜”及中英文更新标题;运行时 chunks 不含 `devMarketMoversFixture` / `stockReturnBenchmarkRows` 标识。未登录 quote、market-movers、earnings、pnl-benchmark 四条 API 均返回 `401`。
- Deployment: runtime GitHub Actions `29269405728` success,Vercel `https://vercel.com/chenshuai1190-7580s-projects/boduan-tracker/526DFjY3SKDmd9md2rpx9Cjoq9Ns` success,production alias 已更新,入口 `/assets/index-DW_Jo82w.js`;本 docs-only 提交推送后再确认自身 Actions/Vercel success 且生产入口未异常切换。
- Production verification: `SettingsTab-DpMW-boE.js` 命中 `v10.7.9.327`,`StockDetailPage-CuSoiDTP.js` 命中“收益对比”,`HomeTab-RVzG_gIx.js` 命中“涨幅榜”/“跌幅榜”,`i18n-CvwWM-GG.js` 命中 `First buy`,`settingsChangelog-hc9z3NJB.js` 命中 v327 中英文更新标题;未登录 `/api/quote?symbols=VIX`、`/api/quote?view=market-movers`、`/api/earnings-calendar?symbols=NVDA`、`/api/pnl-benchmark?symbol=QQQ&from=2026-06-01&to=2026-07-10` 均为 `401`。
- Boundaries: 本提交只回填部署证据,不改运行时代码、API、交易账本、个人收益快照、比赛、数据库、RLS、provider、环境变量或生产数据。
- Rollback: 回退本条和交接当前状态即可;不回退已验证的 v327 运行时代码。

### 2026-07-14 - 个股收益与 QQQ 同期现金流对比

- Commit: `55e02c8faa8fdac7b57b44335b6edb5d153494a2`。
...
- Deployment: completed;GitHub Actions run `29269405728` success,Vercel target `https://vercel.com/chenshuai1190-7580s-projects/boduan-tracker/526DFjY3SKDmd9md2rpx9Cjoq9Ns` success,production alias 已更新,入口 `/assets/index-DW_Jo82w.js`。
- Production verification: 生产 Settings/StockDetail/Home/i18n/更新日志 chunks 命中 `v10.7.9.327`、收益对比、首笔买入、涨幅榜/跌幅榜及中英文更新标题,且不含本地财务 fixture 标识;未登录 quote、market-movers、earnings、pnl-benchmark 均为 `401`。真实 30/30 榜单和 QQQ `rawClose` 证据来自发布前同代码、server-only provider 探针;本轮未伪称已完成生产登录态人工数据检查。
...
- Commit: `4e56573afaf06680ed22bbbf90ea081e9b559479`。
...
- Deployment: completed;独立代码提交 `4e56573afaf06680ed22bbbf90ea081e9b559479` 随 v327 runtime `55e02c8faa8fdac7b57b44335b6edb5d153494a2` 上线,GitHub Actions run `29269405728` 与 Vercel target `526DFjY3SKDmd9md2rpx9Cjoq9Ns` success,生产入口 `/assets/index-DW_Jo82w.js`。
- Production verification: Home/i18n/更新日志生产 chunks 命中涨幅榜、跌幅榜和 v326 更新内容,运行时不含 `devMarketMoversFixture`;未登录 market-movers 返回 `401`。发布前真实 server-only 探针确认严格范围内同日涨/跌各 30 只;本轮未伪称已完成生产登录态人工榜单检查。
...
- Workflow tier: `docs-only`。
- Changes:
  - 交接基准统一提升到 `v10.7.9.323` / runtime `d8ea6ef5f292116c2e3ad0a6e6e6ac1f6d602376`,补齐 Actions `29198603364`、Vercel `6M39R6ojEAwcenGshecCRPHVKcw2`、生产入口和当前 docs-only 部署证据。
  - 重写“下一个人接手后的第一步”,删除仍要求确认旧版 `v10.7.9.294` 的过期门禁,改为 workspace/bootstrap、风险分档和当前生产基准核验。
  - 新增“快速上线必须通过”清单:UI-fast 必须完成定向测试或说明、build、docs consistency、diff check、iOS Simulator 用户可见验收和推送后的 deploy status;任一失败不得上线。
...
- Commit: `d8ea6ef5f292116c2e3ad0a6e6e6ac1f6d602376`。
- Deployment: `v10.7.9.323` 已上线。`npm run verify:deploy-status -- d8ea6ef` pass;GitHub Actions run `29198603364` success,Vercel target `https://vercel.com/chenshuai1190-7580s-projects/boduan-tracker/6M39R6ojEAwcenGshecCRPHVKcw2` success,生产入口 `/assets/index-DN2-ymxd.js`。
...
- Changes:
  - 中英文词典补齐设置页使用的全部 77 个 `settings.*` 键;英文模式中的语言、显示配色、账户状态、邀请码、账户切换、社区资料、修改密码、错误提示和无障碍关闭文案不再回退中文。
...
- Validation: 设置/弹窗定向测试 46/46 pass、`npm run build`、docs consistency、`git diff --check` 和 deploy status 均 pass;词典静态扫描确认设置页使用的 77 个 `settings.*` 键在中英文词典均 77/77 存在。已在本机 Xcode iOS 26.5 `iPhone 17 Pro` Simulator Safari 打开 `?devPreview=1&tab=settings&lang=en`:Language、Display、Account、Switch Account、Log Out、Changelog 和底部导航均为英文,配色摘要 `Red ↑ · Green ↓` 完整显示;社区昵称“波段玩家1836”按用户内容规则保持原文。截图: `~/Desktop/boduan-previews/settings-english-v323.png`。生产 quote、earnings 未登录保持 `401`。本轮不改业务逻辑、数据或安全边界,因此按 UI-fast 不跑完整 `npm test` 或 audit。
...
- Workflow tier: `ui-fast`。
- Changes:
  - “12 个月资产走势”从独立 `fixed inset` 旧外壳切换到交易/账户操作共用的 `ActionModalCard`,统一遮罩、27px 玻璃卡、标题、关闭按钮、内容滚动和底部中性操作按钮。
  - 保留原宽版几何:`390px` 桌面上限、手机端左右各 16px;移除标题旧日历图标、金色底部按钮和独立 `70vh` 滚动外壳。
  - 按本地反馈取消月份列表外侧的整块内框、内背景和内阴影,只保留每个月份小卡的弱边界;其他共享弹窗不受影响。
...
- Workflow tier: `runtime`。
- Changes:
  - 当前年右侧摘要的目标、实现、落后/超额三行增加右对齐百分比;目标固定显示 `100%`。
  - 实现百分比严格按 `当年实际收益 / 当年计划目标`,落后/超额百分比按 `|差额| / 当年计划目标` 计算;未填写时显示 `--`。
...
- Workflow tier: `runtime`。
- Changes:
  - 仅在设置页头部身份卡的头像容器增加低亮中性外边框、暗色底层和轻量外发光,素材本身继续保持 v313 的无白边裁切。
  - 头部头像容器从 `79px` 放大到 `95px`,约增加 20%;卡片宽度、最小高度和昵称位置逻辑不变。
...
- Workflow tier: `runtime`。
- Changes:
  - 设置页、资料选择器与收益比赛头像统一从 `scale-[1.02]` 加深到 `scale-[1.15]`,把素材圆形外侧的白底完全裁出可视区域。
  - 设置页头部身份头像从 `66px` 放大到 `79px`,约增加 20%;头像卡片宽度和最小高度不变。
...
- Workflow tier: `sensitive`。
- Changes:
  - 新增最多 5 个账户的本机会话保险箱,只保存 Supabase access/refresh session 与邮箱展示信息,不保存密码;设置页可查看当前账户、直接切换、确认移除和添加账户。
  - 使用官方 `auth.setSession` 恢复目标账户;普通退出显式使用 `signOut({ scope: 'local' })`,避免默认 global 退出使其他设备或已添加账户会话失效。
  - `AuthGate` 以 `user.id` 作为 `MainApp` key,账户变化后完整卸载旧应用状态并重新读取目标用户云端数据;添加账户期间可取消回到原账户。
  - 数据库离线缓存、摊薄工具记录/当前标的、波段展开记忆、行情诊断和预警记忆改为 `user.id` 隔离键;旧全局缓存不迁移,防止复制到另一账户。
...
- Deployment: completed;GitHub Actions run `29177139868` success,Vercel target `https://vercel.com/chenshuai1190-7580s-projects/boduan-tracker/7n18YeG4g6329NyX3qwZmqDF7uDu` success,production alias 已更新,入口 `/assets/index-BdrG4ob7.js`。
...
- Deployment: completed;GitHub Actions run `29176830040` success,Vercel target `https://vercel.com/chenshuai1190-7580s-projects/boduan-tracker/HXVkbhd5FJcrwE51eCmjusQnK7qm` success,production alias 已更新,入口 `/assets/index-BcjncCZW.js`。
...
- Deployment: completed;GitHub Actions run `29162215875` success,Vercel target `https://vercel.com/chenshuai1190-7580s-projects/boduan-tracker/GLo8GFDrxfEwbWznX5adZXyEHDht` success,production alias 已更新,入口 `/assets/index-CPJKiFds.js`。
...
- Commit: runtime `4bfab846ab6d7b87ea9ce41af26e80aeeed3b6ad`;后续 docs-only 证据回填提交待追加。
- Deployment: completed;使用项目 SSH key 推送 GitHub `main`,GitHub Actions run `29159386949` success,Vercel status success,target `https://vercel.com/chenshuai1190-7580s-projects/boduan-tracker/5BdqRfYevu2vMhpshwAUrCUqoqXo`,production alias 已更新,入口 `/assets/index-B4MFy0ZP.js`。2026-07-12 已在生产 Supabase SQL Editor 执行 `supabase/community_profiles.sql`;`npm run verify:rls:rest` 18/18 pass,`community_profiles` 匿名 REST 返回 `401`。
...
- Validation: `npm run verify:workspace-state` pass;`npm run verify:local-env` pass;`node --test tests/tool-ledger-boundaries.test.js` 42/42 pass;`npm test` 203/203 pass;`npm run build` pass,生成 `SettingsTab-CynW1UAG.js`,`communityProfile-BQxHddR-.js`,`settingsChangelog-ChrBLMSV.js`,`i18n-8IWC6k6Q.js` 和 `App-ToHpWbAd.js`;`npm run verify:frontend-smoke` 5/5 pass,console/runtime error 0;`npm audit --audit-level=moderate` found 0 vulnerabilities;`npm run verify:docs-consistency` pass;`npm run verify:rls:rest` 18/18 pass;未登录 `/api/quote?symbols=VIX` 与 `/api/earnings-calendar?symbols=NVDA` 均为 `401`;`git diff --check` pass。390x844 本地 `?devPreview=1&tab=settings` CDP 视觉复核:社区资料模块显示默认头像、昵称输入、6 个默认头像和保存按钮;`scrollWidth=390` / `clientWidth=390`,设置页命中 `v10.7.9.301`,console error 0。生产 marker 递归扫描 26 个 JS chunks,命中 `/assets/index-B4MFy0ZP.js`,`/assets/App-ToHpWbAd.js`,`/assets/SettingsTab-CynW1UAG.js`,`/assets/settingsChangelog-ChrBLMSV.js`,`/assets/communityProfile-BQxHddR-.js`,`/assets/i18n-8IWC6k6Q.js`,`v10.7.9.301`,`设置页社区资料上线`,`community_profiles`,`community-avatars/avatar-gold.webp`,`社区资料`,`保存社区资料` 和 `默认头像`。截图: `~/Desktop/boduan-previews/settings-community-profile-top-cdp-390x844.png`,`settings-community-profile-module-cdp-390x844.png`。
...
- Rollout gate: 本地代码/视觉验证、生产 SQL、匿名 REST RLS gate、部署、线上 marker 和 quote/earnings `401` 均已完成;双用户 owner/cross-user 隔离 smoke 因本机无非空 admin 通道留待后续补做。
...
- Deployment: completed;使用项目 SSH key 推送 GitHub `main`,GitHub Actions run `29156492612` success,Vercel status success,target `https://vercel.com/chenshuai1190-7580s-projects/boduan-tracker/7mr1AwcSoVhKz5VToaTjoYgtPAh2`,production alias 已更新,入口 `/assets/index-BXPK-qSG.js`。`npm run verify:deploy-status -- eae8a7a` pass,未登录 `/api/quote?symbols=VIX` 与 `/api/earnings-calendar?symbols=NVDA` 均返回 `401`;生产关键 chunks `/assets/App-BKWfipPv.js`,`/assets/CommunityCompetitionPage-sFBPPnBQ.js`,`/assets/TradesTab-YTBUkhPh.js`,`/assets/SettingsTab-CV_7bZrF.js`,`/assets/settingsChangelog-BuACAEgI.js` 均可访问,marker 命中 `v10.7.9.300`、`社区比赛 mock 小工具第一版`、`社区比赛`、`全部功能`、`boduan_community_competition_joined_v1` 和深灰头像边框 `border-[#2a313b]/90`,且不含旧 `border-white/12`。
...
- Rollout gate: 本地验证、GitHub Actions、Vercel、生产入口/版本 marker 和未登录 quote/earnings `401` 均已完成。
- Rollback: 回退新增页面、交易页入口、App/DevVisualPreview 接线、i18n、设置页版本/更新日志、测试和本条文档并重新部署即可;无需数据库或数据回滚。
...
- Workflow tier: `runtime`。
- Changes:
  - 波段首页恢复股票卡片折叠/展开按钮,同股多波段不再被 `lockedExpanded` 强制锁定展开。
  - 新增 `boduan_wave_tracker_expanded_v1` 本地状态,按“全部 / 进行中 / 已完成”筛选和标准化股票代码分别记忆展开状态。
  - 没有用户记忆时,当前筛选下同一股票多个波段仍默认展开,保留完整展示的默认体验;用户手动收起后下次进入保持收起。
...
- Commit: runtime `18f25333c8fb4cfddb54eb4298afc8d9e20d171e`;本条后续 docs-only 提交只回填部署证据。
- Deployment: completed;使用项目 SSH key 推送 GitHub `main`,GitHub Actions run `29155184666` success,Vercel status success,target `https://vercel.com/chenshuai1190-7580s-projects/boduan-tracker/FGs3ThBsRE6nE1XLB8zW8tpdKDZn`,production alias 已更新,入口 `/assets/index-C4i0j3Ob.js`。线上 `App-CFi3XLQy.js` 引用 `WaveTrackerPage-ClYqGD2a.js`、`ActionModalCard-CTI_wgqk.js` 和 `SettingsTab-BUnYaY2P.js`;设置页与 `settingsChangelog-BXVNnlzy.js` 均命中 `v10.7.9.298`,波段 chunk 命中默认 `active` 筛选、`wave-form-date-input` 和完整卖出提示。`npm run verify:deploy-status -- 18f2533` pass,未登录 `/api/quote?symbols=VIX` 与 `/api/earnings-calendar?symbols=NVDA` 均返回 `401`。
...
- Changes:
  ...
  - 设置页三个可见版本面和更新日志已同步上线为 `v10.7.9.298`。
...
- Validation: `npm run verify:workspace-state` pass;`npm run verify:local-env` pass;`npm run verify:toolchain` pass;`node --test tests/swing-waves.test.js tests/tool-ledger-boundaries.test.js` 52/52、`npm test` 201/201、`npm run build`、`npm run verify:frontend-smoke` 5/5、`npm audit --audit-level=moderate`（0 vulnerabilities）、`npm run verify:docs-consistency` 和 `git diff --check` 均通过。使用真实生产页面组件与只读 `DevVisualPreview` 在 390x844 完成首页/新增弹框截图复核,并在 390x500、390x300 键盘压缩视口依次录入股票代码、买入成本、买入数量和备注;弹框始终位于可视区域内、备注可滚动到达、关闭后滚动锁完整恢复,浏览器 console error 0。运行时样式确认日期 `line-height: 40px`、上下 padding 为 0;取消和确认的 color/background/border/opacity 完全一致,无效表单下确认仍有原生 `disabled`。线上 `npm run verify:deploy-status -- 18f2533` pass,生产 marker 命中 `v10.7.9.298`、新 `WaveTrackerPage` / `ActionModalCard` / `SettingsTab` chunks 与更新日志。
...
- Rollout gate: 本地视觉复核、完整 `runtime` 验证、GitHub Actions、Vercel、生产入口/版本 marker 和未登录 quote/earnings `401` 均已完成。
...
- Deployment: completed;使用项目 SSH key 推送 GitHub `main`,GitHub Actions run `29154192896` success,Vercel status success,target `https://vercel.com/chenshuai1190-7580s-projects/boduan-tracker/B44CodL2WgfoRBHcFqSCr1yQcMH1`,production alias 已更新,入口 `/assets/index-D58eoxFB.js`。线上 `App-Ye5GmQSu.js` 引用独立 `WaveTrackerPage-DijsB-a2.js`,`SettingsTab-BPMtXyIq.js` 命中 `v10.7.9.297` 并引用 `settingsChangelog-8fy72x0C.js`;波段 chunk 命中“波段记录”“新增波段”和“波段需一次性卖出，不支持部分卖出。”。`npm run verify:deploy-status -- b56b712` pass,未登录 `/api/quote?symbols=VIX` 与 `/api/earnings-calendar?symbols=NVDA` 均返回 `401`。
...
- Changes:
  ...
  - 设置页三个可见版本面和更新日志已同步上线为 `v10.7.9.297`。
...
- Sensitive remote note: 本轮代码修改前、生产 SQL 执行后已确认 `npm run verify:rls:rest` 17/17 pass,其中 `swing_waves` 匿名 GET 为 `401`,未登录 quote/earnings 均为 `401`。收口中曾遇到本机到生产域名的 TLS 重置,但部署完成后的标准状态脚本已成功取回生产入口并重新确认 quote/earnings `401`。生产 Supabase SQL editor 选取两个现有真实 `auth.users`,分别设置 `role=authenticated` 与对应 `request.jwt.claim.sub`,完成独立 A/B 记录的 CRUD/RLS 隔离 smoke:双方只读取本人记录,跨用户 read/update/delete 均为 0,owner update/complete/delete 均为 1,1.5 碎股保持准确,最终 14/14 结果均为 `true`;独立清理查询 `no_smoke_rows=true`。该验证未导出 service-role key,也不是密码登录 REST token 会话。
...
- Rollout gate: 本地 390x844 与 320x568 真实页面/弹框视觉复核、完整 `sensitive` 验证、双真实 Auth 用户 CRUD/RLS 14/14 隔离 smoke、GitHub Actions、Vercel、生产入口/版本 marker 和未登录 quote/earnings `401` 均已完成。
- Rollback: 如需回退,撤销独立页面、入口、view model、共用 Logo、i18n、版本/更新日志和测试并重新部署;不要删除已创建且当前为空的生产 `swing_waves` 表。若未来明确要求数据库回滚,必须另做显式审计 SQL,不得顺带删除旧波段或其它账本数据。
...
- Workflow tier: `runtime`。
- Changes:
  - 新增明确的波段 USD 单价格式化出口;买入均价、卖出均价、当前价、三处分支的交易明细单价、全部波段交易弹窗单价和删除确认单价统一固定为 USD。
  - 波段必填校验和保存确认中的已输入价格显式显示美元;录入标签继续为 `价格 ($)`。
  - 浮盈、总盈亏和成交总金额继续跟随首页 USD/CNY;百分比、股数和天数不参与换算。
