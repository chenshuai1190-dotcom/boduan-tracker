...
- 自动个人/比赛收盘任务对 EODHD symbol 做最多三次有界重试,可恢复缺口和没有目标日 SPY 行的保守休市判断返回 503,不退回旧日期。`v10.7.9.349` 保持 v346 的三个统一 `/api/close-snapshot-schedule*` 窗口,同一调用只以 `America/New_York` 解析一次目标日并并行启动两个独立 runner。无显式日期的调用在美东 17:00 前只返回 deferred 且不访问 provider/数据库。合法显式日期仍只走各自受 `CRON_SECRET` 保护的人工修复路径,必须先确认目标日精确 SPY 收盘;比赛显式路径不允许 rebaseline。个人继续按最近 31 个日历日的 SPY 真交易日补缺口,比赛仍按完整 SPY 日历有界分批;两套表、完成语义和 fail-closed 结果不合并。
...
- 其余安全恢复只允许 active 且全表 0 snapshots、两个 ranking 字段均为空的成员在 scheduled D1 向前重建 eligible date/hash/revision。D1 严格校验完整正式账本的非空 USD、正数成交价/股数、字段、顺序和不超卖,并拉取 SPY 与所有相关个股的精确正数 D1 EOD;缺个股 D1 行、provider 失败或旧日收盘替代都不得调用 RPC。D1 当日交易还必须由数据库时间证明为同一纽约日期且 `created_at <= 16:00 America/New_York`。已发布 v347 因内部比赛属性取消 provider raw high/low 成交价区间门槛，但不取消上述真实收盘和账本防篡改边界。D1 不写 snapshot/ranking/收益;D2 下一真实收盘只有账本未变且精确 EOD 再次通过才可开始排名。已有任一 snapshot/ranking 永久禁止 rebaseline。`service_role`-only RPC 与 membership/snapshot insert guards 都先锁 revision row 再锁 member row,并在锁内重验 revision、active 状态与日期;malformed ledger row 不得被过滤。两条链路都不改 `stock_trades`,不新增浏览器数据面、账本写入口或 mock/实时价/估算收益兜底。生产 revision/CAS SQL、21-table + 2-RPC REST gate、metadata 回读、真实双事务并发 smoke 与 runtime `8f23a471be3cd63b657bf1f7a807c438881a23ea` 已完成;真实 D1/D2 补缺口仍待收盘后观察,不得提前声称结果。
- v350 沿用 v347 引入的 service-only `snapshot_publication_markers`,仅保存 channel、snapshot date、不透明 version 和 database-authored completed timestamp。发布前不再信任 runner 的汇总 success，而是重新读取 active、资料完成、目标日前已开始排名的真实 cohort，并要求每名成员都存在 exact target、有效收益、lock/source/hash/revision 完整的不可变行。部分 `7/8`、`8/9` 返回脱敏 retryable 503，由 catch-up 继续从最后真实 anchor 顺序补齐；只有 `8/8`、`9/9` 才发布，later-start 未排名成员不阻塞历史日。PUBLIC/anon/authenticated 无表权限,浏览器只能通过已登录 `operation=snapshot-status` 获取四项脱敏字段。v350 UI 使用 `asOfDate` 标记收益日期，marker completion time 只作审计/缓存兼容。可见旧榜每分钟最多读取一次状态,四周期共享独立的最新 publication metadata,完整榜只在 marker 推进时读取;Web Lock、持久清缓存 generation 与单调 marker 比较防止旧标签页回写。QQQ 或完整榜暂不可用时保留上一完整缓存并有界冷却,不得缓存半成品。v349 cache v5 保持；每位用户保留自然周期内的个人真实累计起点，QQQ 从本人同一起点计算，榜单按个人收益减本人同期 QQQ 的超额收益排序。新用户首份有效收盘后立即进入全部榜单，不等待下一年，也不重置旧用户日期或累计收益。该 marker 是持久完成边界,不是 Supabase Realtime publication。数据库源 `0bc0ef2`、生产 migration、metadata/grant 回读、22-table + 2-RPC gate 和 v348 bootstrap 均保持不变；iOS 主屏 PWA 验证仍 pending。
- 生产 v348 已修复 marker 首次上线没有接纳历史锁定快照的升级遗漏。原始只读证据为 23 条快照（`2026-07-13/14/15` 分别 `8/7/8`）且 marker 0 行，因此 API waiting 并非数据被删除。source `3c85f64c0dcb26afd4b6b776f1a4039a7b0fb961` 中的 `snapshot_publication_marker_bootstrap_20260716.sql` 只在 marker 为空时，对 2026-07-16 前的 profiles/members/snapshots/marker 边界加锁，选择最新 exact complete locked batch；缺资料/资格/ranking baseline/64-hex hash/revision、少行或意外行都拒绝候选，无候选异常回滚。精确 SQL SHA-256 `b53314d864dd568d5525814de681be9e3d758edf2dc1da8654f85ed5080de806` 已应用生产；postflight 为 marker `2026-07-13` / `verified_bootstrap_20260716`、8 expected / 8 complete / 0 missing / 1 later-start。快照仍为 23 条、分布仍为 `8/7/8`、摘要仍为 `4e144e79415dd4f423bcfd76b8fe500b`。它只写 marker，没有生成或改写 snapshot/收益/交易；07-14/15 缺口仍由正式美东 17:00 后 scheduler 追赶。
- v349 修复的是 marker 已经补齐后旧 PWA 仍不唤醒的客户端缓存缺口,不是第二次数据库恢复。生产只读核验确认 snapshot、marker 与已登录完整榜 API 均已完整;用户现场同一时刻其他 PWA 请求正常,但比赛页产生 0 次 `community-competition` 请求。根因是 waiting cache 在 marker 后补前已耗尽两次完整读取,又没有当前 target 的 statusCheck,随后命中 `attempt_limit` 并睡到下一纽约收盘窗口。cache v5 淘汰 v4；只有已满足资格、当前 target 仍为 `waiting_snapshot`、完整读取次数已耗尽且 statusCheck 未初始化时,才启动轻量 `snapshot-status`。marker 日期或版本推进后才读取完整榜。该修复不修改 snapshot、收益、排名、交易、参赛资格、marker 或任何数据库行。
...
- [~] Verify Supabase RLS live: the production anonymous REST probe now passes 22 table probes plus 2 RPC-denial probes, including anonymous `401` for `community_profiles`, `community_competition_members`, `community_competition_snapshots`, `stock_trade_ledger_revisions`, and `snapshot_publication_markers`; the new marker table also passed FORCE RLS, zero policy/Realtime publication, trigger/function, constraint/index and service-only grant readback. `swing_waves` metadata passed 13/13 checks, and its two-real-user authenticated-role/JWT-claim CRUD isolation smoke passed 14/14. Complete a two-user owner/cross-user isolation smoke for `community_profiles` when a stable admin channel is available.
...
- [~] Add the independent real community competition boundary: strict auth, opt-in gating, immutable percentage-only close snapshots, no-mock UI, and `v10.7.9.303` application deployment are complete. The authoritative revision/database-time/CAS and forward-only recovery production release remains verified. v349 keeps that data model independent behind the unified scheduler and service-only publication marker plus authenticated lightweight status cache. Cache v5 evicts stuck v4 waiting entries and initializes only the eligible current-target lightweight status poll after bounded full-read exhaustion; it still requires marker advancement before another full read. Marker-driven cache writes remain monotonic across periods/tabs, QQQ incompleteness fails closed, and full-read failures preserve the previous complete cache with a bounded retry. Database source `0bc0ef2`, production marker migration, metadata/grant readback, the 22-table + 2-RPC gate, and the fail-closed one-time historical marker bootstrap from source `3c85f64` are complete and unchanged. The bootstrap published only the exact complete `2026-07-13` batch and left all 23 immutable snapshots unchanged. iOS Home Screen PWA verification and real-close D1/D2 observation remain pending.
