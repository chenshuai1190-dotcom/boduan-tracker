...
- 自动个人/比赛收盘任务对 EODHD symbol 做最多三次有界重试,可恢复缺口和没有目标日 SPY 行的保守休市判断返回 503,不退回旧日期。当前生产 `v10.7.9.347` 已包含 v346 的三个统一 `/api/close-snapshot-schedule*` 窗口,同一调用只以 `America/New_York` 解析一次目标日并并行启动两个独立 runner。无显式日期的调用在美东 17:00 前只返回 deferred 且不访问 provider/数据库。合法显式日期仍只走各自受 `CRON_SECRET` 保护的人工修复路径,必须先确认目标日精确 SPY 收盘;比赛显式路径不允许 rebaseline。个人继续按最近 31 个日历日的 SPY 真交易日补缺口,比赛仍按完整 SPY 日历有界分批;两套表、完成语义和 fail-closed 结果不合并。
...
- 其余安全恢复只允许 active 且全表 0 snapshots、两个 ranking 字段均为空的成员在 scheduled D1 向前重建 eligible date/hash/revision。D1 严格校验完整正式账本的非空 USD、正数成交价/股数、字段、顺序和不超卖,并拉取 SPY 与所有相关个股的精确正数 D1 EOD;缺个股 D1 行、provider 失败或旧日收盘替代都不得调用 RPC。D1 当日交易还必须由数据库时间证明为同一纽约日期且 `created_at <= 16:00 America/New_York`。待发布 v347 因内部比赛属性取消 provider raw high/low 成交价区间门槛，但不取消上述真实收盘和账本防篡改边界。D1 不写 snapshot/ranking/收益;D2 下一真实收盘只有账本未变且精确 EOD 再次通过才可开始排名。已有任一 snapshot/ranking 永久禁止 rebaseline。`service_role`-only RPC 与 membership/snapshot insert guards 都先锁 revision row 再锁 member row,并在锁内重验 revision、active 状态与日期;malformed ledger row 不得被过滤。两条链路都不改 `stock_trades`,不新增浏览器数据面、账本写入口或 mock/实时价/估算收益兜底。生产 revision/CAS SQL、21-table + 2-RPC REST gate、metadata 回读、真实双事务并发 smoke 与 runtime `8f23a471be3cd63b657bf1f7a807c438881a23ea` 已完成;真实补缺口仍待 v347 发布后验证,不得提前声称结果。
- 生产 v347 已使用 service-only `snapshot_publication_markers`,仅保存 channel、snapshot date、不透明 version 和 database-authored completed timestamp。整批比赛完成前不得发布 marker;部分行或 batch-limited 结果不得提前暴露新日期。PUBLIC/anon/authenticated 无表权限,浏览器只能通过已登录 `operation=snapshot-status` 获取四项脱敏字段。可见旧榜每分钟最多读取一次状态,四周期共享独立的最新 publication metadata,完整榜只在 marker 推进时读取;Web Lock、持久清缓存 generation 与单调 marker 比较防止旧标签页回写。QQQ 或完整榜暂不可用时保留上一完整缓存并有界冷却,不得缓存半成品。v347 缓存为 v4；每位用户保留自然周期内的个人真实累计起点，QQQ 从本人同一起点计算，榜单按个人收益减本人同期 QQQ 的超额收益排序。新用户首份有效收盘后立即进入全部榜单，不等待下一年，也不重置旧用户日期或累计收益。该 marker 是持久完成边界,不是 Supabase Realtime publication。数据库源 `0bc0ef2`、生产 migration、metadata/grant 回读、22-table + 2-RPC gate 和 runtime 均已完成；iOS 主屏 PWA 验证仍 pending。
- 待发布 v348 修复 marker 首次上线没有接纳历史锁定快照的升级遗漏。生产只读证据为 23 条快照（`2026-07-13/14/15` 分别 `8/7/8`）且 marker 0 行，因此 API waiting 并非数据被删除。`snapshot_publication_marker_bootstrap_20260716.sql` 只在 marker 为空时，对 2026-07-16 前的 profiles/members/snapshots/marker 边界加锁，选择最新 exact complete locked batch；缺资料/资格/ranking baseline/64-hex hash/revision、少行或意外行都拒绝候选，无候选异常回滚。它只写 marker，不生成或改写 snapshot/收益/交易；预计完整候选为 2026-07-13，07-14/15 缺口仍由正式美东 17:00 后 scheduler 追赶。source/生产执行与验证均 pending。
...
- [~] Add the independent real community competition boundary: strict auth, opt-in gating, immutable percentage-only close snapshots, no-mock UI, and `v10.7.9.303` application deployment are complete. The authoritative revision/database-time/CAS and forward-only recovery production release remains verified. Production v347 keeps that data model independent behind the unified scheduler and service-only publication marker plus authenticated lightweight status cache. Marker-driven cache writes are monotonic across periods/tabs, QQQ incompleteness fails closed, and full-read failures preserve the previous complete cache with a bounded retry. Database source `0bc0ef2`, production migration, metadata/grant readback, the 22-table + 2-RPC gate, and runtime deployment are complete. Pending v348 adds only a fail-closed one-time bootstrap for the pre-marker exact complete historical batch; it never writes snapshots or returns. Its source/production execution and verification, iOS Home Screen PWA verification, and real-close D1/D2 observation remain pending.
