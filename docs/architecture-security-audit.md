...
- v349 沿用 v347 引入的 service-only `snapshot_publication_markers`,仅保存 channel、snapshot date、不透明 version 和 database-authored completed timestamp。整批比赛完成前不得发布 marker;部分行或 batch-limited 结果不得提前暴露新日期。PUBLIC/anon/authenticated 无表权限,浏览器只能通过已登录 `operation=snapshot-status` 获取四项脱敏字段。可见旧榜每分钟最多读取一次状态,四周期共享独立的最新 publication metadata,完整榜只在 marker 推进时读取;Web Lock、持久清缓存 generation 与单调 marker 比较防止旧标签页回写。QQQ 或完整榜暂不可用时保留上一完整缓存并有界冷却,不得缓存半成品。v349 缓存升级为 v5 并淘汰 v4；每位用户保留自然周期内的个人真实累计起点，QQQ 从本人同一起点计算，榜单按个人收益减本人同期 QQQ 的超额收益排序。新用户首份有效收盘后立即进入全部榜单，不等待下一年，也不重置旧用户日期或累计收益。该 marker 是持久完成边界,不是 Supabase Realtime publication。数据库源 `0bc0ef2`、生产 migration、metadata/grant 回读、22-table + 2-RPC gate 和 v348 bootstrap 均保持不变；iOS 主屏 PWA 验证仍 pending。
