...
- 自动个人/比赛收盘任务对 EODHD symbol 做最多三次有界重试,可恢复缺口和没有目标日 SPY 行的保守休市判断返回 503,不退回旧日期。当前生产 runtime 仍为 `v10.7.9.343`,使用两组独立 UTC 21/22/23 Cron;本地待发布 `v10.7.9.346` 改为三个统一 `/api/close-snapshot-schedule*` 窗口,同一调用只以 `America/New_York` 解析一次目标日并并行启动两个独立 runner。无显式日期的调用在美东 17:00 前只返回 deferred 且不访问 provider/数据库。合法显式日期仍只走各自受 `CRON_SECRET` 保护的人工修复路径,必须先确认目标日精确 SPY 收盘;比赛显式路径不允许 rebaseline。个人继续按最近 31 个日历日的 SPY 真交易日补缺口,比赛仍按完整 SPY 日历有界分批;两套表、完成语义和 fail-closed 结果不合并。
...
- 待发布 v346 新增 service-only `snapshot_publication_markers`,仅保存 channel、snapshot date、不透明 version 和 database-authored completed timestamp。整批比赛完成前不得发布 marker;部分行或 batch-limited 结果不得提前暴露新日期。PUBLIC/anon/authenticated 无表权限,浏览器只能通过已登录 `operation=snapshot-status` 获取四项脱敏字段。可见旧榜每分钟最多读取一次状态,四周期共享独立的最新 publication metadata,完整榜只在 marker 推进时读取;Web Lock、持久清缓存 generation 与单调 marker 比较防止旧标签页回写。QQQ 或完整榜暂不可用时保留上一完整缓存并有界冷却,不得缓存半成品。待发布 v347 把缓存升到 v4；每位用户保留自然周期内的个人真实累计起点，QQQ 从本人同一起点计算，榜单按个人收益减本人同期 QQQ 的超额收益排序。新用户首份有效收盘后立即进入全部榜单，不等待下一年，也不重置旧用户日期或累计收益。该 marker 是持久完成边界,不是 Supabase Realtime publication。数据库源 `0bc0ef2`、生产 migration、metadata/grant 回读和 22-table + 2-RPC gate 已完成；runtime 与 iOS 主屏 PWA 验证仍 pending。
...
- [~] Add the independent real community competition boundary: strict auth, opt-in gating, immutable percentage-only close snapshots, no-mock UI, and `v10.7.9.303` application deployment are complete. The authoritative revision/database-time/CAS and forward-only recovery production release remains verified. Pending v346 keeps that data model independent behind a unified scheduler and adds a service-only publication marker plus authenticated lightweight status cache. Marker-driven cache writes are monotonic across periods/tabs, QQQ incompleteness fails closed, and full-read failures preserve the previous complete cache with a bounded retry. Database source `0bc0ef2`, production migration, metadata/grant readback and the 22-table + 2-RPC gate are complete; current full 464/464, build, toolchain, audit high, docs consistency and diff check pass locally. Runtime deployment, iOS Home Screen PWA verification, and real-close D1/D2 observation remain pending.
