...
门禁出现断言失败时，先停止发布，由开发代理自动定位原因并修复任务范围内的问题：过时的视觉合同随已批准的 UI 更新；真实业务失败修复实现并补回归测试。不得通过删除测试、`skip` 或降低标准凑通过。修复后，对最终 diff 完整重跑所选门禁；涉及新增权限或无法安全判断的业务规则时，说明阻塞并请求确认，不承诺自动修复所有错误。
