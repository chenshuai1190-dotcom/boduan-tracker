-- Forward-only P&L report immediate rebuild foundation.
--
-- This phase is deliberately additive so the previously deployed browser and
-- scheduled writer keep working while the new runtime is rolled out. The
-- later privilege contraction is a separate after-deploy migration.
...
-- Prevent a ledger write from being handled by the old revision trigger while
-- rebuild-state revision/generation are backfilled and the new trigger body is
-- installed. This lock is migration-only; runtime RPCs use per-user row locks.
