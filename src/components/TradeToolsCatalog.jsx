...
import { BarChart3, BookOpen, Calculator, ChevronRight, History, ListChecks, TrendingUp, Users, Waves } from 'lucide-react';
...
const TOOL_GROUPS = [
  ...
  {
    id: 'review', titleKey: 'trades.toolsReview', icon: BookOpen, columns: true,
    tools: [
  ...
  return <div className="trade-tools-catalog">
    {TOOL_GROUPS.map(group => {
      ...
      return <section key={group.id} className="trade-tools-group" aria-labelledby={`${headingId}-${group.id}`}>
        ...
        <div className={`trade-tools-items${group.columns ? ' trade-tools-columns' : ''}`}>
          {group.tools.map(tool => {
            const Icon = tool.icon;
            return <button key={tool.id} type="button" data-tool-id={tool.id} className={`trade-tool-card${tool.featured ? ' trade-tool-featured' : ''}`} onClick={() => onSelect(tool.id)}>
