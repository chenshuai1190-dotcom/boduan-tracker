...
function SummaryValue({ value, yoy, language, marketColorMode, muted = false }) {
  return (
    <div className="text-right">
      <div
        className={`text-[13.5px] tabular-nums ${muted ? 'text-white/[0.30]' : 'text-white/[0.80]'}`}
  ...
  return (
    <div className="mt-3">
      <div className="grid grid-cols-[minmax(0,1.12fr)_0.92fr_0.92fr] items-end border-b border-white/[0.045] py-2">
        <div className="text-[9.5px] text-white/[0.25]">{language === 'en' ? 'Metric' : '指标'}</div>
        <div className="text-right text-[9.5px] leading-[1.35] text-white/[0.28]">
        ...
        {rows.map((row) => (
          <div key={row.key} className="grid grid-cols-[minmax(0,1.12fr)_0.92fr_0.92fr] items-center py-2.5">
            <div className="min-w-0 pr-2">
              <div className="truncate text-[11.5px] text-white/[0.65]">{row.label}</div>
              {row.qualifier ? <div className="mt-0.5 text-[9px] text-white/[0.24]">{row.qualifier}</div> : null}
            </div>
...
function Metric({ label, value, detail, color = 'rgba(255,255,255,0.78)', align = 'left' }) {
  return (
    <div className={align === 'right' ? 'text-right' : ''}>
      <div className="text-[10.5px] text-white/[0.31]">{label}</div>
      <div className="mt-1 text-[14px] font-normal tabular-nums" style={{ color, fontFamily: NUMBER_FONT }}>{value}</div>
      {detail ? <div className="mt-0.5 text-[9.5px] text-white/[0.24]">{detail}</div> : null}
    </div>
  );
}

function SectionState({ status, language }) {
  const text = status === 'pending'
    ? (language === 'en' ? 'Official breakdown is syncing' : '官方细分数据同步中')
    : (language === 'en' ? 'No unambiguous structured disclosure' : '本期没有可确认的结构化披露');
  return <div className="rounded-[16px] border border-dashed border-white/[0.07] bg-white/[0.018] px-4 py-8 text-center text-[11px] text-white/[0.30]">{text}</div>;
}
      ...
      <div className="px-4 pb-3 pt-3.5">
        <div className="flex min-w-0 items-start gap-3">
          ...
          <div className="min-w-0">
            <div className="truncate text-[14px] text-white/[0.84]">{language === 'en' ? item.label : item.labelZh || item.label}</div>
            <div className="mt-0.5 truncate text-[10.5px] text-white/[0.30]">{language === 'en' ? item.labelZh : item.label}</div>
          </div>
          <span className="ml-auto shrink-0 rounded-md border border-[#f6b54b]/15 bg-[#f6b54b]/[0.065] px-1.5 py-0.5 text-[9.5px] text-[#f6b54b]/75">
  ...
  return (
    <div className="px-4 py-3.5">
      <div className="grid grid-cols-[22px_minmax(0,1fr)_auto] items-center gap-2.5">
        <span className="flex h-[18px] w-[18px] items-center justify-center rounded-full bg-white/[0.045] text-[9.5px] text-white/[0.28]" style={{ fontFamily: NUMBER_FONT }}>{index + 1}</span>
        <div className="min-w-0">
          <div className="truncate text-[12.5px] text-white/[0.75]">{language === 'en' ? item.label : item.labelZh || item.label}</div>
          <div className="mt-0.5 truncate text-[9.5px] text-white/[0.25]">{language === 'en' ? item.labelZh : item.label}</div>
        </div>
        <div className="text-right">
          <div className="text-[13px] tabular-nums text-white/[0.78]" style={{ fontFamily: NUMBER_FONT }}>{formatEarningsDetailMoney(item.revenue, language)}</div>
          <div className="mt-0.5 text-[9.5px] tabular-nums" style={{ color: valueColor(yoy, marketColorMode), fontFamily: NUMBER_FONT }}>{formatSignedPercent(yoy)}</div>
        </div>
      </div>
      <div className="ml-[32px] mt-2.5 flex items-center gap-2">
        ...
        <span className="w-[34px] text-right text-[9.5px] tabular-nums text-white/[0.26]" style={{ fontFamily: NUMBER_FONT }}>{share == null ? '—' : `${share.toFixed(1)}%`}</span>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="px-4 py-3.5">
      <div className="flex items-center justify-between gap-4">
        <div className="min-w-0">
          <div className="truncate text-[12.5px] text-white/[0.74]">{language === 'en' ? item.label : item.labelZh || item.label}</div>
          <div className="mt-0.5 truncate text-[9.5px] text-white/[0.25]">{language === 'en' ? item.labelZh : item.label}</div>
        </div>
        <div className="flex shrink-0 items-baseline gap-3">
          <span className="text-[9.5px] tabular-nums" style={{ color: valueColor(yoy, marketColorMode), fontFamily: NUMBER_FONT }}>{formatSignedPercent(yoy)}</span>
          <span className="w-[76px] text-right text-[13px] tabular-nums text-white/[0.78]" style={{ fontFamily: NUMBER_FONT }}>{formatEarningsDetailMoney(item.revenue, language)}</span>
        </div>
      </div>
    ...
    <>
      <section className="mt-4 scroll-mt-24">
        <div className="mb-2.5 flex items-center justify-between px-1">
          <div className="flex items-start gap-2.5">
            ...
            <div><h2 className="text-[14px] text-white/[0.80]">{language === 'en' ? 'Reportable segments' : '报告分部'}</h2><p className="mt-1 text-[10px] text-white/[0.27]">{language === 'en' ? 'Official accounting segments' : '公司正式披露的会计分部'}</p></div>
          </div>
          <span className="text-[9.5px] text-white/[0.23]">{report.items.length || '—'} {language === 'en' ? 'segments' : '个分部'}</span>
        </div>
        {report.items.length ? <div className="space-y-2.5">{report.items.map((item, index) => <SegmentCard key={item.id} item={item} index={index} totalRevenue={totalRevenue} language={language} marketColorMode={marketColorMode} />)}</div> : <SectionState status={report.status} language={language} />}
      </section>
      ...
      <section className="mt-4 overflow-hidden rounded-[18px] border border-white/[0.075] bg-[#0b0f15]">
        <div className="flex items-end justify-between border-b border-white/[0.055] px-4 py-3.5">
          <div className="flex items-start gap-2.5"><PieChart className="mt-0.5 h-4 w-4 text-[#f6b54b]/65" /><div><h2 className="text-[14px] text-white/[0.80]">{language === 'en' ? 'Revenue breakdown' : '细分结构'}</h2><p className="mt-1 text-[10px] text-white/[0.27]">{language === 'en' ? 'Products and services' : '产品与服务类别，不等同于会计分部'}</p></div></div>
          <span className="text-[9.5px] text-white/[0.23]">{language === 'en' ? 'Revenue · YoY · Share' : '营收 · 同比 · 占比'}</span>
        </div>
        {breakdown.items.length ? <div className="divide-y divide-white/[0.045]">{breakdown.items.map((item, index) => <RevenueRow key={item.id} item={item} index={index} totalRevenue={totalRevenue} language={language} marketColorMode={marketColorMode} />)}</div> : <div className="p-3"><SectionState status={breakdown.status} language={language} /></div>}
      </section>

      <section className="mt-4 overflow-hidden rounded-[18px] border border-white/[0.075] bg-[#0b0f15]">
        <div className="flex items-end justify-between border-b border-white/[0.055] px-4 py-3.5">
          <div className="flex items-start gap-2.5"><MapPinned className="mt-0.5 h-4 w-4 text-[#60a5fa]/65" /><div><h2 className="text-[14px] text-white/[0.80]">{language === 'en' ? 'Geographic revenue' : '地区收入'}</h2><p className="mt-1 text-[10px] text-white/[0.27]">{language === 'en' ? 'As officially disclosed' : '按公司官方披露口径'}</p></div></div>
          <span className="text-[9.5px] text-white/[0.23]">{language === 'en' ? 'YoY · Share' : '同比 · 占比'}</span>
        </div>
        {regions.items.length ? <div className="divide-y divide-white/[0.045]">{regions.items.map((item) => <RegionRow key={item.id} item={item} totalRevenue={totalRevenue} language={language} marketColorMode={marketColorMode} />)}</div> : <div className="p-3"><SectionState status={regions.status} language={language} /></div>}
      </section>
    </>
  );
}
  ...
  return (
    <div className="min-h-[100dvh] bg-[#05070b] text-white" style={{ fontFamily: PAGE_FONT }}>
      <div ref={exportRef} data-earnings-detail-export-root="true" className="min-h-[100dvh] bg-[#05070b]">
        <div className="pointer-events-none absolute inset-x-0 top-0 h-[220px] bg-[radial-gradient(circle_at_22%_0%,rgba(40,93,77,0.18),transparent_58%)]" />
        <header
        ...
        >
          <div className="mx-auto grid w-full max-w-[430px] grid-cols-[40px_1fr_40px] items-center">
            ...
            <div className="text-center">
              <h1 className="text-[16px] font-normal tracking-[0.02em] text-white/[0.86]">{symbol} {language === 'en' ? 'Earnings detail' : '财报详情'}</h1>
              <p className="mt-0.5 text-[9.5px] text-white/[0.25]">{periodLabel(event, detail, language)} · {earningsResultText(event?.earningsResult, language)}</p>
            </div>
            <button
              data-export-ignore="true"
              type="button"
        ...
        <main className="relative z-10 mx-auto w-[calc(100%-32px)] max-w-[430px] pb-[calc(86px+env(safe-area-inset-bottom))] pt-3.5">
          <section className="w-full overflow-hidden rounded-[20px] border border-white/[0.085] bg-[#0b0f15] shadow-[0_18px_45px_rgba(0,0,0,0.30),inset_0_1px_0_rgba(255,255,255,0.045)]">
            <div className="px-4 pb-4 pt-4">
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0">
                  <div className="flex items-baseline gap-2"><span className="text-[16px] text-white/[0.87]">{symbol}</span><span className="truncate text-[12px] text-white/[0.38]">{name}</span></div>
                  <div className="mt-1 flex items-center gap-1.5 text-[10px] text-white/[0.27]">
                ...
                <span className="ml-auto inline-flex items-center gap-1 rounded-full border border-emerald-400/15 bg-emerald-400/[0.07] px-2 py-1 text-[9.5px] text-emerald-300/75">
              ...
              <div className="mt-3.5 flex items-center justify-between border-t border-white/[0.055] pt-3">
                <span className="text-[10px] text-white/[0.28]">{language === 'en' ? 'Reporting period' : '财报区间'} {displayDate(detail?.period?.start, language)}—{displayDate(detail?.period?.end || event?.fiscalDate, language)}</span>
                <span className="text-[9.5px] text-white/[0.23]">{detail?.currency || 'USD'} · {language === 'en' ? 'B/M' : '万/亿'}</span>
              </div>
          ...
          {loading ? (
            <div className="mt-4 flex h-36 items-center justify-center rounded-[18px] border border-white/[0.07] bg-[#0b0f15] text-[12px] text-white/[0.35]"><Loader2 className="mr-2 h-4 w-4 animate-spin text-[#f6b54b]" />{language === 'en' ? 'Loading official breakdown…' : '正在读取官方细分数据…'}</div>
          ) : detail ? (
            <DetailSections detail={detail} event={event} language={language} marketColorMode={marketColorMode} />
          ) : (
            <div className="mt-4 rounded-[18px] border border-white/[0.07] bg-[#0b0f15] px-4 py-8 text-center text-[12px] text-white/[0.35]">{error}</div>
          )}
          ...
          <div className="mt-3 rounded-[14px] border border-white/[0.05] bg-white/[0.018] px-3.5 py-3">
            <div className="flex items-start gap-2">
              ...
              <div>
                <p className="text-[10px] leading-[1.55] text-white/[0.27]">
                  {language === 'en'
                ...
                <p className="mt-1 text-[9.5px] leading-[1.5] text-white/[0.19]">
                  {language === 'en' ? 'Financial amounts stay in the report currency.' : '财务金额保持财报原币种，不跟随持仓币种换算。'}
                </p>
                {filingUrl ? <a href={filingUrl} target="_blank" rel="noreferrer" className="mt-2 inline-flex items-center gap-1 text-[9.5px] text-[#f6b54b]/55">{language === 'en' ? 'SEC official filing' : 'SEC 官方财报'}<ExternalLink className="h-3 w-3" /></a> : null}
              </div>
            </div>
          </div>
        </main>
      ...
      {shareNotice ? (
        <div data-export-ignore="true" className="fixed left-1/2 z-[70] -translate-x-1/2 rounded-full border border-white/10 bg-[#111720]/95 px-3 py-1.5 text-[10.5px] text-white/70 shadow-xl backdrop-blur-md" style={{ bottom: 'calc(72px + env(safe-area-inset-bottom))' }}>
          {shareNotice}
        </div>
      ) : null}
    </div>
