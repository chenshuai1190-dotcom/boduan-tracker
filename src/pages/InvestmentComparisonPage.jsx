...
export default function InvestmentComparisonPage({ ctx = {}, previewSource }) {
  ...
  return <div className="investment-comparison ic-page" data-investment-comparison-page="true">
    ...
    {snapshot ? <>
      ...
      <InvestmentComparisonChart model={model} snapshot={snapshot} hiddenSymbols={hiddenSymbols} scale={scale} englishMode={englishMode} onPause={() => setPlaying(false)} />
      <div className="ic-playback"><label className="ic-range-head" htmlFor="investment-comparison-timeline"><span>{model.actualStartDate}</span><span>{englishMode ? 'Playback timeline' : '回放时间轴'}</span><span>{model.asOfDate}</span></label><input id="investment-comparison-timeline" className="ic-timeline" type="range" min="0" max={lastIndex} step="1" value={snapshot.index} aria-label={englishMode ? 'Daily playback timeline' : '按交易日回放时间轴'} aria-valuetext={`${snapshot.point.date}, ${symbols.map(symbol => `${symbol} ${formatInvestmentAmount(snapshot.point.values[symbol], englishMode)}`).join(', ')}`} onChange={event => { setPlaying(false); setCursorState({ key: cursorKey, index: Number(event.target.value) }); }} /><div className="ic-player"><button type="button" className="ic-play-button" onClick={togglePlayback} disabled={!lastIndex}>{playing ? <Pause size={16} fill="currentColor" /> : <Play size={16} fill="currentColor" />}<span>{playLabel}</span></button><select className="ic-speed" value={speed} onChange={event => setSpeed(Number(event.target.value))} aria-label={englishMode ? 'Playback speed' : '回放速度'}>{[1, 2, 4].map(value => <option value={value} key={value}>{value}× {englishMode ? 'speed' : '速度'}</option>)}</select></div></div>
