...
const mockReviewLogs = [
  { id: 'dev_log_1', date: '2026-06-11', mood: '冷静', text: '这是一条用于本地 UI 验收的长复盘。今天没有因为盘中波动临时改变计划，而是先检查仓位、现金和风险敞口，再逐笔回顾买卖理由。值得保留的是按既定条件执行，没有追涨，也没有用短期价格替代完成收盘数据；需要改进的是记录还不够及时，部分判断缺少明确证据。下周继续等待高胜率机会，交易前先写清触发条件、失效条件和最大风险，交易后补全结果与反思。即使内容超过原来的 150 字，卡片也应当完整换行显示，不出现省略号，也不再提示查看全文。' },
...
function StandardDevVisualPreview({ initialTab = '' }) {
  ...
  const [activeTab, setActiveTab] = React.useState(() => {
    ...
    return ['home', 'trades', 'analysis', 'review', 'settings', 'pnl-report', 'pnl-share', 'home-margin-risk', 'drawdown-observation', 'stock-detail', 'watchlist-stock-detail', 'wave-tracker', 'community-competition', 'vix-comparison', 'fear-greed', 'investment-comparison', 'portfolio-overlap', 'dca-lab'].includes(requestedTab) ? requestedTab : 'analysis';
  });
  ...
  const drawdownInitialSymbol = typeof window === 'undefined'
    ? ''
    : new URLSearchParams(window.location.search).get('drawdownSymbol') || '';
  ...
  const homeCtx = {
    ...
    openFearGreed: () => { setActiveTab('fear-greed'); window.scrollTo(0, 0); },
    ...
    openWatchlistStockDetail: (symbol) => {
      ...
      setPreviewWatchlistDetailSymbol(normalizedSymbol);
      setActiveTab('watchlist-stock-detail');
    },
    ...
    openDrawdownObservation: () => {
      ...
      setActiveTab('drawdown-observation');
  ...
  const watchlistDetailCtx = {
    ...homeCtx,
    watchlistStockDetailSymbol: watchlistFundCompositionPreview ? 'TQQQ' : previewWatchlistDetailSymbol,
    closeWatchlistStockDetail: () => setActiveTab('home'),
    watchlist: [{
      symbol: watchlistFundCompositionPreview ? 'TQQQ' : previewWatchlistDetailSymbol,
      name: watchlistFundCompositionPreview
        ? 'TQQQ'
        : homeWatchlist.find((row) => row.symbol === previewWatchlistDetailSymbol)?.name || previewWatchlistDetailSymbol,
      targetPriceUsd: watchlistDetailTargetUsd,
    }],
    homeWatchlist: [{
      symbol: watchlistFundCompositionPreview ? 'TQQQ' : previewWatchlistDetailSymbol,
      name: watchlistFundCompositionPreview
        ? 'TQQQ'
        : homeWatchlist.find((row) => row.symbol === previewWatchlistDetailSymbol)?.name || previewWatchlistDetailSymbol,
      price: homeWatchlist.find((row) => row.symbol === previewWatchlistDetailSymbol)?.price || 202.81,
    }],
    quoteRows: [{
      symbol: watchlistFundCompositionPreview ? 'TQQQ' : previewWatchlistDetailSymbol,
      name: watchlistFundCompositionPreview
  ...
  return (
    <div
      className={`min-h-screen ${['fear-greed', 'pnl-report'].includes(activeTab) ? 'bg-[#08090b]' : 'bg-[#05070b]'} text-white ${['pnl-report', 'pnl-share'].includes(activeTab) ? 'pb-0' : 'pb-24'} ${['pnl-report', 'pnl-share', 'community-competition', 'earnings-detail'].includes(activeTab) ? 'px-0' : 'px-4'}`}
      style={{
        paddingTop: ['pnl-report', 'pnl-share', 'home-margin-risk', 'drawdown-observation', 'stock-detail', 'wave-tracker', 'community-competition', 'watchlist-stock-detail', 'earnings-detail', 'vix-comparison', 'fear-greed', 'investment-comparison', 'portfolio-overlap', 'dca-lab'].includes(activeTab) ? 0 : 'calc(1rem + env(safe-area-inset-top))',
        ...(visualViewportWidth
          ...
          <div className="report-bottom-nav-grid grid grid-cols-5">
            {nav.map(tab => {
              ...
              const isActive = tab.id === activeTab
                ...
                || (activeTab === 'fear-greed' && tab.id === 'home')
