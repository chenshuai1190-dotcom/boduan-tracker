...
test('main trade entry modal uses compact four-step buy sell submission flow', () => {
  ...
  assert.ok(settingsTabSource.includes('v10.7.9.183'), 'settings version badge should document the daily pnl baseline protection update');
  assert.ok(settingsChangelogSource.includes('v10.7.9.183'), 'settings changelog should document the daily pnl baseline protection update');
...
test('asset and review module cards do not keep legacy scale interactions', () => {
  ...
  assert.ok(settingsTabSource.includes('v10.7.9.183'), 'settings version badge should document the latest daily pnl baseline protection update');
  assert.ok(settingsChangelogSource.includes('v10.7.9.183'), 'settings changelog should document the latest daily pnl baseline protection update');
...
test('review target page uses dark mobile cards and click action modals', () => {
  ...
  assert.ok(reviewTabSource.includes("{tt('review.actualGain', '实际收益')} <span className=\"text-rose-400 tabular-nums\""), 'compound actual gain should use the home pink amount color');
