...
test('main trade entry modal uses compact four-step buy sell submission flow', () => {
  ...
  assert.ok(settingsTabSource.includes('v10.7.9.171'), 'settings version badge should document the tool modal and currency sync update');
  assert.ok(settingsChangelogSource.includes('v10.7.9.171'), 'settings changelog should document the tool modal and currency sync update');
  assert.ok(settingsChangelogSource.includes('工具弹窗和币种同步'), 'settings changelog should describe the tool modal and currency sync update');
...
test('language framework covers settings switch, bottom nav, home page, and stock ticker display', () => {
  ...
  assert.ok(i18nSource.includes("'review.disciplines': 'Investment Rules'"), 'English dictionary should include review discipline labels');
  ...
  assert.ok(appSource.includes("tt('review.editDiscipline', '编辑戒律')"), 'Review discipline modal should read labels from i18n');
...
test('asset and review module cards do not keep legacy scale interactions', () => {
  ...
  assert.ok(settingsTabSource.includes('v10.7.9.171'), 'settings version badge should document the latest tool modal and currency sync update');
  assert.ok(settingsChangelogSource.includes('v10.7.9.171'), 'settings changelog should document the latest tool modal and currency sync update');
  assert.ok(settingsChangelogSource.includes('工具弹窗和币种同步'), 'settings changelog should describe the latest tool modal and currency sync update');
  assert.ok(settingsChangelogSource.includes('v10.7.9.170'), 'settings changelog should retain the trade entry modal refinement');
  assert.ok(settingsChangelogSource.includes('交易录入弹窗细节修正'), 'settings changelog should describe the trade entry modal refinement');
