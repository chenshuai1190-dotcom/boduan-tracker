...
test('review target page uses a flat dark report and preserves click action modals', () => {
  ...
  const previewRouteList = devVisualPreviewSource.match(/return (\[[^\n]+\])\.includes\(requestedTab\) \? requestedTab : 'analysis';/)?.[1];
  assert.ok(previewRouteList, 'local visual preview must guard its requested route');
  assert.deepEqual([...previewRouteList.matchAll(/'([^']+)'/g)].map(([, route]) => route), [
