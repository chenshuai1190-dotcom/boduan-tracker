...
test('asset page visual shell and local preview stay debuggable', () => {
  ...
  assert.ok(analysisTabSource.includes('const requestedChartSlot = overviewChartModel.slots[chartSelectedMonthIdx]') && analysisTabSource.includes('const overviewChartReading = requestedChartSlot?.hasData && requestedChartSlot.month !== currentMonth') && !analysisTabSource.includes('overviewChartModel.slots[overviewChartLatestIndex] || null'), 'only an explicitly selected valid historical month should have a chart reading, without duplicating the current total by default');
