...
test('approved modal families share the new shell without widening business boundaries', () => {
  ...
  const assetTrendPageStart = analysisTabSource.indexOf('if (showMonthsDetail) {');
  const assetTrendPageEnd = analysisTabSource.indexOf('<div className="space-y-3.5 text-[#f5f7fb]"', assetTrendPageStart);
  ...
  assert.ok(monthlyAssetTrendContentSource.includes('data-asset-trend-month-row={slot.month}') && monthlyAssetTrendContentSource.includes('onClick={() => onEditMonth?.(slot.month)}'), 'every monthly detail row should hand off its exact month');
  ...
  assert.ok(monthlyAssetTrendContentSource.includes('const CHART_WIDTH = 370;') && monthlyAssetTrendContentSource.includes('grid-cols-[90px_minmax(74px,1fr)_minmax(72px,0.96fr)_52px_11px]'), 'asset trend chart and monthly grid should use the wider standalone-page content geometry');
  assert.ok(monthlyAssetTrendContentSource.includes('Object.freeze({ left: 18, right: 392, top: 22, bottom: 169 })') && monthlyAssetTrendContentSource.includes('x={-CHART_EDGE_LABEL_OFFSET}') && monthlyAssetTrendContentSource.includes('const labelX = last ? CHART_WIDTH + CHART_EDGE_LABEL_OFFSET'), 'chart plot and axis labels should expand together to the current asset amount and comparison-arrow boundaries');
