...
test('asset and review module cards do not keep legacy scale interactions', () => {
  ...
  assert.ok(settingsTabSource.includes('v10.7.9.151'), 'settings version badge should document the latest fear card compaction update');
...
test('review target page uses dark mobile cards and click action modals', () => {
  ...
  assert.ok(homeTabSource.includes('function describeFgiArc(cx, cy, radius, startValue, endValue)'), 'CNN gauge should keep an explicit SVG arc generator');
  assert.ok(homeTabSource.includes('viewBox="0 0 160 70" className="h-[54px] w-full overflow-visible"'), 'CNN gauge should stay shorter than the old 90px SVG');
