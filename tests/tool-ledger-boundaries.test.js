...
test('main trade entry modal uses compact four-step buy sell submission flow', () => {
  ...
  assert.ok(settingsTabSource.includes('v10.7.9.272'), 'settings version badge should document the latest trade positions spacing rebalance update');
  assert.ok(settingsChangelogSource.includes('v10.7.9.272'), 'settings changelog should document the latest trade positions spacing rebalance update');
  assert.ok(settingsChangelogSource.includes('持仓列距再平衡'), 'settings changelog should describe the trade positions daily P&L spacing update');
  ...
  assert.ok(earningsCalendarSource.includes('text-[11px] leading-none font-normal text-white/82'), 'earnings calendar homepage symbol should stay one step smaller than before');
  ...
  assert.ok(tradesTabSource.includes('min-w-[552px]'), 'trade positions table should keep balanced horizontal room while preserving first-screen daily P&L');
  assert.ok(tradesTabSource.includes('grid-cols-[76px_88px_72px_104px_8px_144px_60px]'), 'trade positions table should share one rebalanced buffered column template across header and rows');
  assert.ok(tradesTabSource.includes('<span aria-hidden="true" />'), 'trade positions table should insert a buffer column before holding P&L so it does not leak into the first screen');
