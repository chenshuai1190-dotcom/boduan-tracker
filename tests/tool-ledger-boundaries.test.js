...
test('main trade entry modal uses compact four-step buy sell submission flow', () => {
  ...
  assert.ok(settingsTabSource.includes('v10.7.9.210'), 'settings version badge should document the locked close price display update');
  assert.ok(settingsChangelogSource.includes('v10.7.9.210'), 'settings changelog should document the locked close price display update');
  assert.ok(settingsChangelogSource.includes('收盘锁定价格显示'), 'settings changelog should describe the locked close price display update');
...
test('realtime quote refresh avoids duplicate requests and hides raw Safari network errors', () => {
  ...
  assert.ok(appSource.includes("mergeIndexRestCardsIntoMarketCards(current, indicesData.data, 'fallback')"), 'INDICES REST data should seed local index chart samples before realtime ticks');
