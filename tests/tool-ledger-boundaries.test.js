...
test('main trade entry modal uses compact four-step buy sell submission flow', () => {
  ...
  assert.ok(settingsTabSource.includes('v10.7.9.208'), 'settings version badge should document the EODHD-only index card update');
  assert.ok(settingsChangelogSource.includes('v10.7.9.208'), 'settings changelog should document the EODHD-only index card update');
  assert.ok(settingsChangelogSource.includes('三大指数去 Yahoo 图源'), 'settings changelog should describe the EODHD-only index card update');
...
test('realtime quote refresh avoids duplicate requests and hides raw Safari network errors', () => {
  ...
  assert.ok(appSource.includes('setMarketIndices(mergeFreshIndexTicksIntoCards(indicesData.data))'), 'INDICES REST data should update only index cards');
  ...
  assert.ok(homeTabSource.includes('const indexCards = (resolvedIndexCards || []).slice(0, 3);'), 'home market row should reserve exactly three index cards before BTC');
