...
test('approved modal families share the new shell without widening business boundaries', () => {
  ...
  assert.ok(assetDialogsBlock.includes('contentClassName="!min-h-0 !border-0 !bg-transparent !p-0 !shadow-none"'), 'asset month trend should remove the redundant full-content inner frame');
  assert.ok(assetDialogsBlock.includes('<MonthlyAssetTrendContent') && assetDialogsBlock.includes('onEditMonth={openMonthlyBalanceEditor}'), 'asset month trend should use the approved analysis content and exact-month edit handoff');
  assert.ok(assetDialogsBlock.includes('!bg-none !bg-[#0b0f14]'), 'asset month trend should clear the shared blue-gray gradient and use the system asset black');
  assert.ok(assetDialogsBlock.includes('comparisonStartMonth={yearAgo}') && assetDialogsBlock.includes('comparisonStartValue={totalYearAgo}'), '12-month asset change should use the exact year-ago endpoint while monthly details stay at twelve rows');
