...
test('main trade entry modal uses compact four-step buy sell submission flow', () => {
  ...
  assert.ok(settingsTabSource.includes('v10.7.9.212'), 'settings version badge should document the BTC sparkline visual dampening update');
  assert.ok(settingsChangelogSource.includes('v10.7.9.212'), 'settings changelog should document the BTC sparkline visual dampening update');
  assert.ok(settingsChangelogSource.includes('BTC 小曲线视觉降噪'), 'settings changelog should describe the BTC sparkline visual dampening update');
  assert.ok(settingsChangelogSource.includes('v10.7.9.211'), 'settings changelog should retain the index session chart update');
  assert.ok(settingsChangelogSource.includes('三大指数分时曲线锁定'), 'settings changelog should retain the index session chart update');
...
test('realtime quote refresh avoids duplicate requests and hides raw Safari network errors', () => {
  ...
  assert.ok(homeTabSource.includes('BTC_SPARKLINE_MIN_RANGE_PCT = 0.006'), 'BTC market card should dampen tiny sparkline moves without changing live price updates');
  assert.ok(homeTabSource.includes("function Sparkline({ values = [], color = '#22c55e', className = 'h-9', minRangePct = 0 })"), 'market sparkline should support an optional visual minimum range');
  assert.ok(homeTabSource.includes('minRangePct={isBtc ? BTC_SPARKLINE_MIN_RANGE_PCT : 0}'), 'only the BTC market card should use the dampened sparkline visual range');
