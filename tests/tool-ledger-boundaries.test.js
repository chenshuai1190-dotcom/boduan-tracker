...
test('community competition is an isolated authenticated close-snapshot utility', () => {
  ...
  assert.ok(communityCompetitionPageSource.includes("tt('competition.snapshotAsOf', '更新 {{dateTime}}'") && communityCompetitionPageSource.includes('formatCompactSnapshotTime(data?.snapshotUpdatedAt)'), 'snapshot update copy should use the authoritative lock timestamp with compact month-day and minute precision');
  assert.ok(communityCompetitionPageSource.includes("timeZone: 'America/New_York'") && communityCompetitionPageSource.includes("hourCycle: 'h23'"), 'snapshot update time should stay fixed to New York 24-hour time instead of the customer device time zone');
  ...
  assert.ok(i18nSource.includes("'competition.snapshotAsOf': '更新 {{dateTime}}'") && i18nSource.includes("'competition.snapshotAsOf': 'Updated {{dateTime}}'"), 'compact snapshot minute copy should remain bilingual');
...
test('main trade entry modal uses compact four-step buy sell submission flow', () => {
  ...
  assert.ok(settingsTabSource.includes("const SETTINGS_VERSION = 'v10.7.9.349'"), 'visible settings version surfaces should share one source');
  assert.ok(settingsChangelogSource.includes("ver: 'v10.7.9.349', date: '2026-07-16', latest: true"), 'latest changelog entry should match the visible settings version');
  assert.ok(settingsChangelogSource.includes('收益比赛 PWA 等待状态恢复') && settingsChangelogSource.includes('比赛缓存升级到 v5') && settingsChangelogSource.includes('不修改收益、排名、快照、交易、参赛资格或发布标记'), 'settings changelog should document the bounded waiting-cache recovery and unchanged authoritative data');
...
test('review target page uses dark mobile cards and click action modals', () => {
  ...
  assert.ok(tradesTabSource.includes('fmtAmount(marketValue, 2)'), 'trade position market value should keep two decimal places like daily and holding pnl');
  assert.ok(settingsTabSource.includes("const SETTINGS_VERSION = 'v10.7.9.349'"), 'settings version surfaces should remain synchronized through the shared constant');
  assert.ok(settingsChangelogSource.includes('v10.7.9.218'), 'settings changelog should document the P&L report period stats update');
  assert.ok(settingsChangelogSource.includes('收益报表周期统计'), 'settings changelog should describe the P&L report period stats update');
  assert.ok(settingsChangelogSource.includes('v10.7.9.217'), 'settings changelog should document the P&L calendar visual update');
  assert.ok(settingsChangelogSource.includes('收益日历视觉优化'), 'settings changelog should describe the P&L calendar visual update');
