...
test('asset header card aligns with home and trade header sizing', () => {
  ...
  assert.equal((homeTabSource.match(/rounded-xl border border-transparent bg-\[#101114\]/g) || []).length, 2, 'both market-card states should hide their outer outlines while retaining the raised neutral-black surface');
  ...
  assert.ok(tradesTabSource.includes('grid-cols-[1fr_1.12fr_0.96fr] border-t border-white/[0.07] pt-4'), 'the Trading header should preserve its horizontal divider above today PnL');
...
test('review target page uses dark mobile cards and click action modals', () => {
  ...
  assert.ok(homeTabSource.includes('<span className="block text-[15px] font-normal tracking-[0.01em] text-white/85">{row.symbol}</span>'), 'benchmark menu symbol should use the approved 15px normal weight without a duplicated display name');
