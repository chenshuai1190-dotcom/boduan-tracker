...
test('asset and review module cards do not keep legacy scale interactions', () => {
  ...
  assert.ok(settingsTabSource.includes('v10.7.9.153'), 'settings version badge should document the latest fear card text and endpoint update');
...
test('review target page uses dark mobile cards and click action modals', () => {
  ...
  assert.ok(homeTabSource.includes('const radiusY = 40'), 'CNN gauge should use a flatter elliptical arc');
  assert.ok(homeTabSource.includes('viewBox="0 0 160 72" className="h-[54px] w-full overflow-visible"'), 'CNN gauge should stay short while leaving label headroom');
  ...
  assert.ok(homeTabSource.includes('<text x="27" y="67" fill="#7f8794" fontSize="9" textAnchor="middle">0</text>'), 'CNN left endpoint label should sit near the arc start');
  assert.ok(homeTabSource.includes('<text x="133" y="67" fill="#7f8794" fontSize="9" textAnchor="middle">100</text>'), 'CNN right endpoint label should sit near the arc end');
  ...
  assert.ok(settingsTabSource.includes('v10.7.9.153'), 'settings version badge should document the latest fear card text and endpoint update');
  assert.ok(settingsChangelogSource.includes('v10.7.9.153'), 'settings changelog should document the latest fear card text and endpoint update');
  assert.ok(settingsChangelogSource.includes('恐慌小卡文字和端点微调'), 'settings changelog should describe the fear card text and endpoint update');
