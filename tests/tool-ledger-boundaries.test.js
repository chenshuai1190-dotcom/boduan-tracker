...
test('main trade entry modal uses compact four-step buy sell submission flow', () => {
  ...
  assert.ok(settingsTabSource.includes('v10.7.9.220'), 'settings version badge should document the P&L report date filter update');
  assert.ok(settingsChangelogSource.includes('v10.7.9.220'), 'settings changelog should document the P&L report date filter update');
  assert.ok(settingsChangelogSource.includes('收益报表日期筛选'), 'settings changelog should describe the P&L report date filter update');
