...
test('main trade entry modal uses compact four-step buy sell submission flow', () => {
  ...
  assert.ok(settingsTabSource.includes('v10.7.9.207'), 'settings version badge should document the BTC card placeholder update');
  assert.ok(settingsChangelogSource.includes('v10.7.9.207'), 'settings changelog should document the BTC card placeholder update');
...
test('realtime quote refresh avoids duplicate requests and hides raw Safari network errors', () => {
  ...
  assert.ok(indicesProviderSource.includes("source: 'Yahoo'"), 'INDICES quote provider should prefer Yahoo chart current prices for index cards');
