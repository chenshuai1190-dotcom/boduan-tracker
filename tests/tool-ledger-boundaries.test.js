...
test('main trade entry modal uses compact four-step buy sell submission flow', () => {
  ...
  assert.ok(settingsTabSource.includes('v10.7.9.255'), 'settings version badge should document the latest published earnings calendar comparison update');
  assert.ok(settingsChangelogSource.includes('v10.7.9.255'), 'settings changelog should document the latest published earnings calendar comparison update');
  assert.ok(settingsChangelogSource.includes('财报日历券商式对比口径'), 'settings changelog should describe the broker-style earnings comparison update');
  ...
  assert.ok(earningsCalendarSource.includes('grid-cols-[minmax(78px,1fr)_52px_92px_38px]'), 'earnings calendar modal row should reserve more room for EPS and USD revenue text');
  assert.ok(earningsCalendarSource.includes('whitespace-nowrap text-[11px] text-white/80 tabular-nums'), 'earnings calendar modal revenue should stay on one line');
...
test('asset and review module cards do not keep legacy scale interactions', () => {
  ...
  assert.ok(settingsTabSource.includes('v10.7.9.255'), 'settings version badge should document the latest homepage earnings calendar update');
  assert.ok(settingsChangelogSource.includes('v10.7.9.218'), 'settings changelog should document the P&L report period stats update');
  assert.ok(settingsChangelogSource.includes('收益报表周期统计'), 'settings changelog should describe the P&L report period stats update');
  assert.ok(settingsChangelogSource.includes('v10.7.9.217'), 'settings changelog should document the P&L calendar visual update');
  assert.ok(settingsChangelogSource.includes('收益日历视觉优化'), 'settings changelog should describe the P&L calendar visual update');
