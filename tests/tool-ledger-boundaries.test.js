...
test('asset header card aligns with home and trade header sizing', () => {
  ...
  assert.ok(analysisTabSource.includes('mt-3 grid grid-cols-3 border-t border-white/10 pt-3 text-center'), 'the asset trend range summary should preserve its horizontal divider');
...
test('approved modal families share the new shell without widening business boundaries', () => {
  ...
  assert.ok(monthlyAssetTrendContentSource.includes('const CHART_WIDTH = 370;') && monthlyAssetTrendContentSource.includes('grid-cols-[90px_minmax(70px,1fr)_minmax(68px,0.96fr)_49px_18px]'), 'asset trend chart and monthly grid should use the wider standalone-page content geometry');
  assert.ok(monthlyAssetTrendContentSource.includes('Object.freeze({ left: 48, right: 362, top: 22, bottom: 169 })') && monthlyAssetTrendContentSource.includes('className="mt-2 aspect-[370/206] select-none touch-pan-y"') && monthlyAssetTrendContentSource.includes('<text x="0"') && monthlyAssetTrendContentSource.includes('const labelX = last ? CHART_WIDTH'), 'chart plot, points, and axis labels should stay inside one responsive viewBox aligned with the cards below');
  assert.equal(monthlyAssetTrendContentSource.includes('CHART_EDGE_LABEL_OFFSET'), false, 'chart labels should not overflow the shared page edge through fixed offsets');
  assert.equal(monthlyAssetTrendContentSource.includes("analysis.highestAssetShort"), false, 'asset trend chart should not show highest or current-value callouts before interaction');
  assert.ok(monthlyAssetTrendContentSource.includes('{selectedPoint && (') && monthlyAssetTrendContentSource.includes('selectedPoint.month'), 'tapping any chart point, including the current month, should reveal the selected month value');
  ...
  assert.equal(monthlyAssetTrendContentSource.includes('#75a7ff') || monthlyAssetTrendContentSource.includes('#111a29'), false, 'asset month trend should not reintroduce blue latest-point accents');
