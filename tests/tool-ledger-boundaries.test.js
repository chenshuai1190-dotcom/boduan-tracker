...
const waveTrackerPageCss = readFileSync(new URL('../src/pages/WaveTrackerPage.css', import.meta.url), 'utf8');
const waveTrackerDialogsCss = readFileSync(new URL('../src/pages/WaveTrackerDialogs.css', import.meta.url), 'utf8');
const communityCompetitionPageSource = readFileSync(new URL('../src/pages/CommunityCompetitionPage.jsx', import.meta.url), 'utf8');
const communityCompetitionPageCss = readFileSync(new URL('../src/pages/CommunityCompetitionPage.css', import.meta.url), 'utf8');
...
const pnlReportPageCss = readFileSync(new URL('../src/pages/PnlReportPage.css', import.meta.url), 'utf8');
const pnlReportTrendChartSource = readFileSync(new URL('../src/components/PnlReportTrendChart.jsx', import.meta.url), 'utf8');
const pnlReportChartModelSource = readFileSync(new URL('../src/lib/pnlReportChart.js', import.meta.url), 'utf8');
const pnlReportFiltersSource = readFileSync(new URL('../src/components/PnlReportFilters.jsx', import.meta.url), 'utf8');
const pnlReportFiltersCss = readFileSync(new URL('../src/components/PnlReportFilters.css', import.meta.url), 'utf8');
const stockDetailPageSource = readFileSync(new URL('../src/pages/StockDetailPage.jsx', import.meta.url), 'utf8');
const stockDetailPageCss = readFileSync(new URL('../src/pages/StockDetailPage.css', import.meta.url), 'utf8');
const stockDetailPnlChartCss = readFileSync(new URL('../src/pages/StockDetailPnlChart.css', import.meta.url), 'utf8');
...
const reviewTabCss = readFileSync(new URL('../src/tabs/ReviewTab.css', import.meta.url), 'utf8');
const compoundDetailSource = readFileSync(new URL('../src/pages/CompoundDetailPage.jsx', import.meta.url), 'utf8');
const compoundDetailCss = readFileSync(new URL('../src/pages/CompoundDetailPage.css', import.meta.url), 'utf8');
const compoundPathModelSource = readFileSync(new URL('../src/lib/compoundPathModel.js', import.meta.url), 'utf8');
const annualGoalPlanSource = readFileSync(new URL('../src/components/AnnualGoalPlan.jsx', import.meta.url), 'utf8');
const annualGoalPlanCss = readFileSync(new URL('../src/components/AnnualGoalPlan.css', import.meta.url), 'utf8');
const northStarGoalCardSource = readFileSync(new URL('../src/components/NorthStarGoalCard.jsx', import.meta.url), 'utf8');
const northStarGoalCardCss = readFileSync(new URL('../src/components/NorthStarGoalCard.css', import.meta.url), 'utf8');
const reviewGoalModalSource = readFileSync(new URL('../src/components/ReviewGoalModal.jsx', import.meta.url), 'utf8');
const reviewEntryEditorsSource = readFileSync(new URL('../src/components/ReviewEntryEditors.jsx', import.meta.url), 'utf8');
...
test('community competition is an isolated authenticated close-snapshot utility', () => {
  ...
  assert.ok(communityCompetitionPageSource.includes('cc-join-icon') && communityCompetitionPageSource.includes('<Trophy'), 'the voluntary join sheet should retain its scoped competition icon without restoring the old decorative gold hero');
  ...
  assert.ok(communityCompetitionPageSource.includes('cc-self-rank') && communityCompetitionPageSource.includes("tt('competition.myRank', '我的排名')"), 'the report hero should retain the authoritative self rank');
  assert.ok(communityCompetitionPageSource.includes('data-competition-self-avatar'), 'the report hero should retain the signed-in user avatar');
  assert.ok(communityCompetitionPageSource.includes('data-competition-hero-metrics'), 'the self return, QQQ benchmark, and outperformance should remain grouped in the report hero');
  ...
  assert.ok(communityCompetitionPageSource.includes('data-competition-self-nickname') && communityCompetitionPageSource.includes("{self?.nickname || '--'}"), 'the self profile should show the authoritative community nickname, with missing data distinct from a real nickname');
  assert.ok(communityCompetitionPageSource.includes('cc-self-nickname'), 'the self nickname should use the scoped report layout rather than an avatar-centered negative margin');
  assert.ok(communityCompetitionPageSource.includes('cc-content') && communityCompetitionPageSource.includes('cc-body'), 'competition content should use scoped sections without nested card gutters');
  assert.match(communityCompetitionPageCss, /\.cc-page\s*\{[^}]*max-width:\s*792px;/, 'the full-bleed competition shell should provide 760px of content plus two 16px gutters');
  assert.match(communityCompetitionPageCss, /\.cc-page\s*\{[^}]*padding[^;]*16px/, 'the competition page should own exactly one horizontal gutter');
  assert.match(communityCompetitionPageCss, /\.cc-header\s*\{[^}]*env\(safe-area-inset-top\)/, 'the standalone competition header must account for the iPhone top safe area because its outer shell has no top padding');
  assert.match(communityCompetitionPageCss, /\.cc-page\s*\{[^}]*background:\s*#08090b;/, 'competition should share the neutral continuous report background');
  ...
  assert.ok(communityCompetitionPageSource.includes('cc-hero'), 'self metrics should use the continuous report hero instead of the old nested card');
  assert.ok(communityCompetitionPageSource.includes('data-competition-update-row') && communityCompetitionPageSource.includes('data-competition-update-date'), 'the actual close date should keep its dedicated accessible report metadata hooks');
  ...
  assert.ok(communityCompetitionPageSource.includes('data-rank-avatar'), 'ranking avatars should preserve their shared neutral avatar surface and popover anchor');
  ...
  assert.match(communityCompetitionPageCss, /\.cc-user-card\s*\{[^}]*width:\s*320px;[^}]*max-width:\s*calc\(100vw\s*-\s*20px\);/, 'static user card should be wide enough for multiple holding symbols without exceeding the viewport');
  ...
  for (const selector of ['\\.cc-join-description', '\\.cc-status p', '\\.cc-disclosure p']) {
    assert.match(communityCompetitionPageCss, new RegExp(`${selector}\\s*\\{[^}]*text-wrap:\\s*pretty;`), 'competition hint copy should keep natural wrapping on join, waiting/error, and source-disclosure surfaces');
  }
  ...
  assert.match(communityCompetitionPageCss, /\.cc-user-card\s*\{[^}]*background:\s*#101112;/, 'static user cards should match the neutral report dialog surface instead of a decorative gradient frame');
  assert.ok(communityCompetitionPageSource.includes('cc-rank-row') && communityCompetitionPageSource.includes('cc-rank-self') && communityCompetitionPageSource.includes('cc-rank-selected'), 'ranking rows should preserve distinct self and selected states in their scoped report layout');
...
test('main trade entry modal isolates the compact stacked formal-trade design', () => {
  ...
  assert.equal((homeWatchlistSource.match(/className="hwr-icon-button"/g) || []).length, 2, 'Home add and edit actions should share the compact neutral heading style');
  assert.match(homeWatchlistCss, /\.hwr-icon-button\s*\{[^}]*border:\s*0;[^}]*background:\s*#161719;/, 'paired watchlist actions should remain borderless and neutral');
  assert.ok(homeTabSource.includes('onAdd={() => setShowAddStock(true)}') && homeTabSource.includes('onEdit={() => setShowEditWatchlist(true)}'), 'compact actions must retain their existing local dialog callbacks');
  ...
  assert.ok(tradesTabSource.includes('className="trades-report-add"'), 'explicit add-trade entries should use one report action style');
  assert.match(tradesTabCss, /\.trades-report-add\s*\{[^}]*background:\s*#17181c;[^}]*font-size:\s*13px;/, 'add-trade entries should keep a neutral background and readable text');
  ...
  assert.ok(settingsTabSource.includes("import { SETTINGS_VERSION } from '../lib/releaseMeta.js'"), 'visible settings version surfaces should use the shared release metadata');
  ...
  assert.match(communityCompetitionPageCss, /\.cc-title-row h1\s*\{[^}]*font-size:\s*18px;[^}]*font-weight:\s*400;/, 'competition title should retain the shared 18px normal-weight page typography in its scoped report header');
  assert.match(homeTabSource, /<EarningsCalendar\s+visualVariant="report"/, 'Home should use the minimal earnings report');
  assert.ok(homeTabSource.indexOf('<EarningsCalendar') > homeTabSource.indexOf('<HomeWatchlistReport'), 'the earnings report should have a stable position after watchlist');
  assert.equal(homeTabSource.includes('onPromotionChange='), false, 'the Home report should not jump position when earnings arrive');
  ...
  assert.ok(tradesTabSource.includes('<TradesPositionsReport rows={positionReportRows}'), 'the prepared position rows should render through the dedicated report component');
  assert.ok(tradesPositionsSource.includes('data-trade-positions-table="horizontal"'), 'holdings must keep the user requested horizontal table');
  assert.ok(tradesPositionsSource.includes('data-trades-positions-report="true"'), 'the responsive positions report should retain a stable test marker');
  assert.match(tradesPositionsCss, /\.trades-positions-report\s*\{[^}]*overflow-x:\s*auto;/, 'only the holdings region should scroll horizontally');
  assert.match(tradesPositionsCss, /\.tpr-value\s*\{[^}]*white-space:\s*nowrap;/, 'financial amounts remain single line inside their full width columns');
  assert.ok(tradesTabSource.includes('onScenario={(row) => openPositionScenario(row.position)}'), 'the explicit scenario action must use the original raw position');
  ...
  const reportRowOpeningTag = tradesPositionsSource.match(/<tr className="tpr-position"[^>]*>/)?.[0];
  assert.ok(reportRowOpeningTag && !reportRowOpeningTag.includes('onClick'), 'a holding report row must not carry an implicit trade click that can be triggered by scenario actions');
  assert.ok(tradesPositionsSource.includes('onClick={() => onScenario?.(row)}') && tradesPositionsSource.includes('onClick={() => onTrade?.(row)}'), 'scenario and record-trade buttons must have separate explicit callbacks');
  ...
  assert.match(tradesTabSource, /import ['"]\.\.\/components\/PulseDot\.css['"]/, 'holding return simulator should use the shared breathing halo');
  assert.doesNotMatch(tradesTabSource, /@keyframes scenario-marker-breathe|animation:\s*scenario-marker-breathe/, 'the simulator must not override the shared 2.4-second motion');
  ...
  assert.match(tradesTabSource, /className="(?=[^"]*\bpps-current-marker\b)(?=[^"]*\bquote-pulse-dot\b)[^"]*"/, 'holding return simulator should use its neutral current-price marker with a shared hollow halo');
  ...
  const currentMarkerCss = readFileSync(new URL('../src/tabs/PositionProfitScenario.css', import.meta.url), 'utf8');
  assert.match(currentMarkerCss, /\.pps-current-marker\s*\{[^}]*background:\s*#d2d4d7;[^}]*color:\s*#d2d4d7;/, 'holding return simulator should keep the core and shared halo neutral rather than imply an investment direction');
  assert.doesNotMatch(tradesTabSource, /markerGlowRgb|--scenario-marker-glow/, 'the replaced glow must not leave an independent color or animation path');
  ...
  assert.ok(tradesTabSource.includes('style={{ left: currentPositionItem.left }}'), 'holding return simulator should place the breathing marker on the current-price point');
  ...
  assert.ok(pnlReportPageSource.includes('pnl-report-calendar-toggle pnl-report-calendar-value-toggle'), 'P&L report calendar P&L/rate switch should keep its dedicated two-column control');
  assert.match(pnlReportPageCss, /\.pnl-report-calendar-toggle\s*\{[^}]*grid-template-columns:\s*repeat\(2, minmax\(0, 1fr\)\)/, 'calendar toggle columns should keep equal space');
  assert.match(pnlReportPageCss, /\.pnl-report-calendar-segment\[aria-pressed="true"\]\s*\{[^}]*background:\s*#2a2b2f;/, 'calendar selected controls should share the neutral-gray report palette');
  ...
  assert.equal(pnlReportPageSource.includes('Quote Data testing'), false, 'P&L report header should remove the obsolete testing badge from the DOM');
  ...
  assert.ok(pnlReportPageSource.includes('{currentRangeLabel} · {displayCurrency}'), 'P&L report contribution heading should identify the selected range and currency');
  assert.ok(pnlReportPageSource.includes('<PnlReportDateFilter') && pnlReportPageSource.includes('<PnlReportCalendarPicker'), 'P&L filters should remain wired through the page-owned draft and confirmation state');
  assert.match(pnlReportFiltersCss, /\.pnl-report-filter-input\s*\{[^}]*height:\s*46px;[^}]*line-height:\s*44px;/, 'P&L date filters should vertically center dates in the standard report control');
  ...
  assert.doesNotMatch(pnlReportPageCss.match(/\.pnl-report-rank-row\s*\{([^}]+)\}/)?.[1] || '', /background|box-shadow/, 'ranking rows should not restore decorative colored bars');
  ...
  assert.ok(homeTabSource.includes("formatPrice={(value) => hasFiniteMarketValue(value) && Number(value) > 0 ? fmtMoney(value, 2) : '--'}"), 'Home report formatter should only accept a finite positive resolved price');
  assert.ok(homeWatchlistSource.includes("missing(item.price) || Number(item.price) <= 0 ? '—' : formatted(formatPrice, item.price)"), 'watchlist and holdings must render the prepared price and keep unavailable quotes missing');
  ...
  assert.ok(tradesTabSource.includes('onScenario={(row) => openPositionScenario(row.position)}') && tradesTabSource.includes('position, symbol: position.symbol'), 'the holding scenario tool must retain the raw position and EODHD current price, not the formatted close-locked display');
...
test('P&L report snapshot page stays independent from live trading pipelines', () => {
  ...
  assert.ok(pnlReportTrendChartSource.includes('data-pnl-report-compare-tooltip'), 'P&L report chart should expose a unified comparison readout');
  assert.ok(pnlReportTrendChartSource.includes('pnlReport.tooltip.daily'), 'P&L report comparison readout should show daily return');
  assert.ok(pnlReportTrendChartSource.includes('pnlReport.tooltip.cumulative'), 'P&L report comparison readout should show cumulative return');
  assert.ok(pnlReportTrendChartSource.includes('marketTextClass(readoutSlot.point?.dailyPnlPct, marketColorMode)'), 'P&L report readout mine daily return should follow the configured market color mode');
  ...
  assert.ok(pnlReportTrendChartSource.includes('pnlReport.tooltip.availableCash'), 'the existing asset readout should disclose available cash inside the asset view');
...
test('stock detail page is read-only and separate from trade editing', () => {
  ...
  assert.match(stockDetailPageSource, /import ['"]\.\.\/components\/PulseDot\.css['"]/, 'stock detail peak halo should share the same motion and reduced-motion settings');
  assert.ok(stockDetailPageSource.includes('stock-detail-peak-breathe-ring quote-pulse-halo'), 'stock detail chart should render a separate shared halo for the peak point');
  assert.doesNotMatch(stockDetailPageSource, /@keyframes stock-detail-peak-breathe|animation:\s*stock-detail-peak-breathe/, 'stock detail must not override the shared 2.4-second motion');
  assert.match(stockDetailPageSource, /className="stock-detail-peak-breathe-ring quote-pulse-halo"\s+cx=\{chart\.peakPoint\.x\}\s+cy=\{chart\.peakPoint\.y\}\s+r="7"\s+fill="none"\s+stroke=\{peakPointColor\}\s+strokeWidth="1\.2"\s+vectorEffect="non-scaling-stroke"/, 'the hollow peak ring must keep its existing coordinates, radius and market color');
  ...
  assert.ok(stockDetailPageSource.includes('r="3.6" fill={peakPointColor} stroke="#08090b" strokeWidth="1.4"'), 'stock detail peak dot should keep its size while adopting the neutral report surface and semantic market color');
  assert.ok(stockDetailPageSource.includes('marketHexColor(chart.peakPoint.value, marketColorMode)'), 'stock detail peak dot color should describe the peak value rather than use decorative gold');
  ...
  assert.ok(stockDetailPageSource.includes('stroke={color} strokeWidth="1.8"'), 'the expanded report chart should retain a readable semantic P&L line');
  assert.match(stockDetailPageSource, /tooltipWidth:\s*276,/);
  assert.match(stockDetailPageSource, /Math\.min\(PNL_CHART_GEOMETRY\.tooltipWidth, chart\.width - PNL_CHART_GEOMETRY\.tooltipGutter \* 2\)/, 'tooltip width must fit narrow viewports instead of overflowing');
  assert.ok(stockDetailPageSource.includes('chart.width - tooltipWidth - PNL_CHART_GEOMETRY.tooltipGutter'), 'tooltip horizontal bounds must use the same responsive chart width');
  assert.ok(stockDetailPageSource.includes('chart.height - PNL_CHART_GEOMETRY.tooltipHeight - PNL_CHART_GEOMETRY.tooltipGutter'), 'tooltip vertical bounds must use the chart height');
  assert.match(stockDetailPnlChartCss, /\.stock-detail-pnl-tooltip-readings\s*\{[^}]*grid-template-columns:\s*max-content minmax\(0, 1fr\);/, 'tooltip labels and financial values should each stay on one line');
  assert.match(stockDetailPnlChartCss, /\.stock-detail-pnl-tooltip-value\s*\{[^}]*white-space:\s*nowrap;/);
  assert.match(stockDetailPageSource, /preserveAspectRatio="none"/, 'responsive plot and pointer coordinates must not diverge through SVG letterboxing');
  assert.match(stockDetailPageSource, /const observer = typeof ResizeObserver === 'function' \? new ResizeObserver\(updateWidth\) : null/);
  assert.match(stockDetailPageSource, /observer\?\.disconnect\(\)/, 'chart resize tracking must be cleaned up');
...
test('position clicks default to buy and trade records use ledger edit/delete flow', () => {
  ...
  assert.ok(tradesTabSource.includes("onTrade={(row) => openTradeModal(row.position, 'buy')}"), 'the explicit record-trade action should preserve the original position and default buy mode');
...
test('language framework covers settings switch, bottom nav, home page, and stock ticker display', () => {
  ...
  assert.ok(homeTabSource.includes('home-report mx-auto max-w-[430px]'), 'Home should retain its bounded report width');
  ...
  assert.ok(northStarGoalCardSource.includes("tt('review.polarisGoal', '北极星目标')"), 'Review north-star card should read from i18n');
  assert.ok(reviewTabSource.includes('language={language}'), 'Review should pass its current language to the north-star presentation');
  assert.ok(annualGoalPlanSource.includes("tt('review.annualPlan', '年度计划')"), 'Review annual plan title should read from i18n');
  assert.ok(reviewEntryEditorsSource.includes("function DisciplineModal({ initial, language = 'zh', onCancel, onSave, onDelete })"), 'Review discipline modal should accept the current language');
  ...
  assert.ok(homeWatchlistSource.includes('className="hwr-metric-controls"') && homeWatchlistSource.includes('aria-pressed={auxiliaryMetric === metric}'), 'the responsive report should expose additional metrics through explicit controls');
  assert.ok(homeWatchlistCss.includes('@media (max-width: 359px)') && homeWatchlistCss.includes('minmax(66px, max-content) minmax(62px, max-content)'), 'narrow bilingual rows must retain readable number columns without a wide scrolling table');
  ...
  assert.ok(devVisualPreviewSource.includes("import YearlyActualModal from './components/YearlyActualModal.jsx'") && yearlyActualModalSource.includes("tt('review.actualDataTitle'"), 'local visual preview should reuse the real translated annual editor rather than a placeholder');
});
...
test('asset module redesign keeps database logic while removing legacy controls', () => {
  ...
  assert.match(analysisTabCss, /\.asset-report-owner-total\s*\{[^}]*color:\s*#ff4b1f;/, 'owner asset totals should retain the requested red emphasis');
...
test('asset page visual shell and local preview stay debuggable', () => {
  ...
  const finishOverviewPointerStart = analysisTabSource.indexOf('const finishOverviewPointerTracking =');
  const finishOverviewPointerEnd = analysisTabSource.indexOf('const selectedTrendAccount =', finishOverviewPointerStart);
  const finishOverviewPointerBlock = analysisTabSource.slice(finishOverviewPointerStart, finishOverviewPointerEnd);
  ...
  assert.ok(monthlyAssetTrendChartSource.includes('monthlyAssetLineDraw'), 'asset chart should keep the line drawing animation');
  assert.ok(monthlyAssetTrendChartSource.includes('monthlyAssetAreaFade'), 'asset chart area should keep the fade-in animation');
  assert.ok(monthlyAssetTrendChartSource.includes('monthlyAssetPointPop'), 'asset chart points should keep the pop animation');
  ...
  assert.ok(analysisTabSource.includes('selectedChartChangePct'), 'asset chart reading should include the month-over-month percentage');
  assert.ok(analysisTabSource.includes('const overviewChartLatestIndex = overviewChartModel.points.at(-1)?.index ?? null'), 'asset chart should default the visible marker to the latest valid month');
  assert.ok(analysisTabSource.includes('selectedIndex={chartSelectedMonthIdx}') && analysisTabSource.includes('latestPointIndex={overviewChartLatestIndex}'), 'asset chart should separate the clicked detail state from the latest-point marker');
  assert.ok(analysisTabSource.includes('latestPointDelayMs={780}'), 'asset chart should let the default final marker appear after the line draw animation');
  assert.ok(analysisTabSource.includes('const requestedChartSlot = overviewChartModel.slots[chartSelectedMonthIdx]') && analysisTabSource.includes('const overviewChartReading = chartSelectedMonthIdx !== null && requestedChartSlot?.hasData') && !analysisTabSource.includes('requestedChartSlot.month !== currentMonth') && !analysisTabSource.includes('overviewChartModel.slots[overviewChartLatestIndex] || null'), 'any explicitly selected valid month, including the latest month, should show a reading while the unselected chart remains free of duplicate totals');
  assert.ok(analysisTabSource.includes("splitCurrencyAmount(selectedChartValue, 'CNY', 2)") && analysisTabSource.includes("{ main: '--', decimal: '' }") && analysisTabSource.includes('Number.isFinite(selectedChartValue)'), 'the chart amount should keep two-decimal precision and an unavailable value when no valid reading exists');
  assert.ok(analysisTabSource.includes('data-asset-chart-month={selectedChartMonth}') && analysisTabSource.includes('selectedChartMoney.main') && analysisTabSource.includes('selectedChartMoney.decimal') && analysisTabSource.includes("splitCurrencyAmount(Math.abs(selectedChartChange), 'CNY', 2)") && analysisTabSource.includes('selectedChartChangeMoney.decimal'), 'the historical overlay should show the selected month and preserve two-decimal precision in both the amount and split change value');
  assert.ok(analysisTabSource.includes('{overviewChartReading && (') && /\.asset-report-chart-explorer\s*\{[^}]*position:\s*relative;/.test(analysisTabCss) && /\.asset-report-chart-reading\s*\{[^}]*position:\s*absolute;/.test(analysisTabCss), 'the historical reading should be absent from the default DOM and overlay the chart without pushing its layout');
  assert.ok(analysisTabSource.includes('<MonthlyAssetTrendChart') && monthlyAssetTrendContentSource.includes('<MonthlyAssetTrendChart'), 'asset overview and standalone page should share one chart renderer');
  assert.ok(monthlyAssetTrendChartSource.includes('r="13"'), 'asset chart should keep transparent hit targets for every month');
  assert.equal(monthlyAssetTrendChartSource.includes('r={selected ? 5.8 : 4.4}'), false, 'asset chart should not render visible circles for every month');
  ...
  assert.ok(analysisTabSource.includes('connectGaps') && analysisTabSource.includes('showSelectedLabel={false}'), 'asset overview should preserve connected history with its compact historical overlay');
  assert.ok(analysisTabSource.includes('onPointerDown={handleOverviewPointerDown}') && analysisTabSource.includes('onPointerMove={handleOverviewPointerMove}') && analysisTabSource.includes('event.currentTarget.setPointerCapture?.(event.pointerId)'), 'the overview chart should capture a pointer for continuous month selection');
  assert.ok(analysisTabSource.includes('overviewChartModel.points.forEach((point) => {') && analysisTabSource.includes('overviewChartScale.xForIndex(point.index) - viewX') && analysisTabSource.includes('setChartSelectedMonthIdx(nearest.index)'), 'horizontal selection should use the actual chart coordinates and only recorded month points');
  assert.ok(finishOverviewPointerStart > -1 && finishOverviewPointerEnd > finishOverviewPointerStart && finishOverviewPointerBlock.includes('releasePointerCapture') && !finishOverviewPointerBlock.includes('setChartSelectedMonthIdx'), 'finishing a gesture should release the pointer while retaining the selected month');
  assert.ok(analysisTabSource.includes('onPointerUp={finishOverviewPointerTracking}') && analysisTabSource.includes('onPointerCancel={finishOverviewPointerTracking}') && analysisTabSource.includes('onLostPointerCapture={finishOverviewPointerTracking}'), 'every gesture end should use the same selection-preserving cleanup');
  assert.match(analysisTabSource, /ref=\{overviewChartInteractionRef\} className="asset-report-chart-explorer">\s*\{overviewChartReading && \(\s*<div\s+className="asset-report-chart-reading"[\s\S]*?<MonthlyAssetTrendChart/, 'the historical overlay and chart should share the outside-touch interaction boundary');
  assert.ok(analysisTabSource.includes("document.addEventListener('pointerdown', clearSelectedMonthOutsideOverviewChart, true)") && analysisTabSource.includes('if (overviewChartInteractionRef.current?.contains(event.target)) return;'), 'touching outside the chart and overlay should dismiss the historical reading and retain the default latest marker');
  ...
  assert.ok(monthlyAssetTrendChartSource.includes('highlightSelectedMonth = false') && analysisTabSource.includes('highlightSelectedMonth') && !monthlyAssetTrendContentSource.includes('highlightSelectedMonth'), 'selected-month axis emphasis should be opt-in for the overview without changing the standalone chart');
  assert.ok(monthlyAssetTrendChartSource.includes('labelIndexes.add(selectedPoint.index)') && monthlyAssetTrendChartSource.includes('Math.abs(index - selectedPoint.index) < 2'), 'the selected month should remain labelled without colliding with adjacent axis labels');
...
test('production V2 wave tracker is an independent real-data page with isolated mutations', () => {
  ...
  assert.ok(waveTrackerPageSource.includes('className="wave-dialog-form-scroller"') && waveTrackerPageSource.includes("WebkitMinLogicalWidth: '0px'"), 'production modal forms should retain their scoped scroller and iOS width safeguard');
  assert.match(waveTrackerDialogsCss, /\.wave-dialog \.wave-dialog-form-scroller\s*\{[^}]*max-height:\s*56dvh;[^}]*overflow-x:\s*hidden;[^}]*overflow-y:\s*auto;/, 'production modal forms should stay inside keyboard-reduced mobile viewports');
  ...
  assert.ok(waveTrackerPageSource.includes("isDateInput ? 'wave-form-date-input'") && waveTrackerPageSource.includes("lineHeight: '48px'") && waveTrackerPageSource.includes("textAlign: 'center'"), 'wave date inputs should center their native value in the taller report form controls');
  ...
  assert.ok(waveTrackerPageSource.includes('<StockReportModal') && stockReportModalSource.includes('<ActionModalCard') && waveTrackerPageSource.includes("document.body.style.position = 'fixed'") && actionModalCardSource.includes('window.visualViewport'), 'wave forecast input should reuse the shared report modal, locked body and visual viewport safeguards');
  ...
  assert.match(waveTrackerDialogsCss, /\.wave-dialog \.wave-form-date-input::-webkit-date-and-time-value\s*\{[^}]*min-height:\s*48px;[^}]*line-height:\s*48px;/, 'the scoped Safari date value pseudo-element should match the native input line height');
  ...
  assert.match(waveTrackerPageCss, /\.wave-hero-stats\s*\{[^}]*grid-template-columns:\s*repeat\(2, minmax\(0, 1fr\)\);/, 'top metrics should retain shrinkable columns on narrow screens');
  assert.match(waveTrackerPageCss, /\.wave-toolbar\s*\{[^}]*flex-wrap:\s*wrap;/, 'direct filters and the add action should wrap safely on narrow screens');
  assert.ok(waveTrackerPageSource.includes("tt('swing.buyBadge', '买入')") && waveTrackerPageSource.includes('wave-dialog-badge-buy'), 'the add-wave form should retain its buy badge');
  assert.match(waveTrackerDialogsCss, /\.wave-dialog \.wave-dialog-badge\s*\{[^}]*flex-shrink:\s*0;[^}]*white-space:\s*nowrap;/, 'add-wave buy badge should stay horizontal on narrow screens');
  assert.match(waveTrackerPageCss, /\.wave-record:first-child\s*\{[^}]*border-top:\s*0;/, 'the first expanded wave should not draw an extra divider below the stock summary');
...
test('exact account trend stays read-only while the amount keeps edit and delete actions', () => {
  ...
  assert.ok(/const barColor = slot\.hasPreviousMonth && Number\.isFinite\(slot\.changeAmount\) && slot\.changeAmount !== 0\s*\? marketHexColor\(slot\.changeAmount, marketColorMode\)\s*: '#777780';/.test(accountAssetTrendModalSource) && accountAssetTrendModalSource.includes('style={{ height, backgroundColor: barColor }}'), 'account snapshot bars should follow the configured market colors only for a valid nonzero change from the previous month');
  assert.match(accountAssetTrendModalCss, /\.account-trend-bar\s*\{[^}]*background:\s*#777780;/, 'flat or incomparable account months should retain a neutral gray fill');
  assert.match(accountAssetTrendModalCss, /\.account-trend-bar\.is-selected\s*\{[^}]*filter:\s*brightness\(1\.2\);[^}]*outline:\s*1px solid/, 'selection should emphasize the existing bar with brightness and a fine outline');
  assert.doesNotMatch(accountAssetTrendModalCss, /\.account-trend-bar\.is-selected\s*\{[^}]*(?:background(?:-[\w-]+)?|fill):/, 'selection must not replace the monthly change color');
...
test('primary asset totals split decimal suffixes consistently', () => {
  ...
  assert.ok(homeTabSource.includes('className="home-report-decimal"'), 'Home should render its decimal suffix independently');
...
test('Home, Trading, and Assets use continuous scoped reports', () => {
  ...
  assert.ok(tradesTabSource.includes('className="trades-report-label">{tt(\'trades.totalAssets\''), 'Trading balance labels should use its report style');
  ...
  assert.ok(/\.asset-report-total\s*\{[^}]*font-size:\s*clamp\(28px,\s*8\.8vw,\s*40px\);[^}]*font-weight:\s*400;[^}]*line-height:\s*1\.18;/.test(analysisTabCss) && /\.asset-report-chart-amount\s*\{[^}]*font-size:\s*22px;[^}]*font-weight:\s*400;[^}]*line-height:\s*1\.18;/.test(analysisTabCss), 'the family total should retain the primary 40px scale while the historical overlay uses a compact 22px normal-weight amount');
  assert.ok(analysisTabSource.indexOf('className="asset-report-chart-change-amount"') >= 0 && analysisTabSource.indexOf('className="asset-report-chart-change-amount"') < analysisTabSource.indexOf('className="asset-report-chart-change-percent"') && /\.asset-report-chart-change-values\s*\{[^}]*display:\s*grid;/.test(analysisTabCss), 'the change amount should appear above its secondary percentage');
  assert.match(analysisTabCss, /\.asset-report-chart-amount \.asset-report-decimal\s*\{[^}]*color:\s*inherit;[^}]*font-size:\s*inherit;/, 'the historical asset overlay decimals should match the amount size and color');
  assert.match(analysisTabCss, /\.asset-report-chart-change-decimal\s*\{[^}]*font-size:\s*inherit;[^}]*opacity:\s*1;/, 'the historical change decimals should match the amount size and brightness');
  ...
  assert.ok(homeTabSource.includes('className="home-report-sentiment"'), 'VIX and CNN should share the linear sentiment area');
  assert.ok(homeTabSource.includes('<HomeWatchlistReport'), 'watchlist and holdings should use the continuous report component');
  assert.match(homeEarningsCss, /\.home-earnings-report\s*\{[^}]*background:\s*transparent;/, 'Home earnings should use a transparent report surface');
  ...
  assert.match(tradesTabCss, /\.trades-report-tools\s*\{[^}]*display:\s*flex;[^}]*justify-content:\s*space-between;/, 'all five quick actions must spread evenly between the report edges');
...
test('asset and review module cards do not keep legacy scale interactions', () => {
  ...
  assert.doesNotMatch(northStarGoalCardSource + northStarGoalCardCss, /active:scale|\bscale\s*\(|\banimation\s*:/, 'north-star card and its controls should not introduce scale or decorative animation');
...
test('review target page uses a flat dark report and preserves click action modals', () => {
  ...
  assert.ok(reviewTabSource.includes('annualRateText={`${(targetAnnualRate * 100).toFixed(0)}%`}'), 'north-star display should preserve the existing annual-rate formatting');
  assert.doesNotMatch(northStarGoalCardSource, /\bdb\.|\blocalStorage\b|\bMath\.pow\(|\bsetPlan\(/, 'north-star component should remain presentation-only without ledger, persistence, or compounding logic');
  ...
  assert.ok(reviewTabSource.includes("import CompoundDetailPage from '../pages/CompoundDetailPage.jsx'") && reviewTabSource.includes('if (showCompoundDetails) return (\n    <CompoundDetailPage'), 'north-star details should render as an independent page inside the Review tab');
  assert.ok(compoundDetailSource.includes('data-compound-detail="true"') && /<main\b[^>]*className="compound-detail-page"[^>]*data-compound-page="true"/.test(compoundDetailSource), 'compound detail page should retain its stable visual verification hooks');
  assert.ok(compoundDetailSource.includes("tt('review.compoundTitle'"), 'compound detail should title itself from the current plan years through i18n');
  assert.ok(compoundDetailSource.includes("tt('review.compoundPath'") && compoundDetailSource.includes("tt('review.compoundOriginalPlan'"), 'compound curve should label its original target path through i18n');
  assert.ok(compoundDetailSource.includes('data-compound-selection={activeKind}') && compoundDetailSource.includes("tt('review.compoundCurrentAssets'") && compoundDetailSource.includes("tt('review.actualEndingAssets'"), 'selected compound records should distinguish current assets from historical ending assets');
  assert.ok(compoundDetailSource.includes("const selectedAsset = activeKind === 'future' || activeKind === 'start' ? active?.plannedValue : activeActual"), 'future and starting values should use the original plan while recorded-year summaries use actual data');
  const futureSummaryStart = compoundDetailSource.indexOf("{activeKind === 'future' ? <dl className=\"cp-selected-facts\"");
  const futureSummaryEnd = compoundDetailSource.indexOf("</dl> : activeKind !== 'start'", futureSummaryStart);
  assert.ok(futureSummaryStart >= 0 && futureSummaryEnd > futureSummaryStart, 'future-year facts should render in a distinct summary branch');
  const futureSummary = compoundDetailSource.slice(futureSummaryStart, futureSummaryEnd);
  assert.ok(futureSummary.includes("tt('review.initialCapitalShort'") && futureSummary.includes("tt('review.compoundProjectedGain'") && futureSummary.includes('active.plannedValue - startCapital'), 'future facts should explain the initial principal and projected cumulative gain');
  assert.doesNotMatch(futureSummary, /activeActual|difference|actualGain|actualEndBalance|hasActual/, 'future facts must not invent actual amounts or an actual-performance difference');
  assert.ok(compoundDetailSource.includes("currentShortfall ? tt('review.compoundUntilYearEnd'") && compoundDetailSource.includes("tt('review.compoundVsOriginal'"), 'current-year shortfall should describe remaining year-end distance rather than a final historical result');
  assert.match(compoundDetailCss, /\.compound-detail-page\s*\{[^}]*width:\s*100%;[^}]*max-width:\s*430px;/, 'compound details should use the full Review page width');
  assert.ok(compoundDetailSource.includes('onClick={onBack}') && reviewTabSource.includes('onBack={closeCompoundDetails}'), 'compound details should return through the parent-owned page back action');
  assert.doesNotMatch(compoundDetailSource, /ReviewGoalModal|ActionModalCard|onClose|actions=|key:\s*['"]cancel['"]|fixed inset-0/, 'compound details should not retain the dialog shell, overlay, or close actions');
  assert.doesNotMatch(compoundDetailSource + compoundDetailCss, /document\.(?:body|documentElement)\.style|\boverflow(?:-y)?:\s*(?:auto|scroll|hidden)|\bposition:\s*fixed/, 'compound details should scroll with the document without a private scroll area or body lock');
  assert.ok(reviewTabSource.includes('compoundReturnScrollRef.current = window.scrollY') && reviewTabSource.includes('compoundPendingScrollRef.current = compoundReturnScrollRef.current') && reviewTabSource.includes('window.scrollTo({ top: compoundPendingScrollRef.current'), 'returning from compound details should restore the Review document scroll position');
  assert.ok(compoundDetailSource.includes('data-compound-year-row={row.year}') && compoundDetailSource.includes('<details className="cp-year-row"'), 'compound yearly details should retain stable hooks on native expandable rows');
  assert.match(compoundDetailCss, /\.compound-detail-page \.cp-chart\s*\{[^}]*height:\s*300px;/, 'compound path should use the larger chart space on its independent page');
  assert.match(compoundDetailCss, /\.compound-detail-page \.cp-chart\s*\{[^}]*touch-action:\s*pan-y;/, 'chart year gestures should preserve vertical document scrolling');
  assert.match(compoundDetailCss, /\.cp-plan-line\s*\{[^}]*stroke:\s*#686c76;[^}]*stroke-dasharray:/, 'the original target should use a muted dashed line');
  assert.match(compoundDetailCss, /\.cp-actual-line\s*\{[^}]*stroke:\s*#e4e4e9;/, 'recorded assets should use a separate light line');
  assert.match(compoundDetailCss, /\.cp-axis-label\s*\{[^}]*font-size:\s*10px;/, 'chart labels should respect the global text floor');
  assert.doesNotMatch(compoundDetailSource + compoundDetailCss, /linearGradient|linear-gradient|radial-gradient|#f6b54b|#ffd18a/, 'compound details should not restore the old gold or gradient decoration');
  for (const selectorList of compoundDetailCss.matchAll(/([^{}]+)\{/g)) {
    const selector = selectorList[1].trim();
    if (selector.startsWith('@')) continue;
    assert.ok(selector.split(',').every(part => /^\.compound-detail-page(?:$|[\s.:])/.test(part.trim())), 'compound CSS must remain scoped to its independent Review page');
  }
  assert.doesNotMatch(compoundDetailSource + compoundPathModelSource, /\bdb\.|\blocalStorage\b|\bsessionStorage\b|\bsetInvestmentPlan\(|\bsetYearlyActuals\(|\bfetch\(/, 'compound presentation and chart model must not write plans, annual records, storage, or network data');
  assert.ok(compoundDetailSource.includes('marketTextClass(row.actualGain, marketColorMode)'), 'compound actual gains and growth should respect the shared market color mode');
  ...
  assert.ok(compoundDetailSource.includes('buildCompoundYearDetailRows(yearRows, { currentYear })'), 'compound detail should derive display-only annual metrics through the unchanged tested helper');
  assert.ok(compoundDetailSource.includes('model.points.find(point => point.year === row.year)') && compoundDetailSource.includes('const hasActual = finite(point?.actualValue)') && compoundDetailSource.includes('actualEndBalance: hasActual ? point.actualValue : null'), 'annual display eligibility should match the strict chart record model and preserve unavailable actual balances');
  assert.ok(compoundPathModelSource.includes('row?.isProjected === false') && compoundPathModelSource.includes('year <= clockYear') && compoundPathModelSource.includes('actualGain !== null') && compoundPathModelSource.includes('endBalance !== null'), 'actual record eligibility must reject projections, future years, missing gains, and missing ending balances');
  assert.ok(compoundDetailSource.includes("tt('review.currentAssets', '当前资产')"), 'the current year should label its live balance as current assets');
  assert.ok(compoundDetailSource.includes("tt('review.actualEndingAssets', '实际期末资产')"), 'completed years should label their carried balance as actual ending assets');
  assert.ok(compoundDetailSource.includes("if (!row.hasActual) return row.isFutureYear ? tt('review.notStarted', '未开始') : tt('review.pending', '待填写')"), 'unrecorded rows should distinguish future years from pending past or current years');
  assert.ok(compoundDetailSource.includes("row.hasActual ? signed(row.actualGain) : '—'"), 'missing annual actuals must remain unavailable rather than appear as recorded zero');
  ...
  assert.ok(compoundDetailSource.includes('data-compound-execution="true"') && compoundDetailSource.includes('data-compound-simulation="true"'), 'annual execution and fixed-rate simulation should remain distinct views');
  assert.ok(compoundDetailSource.includes("view === 'execution' ? <section") && compoundDetailSource.includes('model.simulationRows.map'), 'switching views should display the appropriate annual data series');
  assert.ok(compoundDetailSource.includes("const activeView = activeKind === 'future' ? 'simulation' : 'execution'") && compoundDetailSource.includes('setView(activeView)') && compoundDetailSource.includes('setLinkedYear(active.year)'), 'selected-year navigation should route future years to simulation and recorded periods to annual execution');
  assert.ok(compoundDetailSource.includes('ledgerRef.current?.querySelector(`[data-compound-year-row="${linkedYear}"]`)') && compoundDetailSource.includes('row.open = true') && compoundDetailSource.includes("row.querySelector('summary')?.focus({ preventScroll: true })") && compoundDetailSource.includes('row.scrollIntoView({'), 'selected-year navigation should expand and focus the matching existing display row through its DOM ref');
  assert.ok(compoundDetailSource.includes("tt('review.compoundAnnualRolling'") && compoundDetailSource.includes("tt('review.compoundSimulationBasis'"), 'annual execution and simulation should explain their different calculation bases');
  assert.ok(compoundDetailSource.includes('buildCompoundPathModel({') && compoundPathModelSource.includes('Math.round(principal * Math.pow(1 + annualRate, index))'), 'fixed simulation should preserve the independent initial-principal target-rate series');
  assert.ok(i18nSource.includes("'review.compoundProjection': 'Projection'"), 'English compound details should name the projection view');
  assert.ok(devVisualPreviewSource.includes("initialCompoundDetails: reviewPanel === 'compound' || reviewPanel === 'compound-simulation'") && reviewTabSource.includes('initialView={ctx.initialCompoundView}'), 'local previews should open the requested compound view through the extracted component');
  assert.ok(devVisualPreviewSource.includes("activeTab !== 'review' || !['compound', 'compound-simulation'].includes(reviewPanel)") && devVisualPreviewSource.includes("!['years', 'year-detail', 'curve'].includes(reviewSection)"), 'compound screenshot positioning should require an explicit allowed compound preview section');
  assert.ok(devVisualPreviewSource.includes("const target = reviewSection === 'curve' ? content.querySelector('.cp-path-heading') : tabs") && devVisualPreviewSource.includes("target?.scrollIntoView({ block: 'start', behavior: 'instant' })"), 'compound screenshot fixtures should position either the curve or annual tabs with document scrolling');
  assert.ok(reviewTabSource.includes('onOpenDetails={openCompoundDetails}') && reviewTabSource.includes('setShowCompoundDetails(true)'), 'north-star card should open compound details through local Review navigation');
  assert.ok(reviewTabSource.includes('onCurrencyChange={switchCurrency}') && northStarGoalCardSource.includes('onCurrencyChange(currency)'), 'currency switch should preserve the existing Review currency handler');
  ...
  for (const [button] of northStarButtons) {
    ...
    assert.ok(button.includes('type="button"'), 'north-star controls should avoid accidental form submission');
  }
  assert.doesNotMatch(northStarGoalCardCss, /\.north-star-card\s*\{[^}]*\bheight\s*:/, 'north-star card height should follow its content, including large amounts and English labels');
  assert.ok(northStarGoalCardSource.includes('{motto && <p className="ns-motto">{motto}</p>}'), 'north-star motto should remain optional and preserve user-authored text');
  assert.match(northStarGoalCardCss, /\.ns-settings\s*\{[^}]*background:\s*transparent;/, 'north-star settings should keep a neutral independent control');
  assert.ok(northStarGoalCardSource.includes('className="ns-horizon"') && northStarGoalCardSource.includes('{goalSubtitle}'), 'north-star target subtitle should remain distinct from the goal amount');
  assert.ok(northStarGoalCardSource.includes("tt('review.goalYearsRemaining', '还剩 {{years}} 年', { years: yearsLeft })"), 'north-star remaining-years label should preserve the calculated value and translation');
  assert.ok(reviewTabSource.includes('<AnnualGoalPlan') && reviewTabSource.includes('visibleYears={visibleYears}'), 'annual plan should present the parent-computed carried annual rows');
  assert.ok(reviewTabSource.includes('onOpenYear={setYearAction}') && annualGoalPlanSource.includes('onOpenYear(currentYear)') && annualGoalPlanSource.includes('onOpenYear(item)'), 'current and other annual rows should open the same existing year action panel');
  assert.ok(annualGoalPlanSource.includes("tt('review.annualPlan', '年度计划')"), 'annual section should use its concise localized title');
  assert.match(annualGoalPlanCss, /\.ag-year-heading strong\s*\{[^}]*font-size:\s*22px;[^}]*font-weight:\s*400;/, 'current year should retain a neutral normal-weight number hierarchy');
  assert.match(annualGoalPlanCss, /\.ag-row-year\s*\{[^}]*font-size:\s*19px;[^}]*font-weight:\s*400;/, 'other years should use a lighter compact timeline hierarchy');
  assert.ok(reviewTabSource.includes('className="review-year-facts"') && reviewTabSource.includes("tt('review.yearStart', '年初起点')"), 'annual detail panel should retain the opening balance label');
  assert.ok(annualGoalPlanSource.includes("tt('review.annualTargetAssets', '期末目标资产')"), 'compact future rows should explicitly label projected end assets as a target');
  ...
  assert.equal(northStarGoalCardSource.includes('bottom-[-78px] h-48 w-48'), false, 'north-star card should not keep the lower-right semicircle decoration');
  assert.doesNotMatch(annualGoalPlanCss, /margin[^;]*-\d/, 'annual presentation should keep the north-star content width instead of expanding beyond the page');
  ...
  assert.doesNotMatch(northStarGoalCardSource + northStarGoalCardCss, /rocket-particle|polar-twinkle|review-star|box-shadow|radial-gradient/, 'north-star header should not render decorative particles, stars, or glow');
  ...
  assert.doesNotMatch(annualGoalPlanSource + annualGoalPlanCss, /progress-shine|rocket-particle|box-shadow|linear-gradient|radial-gradient/, 'main annual plan should not reintroduce animated glow or decorative progress effects');
  assert.ok(annualGoalPlanSource.includes('currentGap') && annualGoalPlanSource.includes("tt('review.annualGapRemaining', '距离目标还差 {{amount}}'"), 'current year should state the remaining annual profit gap');
  assert.match(northStarGoalCardCss, /\.ns-motto\s*\{[^}]*color:\s*#696970;[^}]*overflow-wrap:\s*anywhere;/, 'personal motto should remain muted and allow long user-authored text to wrap');
  assert.ok(annualGoalPlanSource.includes('money(currentYear.planTarget)') && annualGoalPlanSource.includes("tt('review.annualProfitTarget', '年度收益目标')"), 'current-year target should show annual planned gain, not total ending assets');
  assert.ok(annualGoalPlanSource.includes("tt('review.annualRealizedGain', '本年已实现')") && annualGoalPlanSource.includes('signedMoney(currentYear.actualGain)'), 'current-year primary amount should display actual annual gain with its sign');
  assert.ok(annualGoalPlanSource.includes('marketHexColor(currentYear.actualGain, marketColorMode)'), 'actual annual gains should preserve the shared user market-color mode');
  assert.ok(annualGoalPlanSource.includes('!item.isProjected && Number.isFinite(item.actualGain)'), 'projected or missing annual actuals must not be presented as recorded zero');
  assert.ok(annualGoalPlanSource.includes("item.year > thisYear ? 'notStarted' : 'pending'"), 'unrecorded past/current years must remain pending, distinct from future years that have not started');
  assert.doesNotMatch(annualGoalPlanSource, /grid-cols-\[minmax\(0,1fr\)_172px\]|targetGapPct/, 'annual hierarchy should not restore the fixed-width duplicate percentage summary');
  ...
  assert.ok(annualGoalPlanSource.includes('currentYear.actualGain / currentYear.planTarget * 100'), 'annual completion should use actual annual gain divided by the annual profit plan');
  assert.ok(annualGoalPlanSource.includes('currentYear.planTarget - currentYear.actualGain'), 'remaining profit gap should use annual target minus recorded gain');
  assert.ok(annualGoalPlanSource.includes('Math.max(0, Math.min(100, progress))') && annualGoalPlanSource.includes('progress.toFixed(1)'), 'progress fill should clamp visually while its text retains the actual negative or over-100 rate');
  assert.ok(annualGoalPlanSource.includes('aria-valuenow={boundedProgress === null ? undefined : boundedProgress}'), 'unrecorded annual progress should omit the numeric accessibility value');
  ...
  assert.ok(annualGoalPlanSource.includes('resolveAnnualGoalStatus(item.actualGain, item.planTarget)'), 'annual status should preserve the tested displayed-cent precision resolver');
  assert.ok(annualGoalPlanSource.includes("tt('review.exceeded', '超额')"), 'overachievement should keep its dedicated localized status');
  assert.ok(annualGoalPlanSource.includes("data-annual-goal-status={status || 'pending'}"), 'annual status should retain its explicit semantic hook and safe pending fallback');
  assert.ok(annualGoalPlanSource.includes("status === 'exceeded' && <TrendingUp"), 'overachievement should retain its distinct upward icon');
  assert.match(northStarGoalCardCss, /\.ns-completion strong\s*\{[^}]*color:\s*#ff4b1f;/, 'overall completion percentage should use system red');
  assert.match(northStarGoalCardCss, /\.ns-progress-fill\s*\{[^}]*background:\s*#ff4b1f;/, 'north-star completion progress should use the same system red');
  assert.match(annualGoalPlanCss, /\.ag-progress-fill\s*\{[^}]*background:\s*#ff4b1f;/, 'annual progress should use the same system-red fill as north-star completion');
  assert.ok(reviewTabSource.includes('money(yearAction.startBalance)') && reviewTabSource.includes('money(yearAction.endBalance)'), 'read-only annual details should retain both carried opening and recorded ending balances');
  ...
  assert.ok(reviewTabSource.includes("tt('review.yearStart', '年初起点')") && reviewTabSource.includes('className="review-year-facts"'), 'annual details should label the carried opening balance explicitly');
  ...
  assert.ok(annualGoalPlanSource.includes('item.startBalance + item.planTarget'), 'future target assets should derive from the same carried year-start balance used by parent calculations');
  assert.doesNotMatch(annualGoalPlanSource, /\bdb\.|\blocalStorage\b|\bMath\.pow\(|\bsetYearlyActuals\(/, 'annual presentation should not write records or rebuild its own compounding series');
  assert.ok(reviewTabSource.includes("yearAction.isProjected ? tt('review.pending', '待填写') : money(yearAction.endBalance)"), 'annual action details must not present projected balances as recorded assets');
  assert.ok(reviewTabSource.includes('className="rgm-projection"') && reviewTabSource.includes('<strong>{money(ageGoalAmountExact)}</strong>'), 'plan settings summary should use the same exact two-decimal target as the north-star card');
  ...
  assert.equal((reviewTabSource.match(/onToggleYears=\{\(\) => setShowAllYears\(!showAllYears\)\}/g) || []).length, 1, 'annual plan should preserve a single parent-controlled expand handler');
  assert.ok(annualGoalPlanSource.includes('onClick={onToggleYears}') && annualGoalPlanSource.includes('aria-expanded={showAllYears}'), 'annual expand control should expose its state and use the parent callback');
  assert.equal((reviewTabSource.match(/className="review-section-header"/g) || []).length, 2, 'notes and logs should share the same reading-section header hierarchy');
  assert.ok(reviewTabSource.includes('id="review-disciplines-heading"') && reviewTabSource.includes("tt('review.disciplines', '投资心得')"), 'investment notes should retain a localized accessible section heading');
  assert.ok(reviewTabSource.includes('id="review-logs-heading"') && reviewTabSource.includes("tt('review.reviewLogs', '复盘日志')"), 'review logs should retain a localized accessible section heading');
  assert.match(reviewTabCss, /\.review-section-header h2\s*\{[^}]*font-size:\s*15px;[^}]*font-weight:\s*500;/, 'reading titles should share a quiet medium-weight hierarchy');
  ...
  assert.equal((reviewTabSource.match(/type="button" className="review-text-action"/g) || []).length, 2, 'notes and logs should use separate native text actions');
  assert.ok(reviewTabSource.includes('onClick={() => setShowAddDiscipline(true)}') && reviewTabSource.includes('onClick={() => setShowAddLog(true)}'), 'new reading-header styling should preserve both add flows');
  assert.ok(reviewTabSource.includes("dotColor: '#18d66b'"), 'discipline level metadata should use colored dots instead of icons');
  assert.ok(reviewTabSource.includes('className="review-filters" role="group"') && reviewTabSource.includes("tt('review.filterDiscipline', '筛选投资心得')"), 'note filters should retain a named accessible group');
  assert.ok(reviewTabSource.includes("type=\"button\" aria-pressed={filterLevel === 'all'}") && reviewTabSource.includes('type="button" aria-pressed={filterLevel === item.level}'), 'native note filter controls should expose their current selected state');
  assert.ok(reviewTabSource.includes('onClick={() => setFilterLevel(item.level)}') && reviewTabSource.includes('discipline.level === item.level'), 'localized filter labels must preserve the existing level enum and selection behavior');
  assert.ok(reviewTabSource.includes('tt(`review.disciplineLevel${index}`, item.label)'), 'discipline level names should be localized without modifying stored levels');
  assert.match(reviewTabCss, /\.review-filters\s*\{[^}]*flex-wrap:\s*wrap;/, 'reading filters should wrap safely on narrow screens');
  ...
  assert.doesNotMatch(reviewTabSource + reviewReadingDetailsSource, /UsFlagBackground|US_FLAG_STRIPES|US_FLAG_STARS|data-us-flag-bg/, 'review reading surfaces should not retain the superseded flag decoration');
  assert.ok(reviewTabSource.includes('className="review-entry" data-pinned={discipline.pinned ? \'true\' : undefined}'), 'pinned notes should retain a scoped distinct reading surface');
  ...
  assert.match(reviewTabCss, /\.review-entry-body\s*\{[^}]*white-space:\s*pre-wrap;[^}]*overflow-wrap:\s*anywhere;[^}]*font-size:\s*14px;/, 'complete note and log bodies should preserve line breaks and wrap safely at a readable body size');
  assert.equal((reviewTabSource.match(/className="review-entry-meta"/g) || []).length, 2, 'notes and logs should share their muted date metadata styling');
  assert.ok(reviewTabSource.includes('<span>{discipline.date}</span>') && reviewTabSource.includes('<span>{log.date}</span>'), 'reading rows should preserve their exact recorded dates');
  assert.ok(reviewTabSource.includes('className="review-pinned"') && reviewTabSource.includes("tt('review.pinned', '置顶')"), 'pinned notes should retain a localized text indication');
  ...
  assert.ok(reviewTabSource.includes('className="review-entry-body">{discipline.text}</span>'), 'discipline article rows should render the complete body directly');
  ...
  assert.ok(reviewEntryEditorsSource.includes('aria-pressed={level === l.level}') && reviewEntryEditorsSource.includes('onClick={() => setLevel(l.level)}>{l.label}</button>'), 'discipline editor should use native text choices without changing stored levels');
  assert.doesNotMatch(reviewEntryEditorsSource, /ringColor|ringBorder|dotColor|<span className="text-base">\{l.level\}/, 'discipline editor should not retain colored rings or render stored emoji as icons');
  assert.ok(reviewReadingDetailsSource.includes('function DisciplineDetailModal') && reviewTabSource.includes("import { DisciplineDetailModal, ReviewLogDetailModal } from '../components/ReviewReadingDetails.jsx'"), 'discipline rows should use the extracted real record detail modal');
  assert.equal((reviewReadingDetailsSource.match(/<ReviewGoalModal/g) || []).length, 2, 'both reading detail modals should share the scoped goal shell');
  assert.ok(reviewGoalModalSource.includes('widthClassName="w-[calc(100vw-32px)] max-w-[398px]"'), 'reading details should share the approved goal-modal width');
  assert.equal(reviewReadingDetailsSource.includes('bg-black/70 px-6 py-8 backdrop-blur-lg'), false, 'reading details should not restore a separate overly dark overlay');
  assert.ok(reviewReadingDetailsSource.includes("tt('review.disciplineDetails', '心得详情')"), 'discipline detail modal should use its dedicated localized title');
  assert.match(reviewReadingDetailsCss, /\.review-goal-modal \.rgm-reading-text\s*\{[^}]*min-height:\s*96px;/, 'reading details should retain space for short content without oversized nested cards');
  assert.equal((reviewReadingDetailsSource.match(/\{String\(item.text \?\? ''\)\}/g) || []).length, 2, 'both detail modals should render complete original text without reformatting or truncation');
  assert.match(reviewReadingDetailsCss, /\.review-goal-modal \.rgm-reading-text\s*\{[^}]*white-space:\s*pre-wrap;[^}]*overflow-wrap:\s*anywhere;/, 'full reading text should retain whitespace and wrap long content');
  assert.doesNotMatch(reviewReadingDetailsSource + reviewReadingDetailsCss, /line-clamp|text-overflow|\.slice\(|\.trim\(/, 'detail views must not shorten or normalize stored text');
  assert.ok(reviewReadingDetailsSource.includes("item.pinned ? tt('review.unpin'") && reviewReadingDetailsSource.includes('onClick: onTogglePin'), 'discipline detail modal must keep pin/unpin through i18n');
  assert.ok(reviewGoalModalSource.includes("actionGridClassName={actions.length === 3 ? 'grid-cols-3'"), 'three detail actions should retain a compact three-button layout');
  assert.equal(reviewReadingDetailsSource.includes('{Edit2 && <Edit2'), false, 'discipline detail actions should not render icons');
  assert.equal(reviewReadingDetailsSource.includes('删除戒律'), false, 'discipline detail modal should not keep the large legacy delete label');
  assert.equal(reviewReadingDetailsSource.includes('修改戒律'), false, 'discipline detail modal should not keep the large legacy edit label');
  ...
  assert.ok(reviewReadingDetailsSource.includes('function ReviewLogDetailModal'), 'review logs should open a detail preview modal before editing');
  assert.ok(reviewReadingDetailsSource.includes("tt('review.reviewDetails'"), 'review log detail modal should use a dedicated detail title through i18n');
  assert.ok(reviewReadingDetailsSource.includes('{item.date && <span>{item.date}</span>}') && reviewReadingDetailsSource.includes('{item.mood && <span>{item.mood}</span>}'), 'review details should retain the original date and mood as quiet metadata');
  ...
  assert.ok(reviewTabSource.includes('className="review-entry-body">{text}</span>'), 'review log rows should render the complete body directly');
  ...
  const readingBlock = reviewTabSource.slice(reviewTabSource.indexOf('<section className="review-reading-section"'), reviewTabSource.indexOf('{yearAction && ('));
  assert.doesNotMatch(readingBlock, /role="button"|line-clamp|text-overflow|\.slice\(0,\s*(?:60|150)\)/, 'reading rows should use native buttons and never truncate full note or log text');
  for (const [button] of readingBlock.matchAll(/<button\b[\s\S]*?<\/button>/g)) {
  ...
  assert.ok(devVisualPreviewSource.includes("['home', 'trades', 'analysis', 'review', 'settings', 'pnl-report', 'pnl-share', 'home-margin-risk', 'drawdown-observation', 'stock-detail', 'stock-pnl-report', 'watchlist-stock-detail', 'wave-tracker', 'community-competition', 'vix-comparison', 'fear-greed', 'stock-decision', 'investment-comparison', 'macro', 'portfolio-overlap', 'dca-lab'].includes(requestedTab)"), 'local visual preview should support all tabs plus the report-style standalone pages and Macro directly');
  ...
  assert.ok(devVisualPreviewSource.includes("import { DisciplineModal, LogModal } from './components/ReviewEntryEditors.jsx'") && reviewEntryEditorsSource.includes("isEdit ? tt('review.editReview', '编辑复盘') : tt('review.addReview', '写复盘')"), 'local visual preview should use the real editor and its correct edit state instead of a stub');
  assert.equal(homeTabSource.includes('FearIndexCards.tsx'), false, 'Home must not restore the retired standalone fear-card family');
  assert.equal(homeTabSource.includes('<VixFearIndexCard') || homeTabSource.includes('<FearGreedIndexCard'), false, 'sentiment should remain in the shared report market section');
  assert.ok(homeTabSource.includes('className="home-report-sentiment"'), 'VIX and CNN should share one compact sentiment area');
  ...
  assert.ok(homeTabSource.includes('data-home-drawdown-trigger') && homeTabSource.includes('onClick={() => openDrawdownObservation?.()}'), 'market position must retain its existing independent drawdown navigation');
  assert.ok(homeTabSource.includes('home-report-drawdown-value') && homeTabSource.includes('pnlColor(benchmarkDrawdown, marketColorMode)'), 'drawdown should retain its signed global market color');
  assert.ok(homeTabSource.includes('<span className="block text-[15px] font-normal tracking-[0.01em] text-white/85">{row.symbol}</span>'), 'benchmark picker symbol typography remains unchanged');
  assert.ok(homeTabSource.includes('<HomeWatchlistReport') && homeTabSource.includes('rows={tableRows}') && homeTabSource.includes('onSort={handleTableSort}'), 'Home must pass its existing prepared rows and sort callback to the report renderer');
  assert.ok(homeTabSource.includes('tableTab={tableTab}') && homeTabSource.includes('onTabChange={setTableTab}'), 'watchlist and holdings tabs must retain their Home-owned state');
  assert.ok(homeTabSource.includes('onOpenStock={openWatchlistStockDetail}'), 'watchlist identity must retain the existing stock-detail route');
  assert.ok(homeWatchlistSource.includes('role="tablist"') && homeWatchlistSource.includes('aria-selected={tableTab === tab}'), 'the simplified list must expose active tab semantics');
  assert.ok(homeWatchlistSource.includes('data-home-watchlist-report="true"'), 'the shared row report should remain directly testable');
  assert.ok(homeWatchlistSource.includes('className="hwr-row-main"') && homeWatchlistSource.includes('className="hwr-column-headings"'), 'headers and rows should use one aligned grid');
  assert.match(homeWatchlistCss, /\.hwr-column-headings,[\s\S]*?\.hwr-row-main\s*\{[^}]*grid-template-columns:\s*var\(--hwr-columns\)/, 'headers and rows must share their grid definition');
  assert.ok(homeWatchlistSource.includes('formatPnl, item.pnlDisplayValue') && homeWatchlistSource.includes('formatPnlPct, item.pnlPct'), 'holding P&L must keep converted money and the original percent');
  assert.ok(homeTabSource.includes('marketColor={(value) => marketColor(value, marketColorMode)}'), 'report colors must still use the global market convention');
  assert.ok(homeWatchlistSource.includes('colorFor(auxiliaryValue)') && homeWatchlistSource.includes('colorFor(item.pnlPct)'), 'holding P&L amount and percent must each retain signed market colors');
  assert.match(homeWatchlistCss, /\.hwr-symbol\s*\{[^}]*font-weight:\s*400;/, 'ticker codes should retain normal weight');
  assert.match(homeWatchlistCss, /\.hwr-price\s*\{[^}]*text-align:\s*right;[^}]*white-space:\s*nowrap;/, 'stock prices should align right without breaking digits');
  assert.match(homeWatchlistCss, /\.hwr-aux-value\s*\{[^}]*flex-wrap:\s*wrap;[^}]*overflow-wrap:\s*anywhere;/, 'long holding P&L must remain visible rather than clipped');
  assert.equal(homeTabSource.includes('data-home-market-table="single-grid"'), false, 'the old horizontally scrolling table should not remain behind the new report');
  assert.ok(tradesTabSource.includes('marketValue: currencyAmount(marketValue, displayCurrency, 2)'), 'trade position market value should keep two decimals and the selected-currency label');
  assert.ok(settingsTabSource.includes('EODHD Core + Yahoo Charts'), 'settings data source should distinguish core EODHD quotes from Yahoo chart visuals');
...
test('review edit modals use in-app validation instead of native alerts', () => {
  const disciplineStart = reviewEntryEditorsSource.indexOf('export function DisciplineModal');
  const disciplineEnd = reviewEntryEditorsSource.indexOf('export function LogModal', disciplineStart);
  const disciplineBlock = reviewEntryEditorsSource.slice(disciplineStart, disciplineEnd);
  const logStart = disciplineEnd;
  const logEnd = reviewEntryEditorsSource.length;
  const logBlock = reviewEntryEditorsSource.slice(logStart, logEnd);
  ...
  assert.ok(disciplineBlock.includes('if (!text.trim())') && disciplineBlock.includes('onSave({ level, text: text.trim(), pinned });'), 'discipline save should retain required text validation and its unchanged trimmed payload');
  assert.ok(logBlock.includes('if (!text.trim())') && logBlock.includes('onSave({ date, mood: mood.trim(), text: text.trim() });'), 'review log save should retain required text validation and its unchanged date and trimmed payload');
  assert.ok(logBlock.includes("setMood(m === mood ? '' : m)") && logBlock.includes('onChange={e => setDate(e.target.value)}'), 'review log mood toggle and date editing should remain unchanged');
  assert.ok(reviewReadingDetailsSource.includes("const DISCIPLINE_LEVELS = ['🟢', '🔺', '📣', '❗'];") && reviewReadingDetailsSource.includes("const DISCIPLINE_LEVEL_LABELS = ['一般', '重要', '强调', '警告'];"), 'detail labels must preserve the existing stored level meanings');
  const disciplineDeleteBlock = reviewTabSource.slice(reviewTabSource.indexOf('const deleteDiscipline ='), reviewTabSource.indexOf('const openDisciplineEdit ='));
  const logDeleteBlock = reviewTabSource.slice(reviewTabSource.indexOf('const deleteReviewLog ='), reviewTabSource.indexOf('const openReviewLogEdit ='));
  assert.ok(disciplineDeleteBlock.includes('showConfirm({') && disciplineDeleteBlock.includes('onConfirm: async () => {\n        await db.deleteDiscipline(discipline.id);'), 'discipline deletion must still pass through the parent confirmation before its original database call');
...
test('approved modal families share the new shell without widening business boundaries', () => {
  ...
  const assetTrendPageEnd = assetCategoryReportPageStart;
  const assetCategoryReportPageEnd = analysisTabSource.indexOf('\n\n  return (', assetCategoryReportPageStart);
  const assetTrendPageBlock = analysisTabSource.slice(assetTrendPageStart, assetTrendPageEnd);
  const assetCategoryReportPageBlock = analysisTabSource.slice(assetCategoryReportPageStart, assetCategoryReportPageEnd);
  const assetAccountReportModelStart = analysisTabSource.indexOf('const assetCategoryReport = React.useMemo');
  const assetAccountReportModelEnd = analysisTabSource.indexOf('const {\n    chartMin,', assetAccountReportModelStart);
  const assetAccountReportModelBlock = analysisTabSource.slice(assetAccountReportModelStart, assetAccountReportModelEnd);
  ...
  assert.ok(assetTrendPageBlock.includes('onClick={closeMonthlyAssetTrend}') && assetTrendPageBlock.includes("analysis.backToAssetOverview"), 'asset month trend should return to the asset overview through a page-level back control');
  assert.ok(assetTrendPageBlock.includes('<MonthlyAssetTrendContent') && assetTrendPageBlock.includes('onOpenMonthReport={openMonthlyAssetCategoryReport}'), 'asset month trend page should hand the exact selected month to the account report');
  assert.ok(assetTrendPageBlock.includes('expanded={monthlyDetailsExpanded}') && assetTrendPageBlock.includes('onExpandedChange={setMonthlyDetailsExpanded}'), 'monthly detail expansion should remain parent-owned while the nested report is open');
  assert.ok(assetTrendPageBlock.includes('comparisonStartMonth={yearAgo}') && assetTrendPageBlock.includes('comparisonStartValue={totalYearAgo}'), '12-month asset change should use the exact year-ago endpoint while monthly details stay at twelve rows');
  assert.equal(assetTrendPageBlock.includes('<ActionModalCard'), false, 'asset month trend should not retain a modal shell or redundant outer card');
  assert.ok(assetCategoryReportPageBlock.includes('data-monthly-asset-category-report-page="true"') && assetCategoryReportPageBlock.includes('<MonthlyAssetCategoryReport'), 'the account report should render as a full-width nested asset page');
  assert.ok(assetCategoryReportPageBlock.includes('onClick={closeMonthlyAssetCategoryReport}') && assetCategoryReportPageBlock.includes("analysis.backToMonthlyAssetTrend"), 'the account report should return to the monthly trend page');
  assert.equal(assetCategoryReportPageBlock.includes('edit') || assetCategoryReportPageBlock.includes('Edit') || assetCategoryReportPageBlock.includes('修改'), false, 'the account report header should not expose an edit action');
  assert.equal(monthlyAssetCategoryReportSource.includes('onEdit') || monthlyAssetCategoryReportSource.includes('setShowFillSnapshot') || monthlyAssetCategoryReportSource.includes('db.'), false, 'the account report content must remain read-only');
  assert.ok(assetAccountReportModelBlock.includes('buildMonthlyAssetAccountReport({')
    && assetAccountReportModelBlock.includes('accounts,')
    && assetAccountReportModelBlock.includes('snapshots,')
    && assetAccountReportModelBlock.includes('month: selectedAssetCategoryMonth,')
    && assetAccountReportModelBlock.includes('toCNY,'), 'the account report must reuse the parent asset page account, exact-month snapshot, and FX inputs');
  assert.ok(monthlyAssetCategoryReportSource.includes('ownerGroups.map')
  ...
  assert.equal(devVisualPreviewSource.includes('accountReportTransitions: monthlyAssetCategoryReportPreview'), false, 'the legacy category-report preview query must not alter default account snapshots');
  assert.ok(analysisTabSource.includes('setFillMonth(localMonthKey());') && analysisTabSource.includes('setShowFillSnapshot(true);'), 'monthly balance editing should remain on the asset overview');
  assert.equal(analysisTabSource.includes('setFillMonth(month);') || analysisTabSource.includes('openMonthlyBalanceEditor'), false, 'monthly detail rows should no longer open the balance editor');
  ...
  assert.equal((analysisTabSource.match(/onClick=\{openMonthlyAssetTrend\}/g) || []).length, 2, 'both existing asset overview entries should open the same independent trend page');
  ...
  assert.ok(monthlyAssetTrendContentSource.includes('data-asset-trend-month-row={slot.month}') && monthlyAssetTrendContentSource.includes('data-open-asset-category-report={slot.month}'), 'monthly detail rows should expose one exact-month report navigation target');
  assert.ok(monthlyAssetTrendContentSource.includes('onClick={() => onOpenMonthReport?.(slot.month)}'), 'clicking anywhere on a monthly detail row should open its exact account report');
  assert.equal(monthlyAssetTrendContentSource.includes('data-asset-trend-month-edit') || monthlyAssetTrendContentSource.includes('onEditMonth'), false, 'monthly detail rows should not retain the old small edit control');
  assert.ok(analysisTabSource.includes('monthlyTrendScrollYRef.current = window.scrollY') && analysisTabSource.includes('window.scrollTo({ top: monthlyTrendScrollY, left: 0'), 'returning from a category report should restore the monthly trend scroll position');
  assert.ok(analysisTabSource.includes("setSelectedAssetCategoryMonth('');") && analysisTabSource.includes('setMonthlyDetailsExpanded(false);') && analysisTabSource.includes('if (showMonthsDetail) return;'), 'leaving the asset subpage should clear nested report and expansion state');
  assert.ok(analysisTabSource.includes('selectedAssetCategoryMonth ? monthlyAssetCategoryReportPage : monthlyAssetTrendPage') && analysisTabSource.includes('      )}\n\n      {showFillSnapshot && ('), 'the read-only account report should stay inside the existing asset subpage stack while asset editors remain outside it');
  assert.ok(monthlyAssetTrendContentSource.includes('data-asset-trend-toggle="true"') && monthlyAssetTrendContentSource.includes('DEFAULT_COLLAPSED_MONTH_COUNT'), 'monthly details should default to six rows and support expanding all twelve');
  assert.ok(monthlyAssetTrendChartSource.includes('MONTHLY_ASSET_CHART_WIDTH = 370') && monthlyAssetTrendContentSource.includes('grid-cols-[90px_minmax(70px,1fr)_minmax(68px,0.96fr)_49px_18px]'), 'asset trend chart and monthly grid should use the wider standalone-page content geometry');
  assert.ok(monthlyAssetTrendChartSource.includes('Object.freeze({ left: 48, right: 362, top: 22, bottom: 169 })') && monthlyAssetTrendContentSource.includes('className="mt-2 aspect-[370/206] select-none touch-pan-y"') && monthlyAssetTrendChartSource.includes('<text x="0"') && monthlyAssetTrendChartSource.includes('const labelX = last ? MONTHLY_ASSET_CHART_WIDTH'), 'chart plot, points, and axis labels should stay inside one responsive viewBox aligned with the cards below');
  assert.equal(monthlyAssetTrendChartSource.includes('CHART_EDGE_LABEL_OFFSET'), false, 'chart labels should not overflow the shared page edge through fixed offsets');
  assert.equal(monthlyAssetTrendChartSource.includes("analysis.highestAssetShort"), false, 'asset trend chart should not show highest or current-value callouts before interaction');
  assert.ok(monthlyAssetTrendChartSource.includes('{selectedPoint && (') && monthlyAssetTrendChartSource.includes('selectedPoint.month'), 'tapping any chart point, including the current month, should reveal the selected month value');
  assert.ok(monthlyAssetTrendContentSource.includes('ref={chartInteractionRef}') && monthlyAssetTrendContentSource.includes('if (selectedIndex === null) return undefined;') && monthlyAssetTrendContentSource.includes("document.addEventListener('pointerdown', clearSelectedPointOutsideChart, true)"), 'touching outside the chart should dismiss an active selected month value without blocking chart point changes');
  assert.ok(monthlyAssetTrendContentSource.includes("document.removeEventListener('pointerdown', clearSelectedPointOutsideChart, true)"), 'asset trend outside-touch handling should clean up when the page unmounts');
  assert.ok(monthlyAssetTrendContentSource.includes('pl-px text-[11px] leading-[15px]'), 'current asset label should keep an iOS glyph safety inset instead of touching the scroll boundary');
  assert.equal([monthlyAssetTrendContentSource, monthlyAssetTrendChartSource].some(source => source.includes('#75a7ff') || source.includes('#111a29')), false, 'asset month trend should not reintroduce blue latest-point accents');
  ...
  assert.ok(reviewGoalModalSource.includes('<ActionModalCard') && reviewGoalModalSource.includes('panelClassName="review-goal-modal"'), 'goal-modal presentation should retain the shared overlay and keyboard handling');
  assert.ok(reviewGoalModalSource.includes("actionGridClassName={actions.length === 3 ? 'grid-cols-3'"), 'discipline-detail domain actions should preserve their three-column layout through the goal shell');
  const annualActions = reviewTabSource.slice(reviewTabSource.indexOf('{yearAction && ('), reviewTabSource.indexOf('{disciplineAction && ('));
  for (const block of [northStarSettings, annualActions, reviewEditorBlock, yearlyActualModalSource]) {
    assert.doesNotMatch(block, /key:\s*['"]cancel['"]/, 'approved Review callers should use top-right close without a duplicate footer cancel');
  }
  assert.ok(northStarSettings.includes('onClose={() => setShowPlanSettings(false)}') && annualActions.includes('onClose={() => setYearAction(null)}'), 'settings and annual actions should preserve their top-right dismissal callbacks');
  assert.equal((reviewEditorBlock.match(/onClose=\{onCancel\}/g) || []).length, 2, 'both record editors should preserve top-right cancellation');
  assert.ok(yearlyActualModalSource.includes('onClose={onCancel}'), 'yearly data editor should preserve top-right cancellation');
