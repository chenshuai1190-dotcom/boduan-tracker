...
test('main trade entry modal uses compact four-step buy sell submission flow', () => {
  ...
  assert.ok(settingsTabSource.includes('v10.7.9.174'), 'settings version badge should document the quote daily pnl update');
  assert.ok(settingsChangelogSource.includes('v10.7.9.174'), 'settings changelog should document the quote daily pnl update');
  assert.ok(settingsChangelogSource.includes('股票昨收兜底和当日盈亏修复'), 'settings changelog should describe the quote daily pnl update');
  assert.ok(settingsChangelogSource.includes('v10.7.9.173'), 'settings changelog should retain the modal detail update');
  assert.ok(settingsChangelogSource.includes('弹窗字重和交易确认细节'), 'settings changelog should retain the modal detail update');
...
test('asset and review module cards do not keep legacy scale interactions', () => {
  ...
  assert.ok(settingsTabSource.includes('v10.7.9.174'), 'settings version badge should document the latest quote daily pnl update');
  assert.ok(settingsChangelogSource.includes('v10.7.9.174'), 'settings changelog should document the latest quote daily pnl update');
  assert.ok(settingsChangelogSource.includes('股票昨收兜底和当日盈亏修复'), 'settings changelog should describe the latest quote daily pnl update');
  assert.ok(settingsChangelogSource.includes('v10.7.9.173'), 'settings changelog should retain the modal detail update');
