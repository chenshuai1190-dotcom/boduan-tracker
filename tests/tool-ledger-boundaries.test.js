...
test('main trade entry modal uses compact four-step buy sell submission flow', () => {
  ...
  assert.ok(settingsTabSource.includes('v10.7.9.199'), 'settings version badge should document the iOS PWA holding price freshness mask update');
  assert.ok(settingsChangelogSource.includes('v10.7.9.199'), 'settings changelog should document the iOS PWA holding price freshness mask update');
...
test('realtime quote refresh avoids duplicate requests and hides raw Safari network errors', () => {
  ...
  assert.ok(appSource.includes('IOS_PWA_REALTIME_SNAPSHOT_INTERVAL_MS = 2500'), 'iOS standalone mode should use fast authenticated snapshot polling instead of browser WebSocket');
  assert.ok(appSource.includes('IOS_PWA_REALTIME_SNAPSHOT_BURST_DELAYS_MS = [0, 1000, 2500, 5000, 9000]'), 'iOS standalone startup should run a short realtime snapshot burst before settling into polling');
  assert.ok(appSource.includes("fetchRealtimeSnapshot('/api/btc-realtime')"), 'iOS standalone mode should poll the BTC snapshot endpoint');
  ...
  assert.ok(appSource.includes("ref.socket.close(1000, 'ios pwa snapshot mode')"), 'iOS standalone mode should close browser realtime sockets before switching connection strategy');
  ...
  assert.ok(appSource.includes("iosPwaRealtimeSnapshotBurstRef.current(nextTrigger)"), 'iOS standalone resume should prefer realtime snapshot burst over REST quote refresh');
  assert.ok(appSource.includes("pendingPwaResumeRefreshRef.current = 'auto-ios-pwa-snapshot-cloud'"), 'iOS standalone cloud load should wait for realtime snapshot burst instead of falling back to REST quotes');
  assert.ok(homeTabSource.includes("'home.market.warming'"), 'BTC card should show a warming state while iOS standalone realtime snapshots are syncing');
