...
test('main trade entry modal isolates the compact stacked formal-trade design', () => {
  ...
  assert.ok(genericHeaderBlock.includes('data-generic-ledger-symbol-header="true"') && genericHeaderBlock.includes('<StockLogo'), 'formal trade should put the editable ticker, name, and logo in one identity header');
  ...
  assert.ok(genericLedgerTradeEntryPanelSource.includes('min-h-[56px]') && genericLedgerTradeEntryPanelSource.includes('text-[25px]') && genericPanelBlock.includes('space-y-[13px]'), 'price, shares, and date should use the approved compact rhythm');
  assert.ok(genericPanelBlock.includes('grid-cols-[22px_minmax(0,1fr)_30px]') && genericPanelBlock.includes('grid-cols-[minmax(0,1fr)_26px]') && genericPanelBlock.includes('grid-cols-[18px_minmax(0,1fr)_16px]'), 'currency, numeric values, clear control, share unit, and date icons should keep fixed columns while editing');
  ...
  assert.ok(tradeModalBlock.includes("isGenericLedgerTradeEntry ? 'min-h-0 !border-transparent !px-[22px] !pb-5 !pt-[22px]'"), 'generic formal trade should keep the approved borderless shell with deliberate inner spacing');
  assert.ok(tradeModalBlock.includes("isGenericLedgerTradeEntry ? '!rounded-none !border-0 !bg-transparent !px-0 !py-0 !shadow-none'"), 'generic formal trade should remove the redundant nested content frame');
  assert.ok(tradeModalBlock.includes("headerClassName={isGenericLedgerTradeEntry ? '!mb-[14px] !border-b !border-white/[0.065] !pb-[14px]' : ''}"), 'generic identity header should retain one compact hierarchy divider');
  assert.ok(tradeModalBlock.includes("actionClassName={isGenericLedgerTradeEntry ? '!h-[48px] !rounded-[15px] !border-white/[0.13] !bg-transparent !text-white/[0.66]' : ''}"), 'generic buy and sell actions should share one compact neutral style');
...
test('approved modal families share the new shell without widening business boundaries', () => {
  ...
  const assetTrendPageEnd = analysisTabSource.indexOf('\n\n  return (', assetTrendPageStart);
  const assetTrendPageBlock = analysisTabSource.slice(assetTrendPageStart, assetTrendPageEnd);
  const monthlyBalanceEditorStart = analysisTabSource.indexOf('const openMonthlyBalanceEditor = React.useCallback');
  const monthlyBalanceEditorEnd = analysisTabSource.indexOf('\n\n  const closeAccountAction', monthlyBalanceEditorStart);
  const monthlyBalanceEditorBlock = analysisTabSource.slice(monthlyBalanceEditorStart, monthlyBalanceEditorEnd);
  ...
  assert.ok(assetTrendPageBlock.includes('<MonthlyAssetTrendContent') && assetTrendPageBlock.includes('onEditMonth={openMonthlyBalanceEditor}'), 'asset month trend page should use the approved analysis content and exact-month edit handoff');
  ...
  assert.ok(analysisTabSource.includes('setFillMonth(month);') && analysisTabSource.includes('setSnapshotDraft({});') && analysisTabSource.includes('setShowFillSnapshot(true);'), 'asset month rows should open the existing monthly balance editor for the selected month');
  ...
  assert.ok(monthlyAssetTrendContentSource.includes('data-asset-trend-month-row={slot.month}') && monthlyAssetTrendContentSource.includes('data-asset-trend-month-edit={slot.month}'), 'monthly detail rows should expose a dedicated exact-month edit control');
  const monthlyAssetRowStart = monthlyAssetTrendContentSource.indexOf('<div\n                key={slot.month}\n                data-asset-trend-month-row={slot.month}');
  const monthlyAssetArrowStart = monthlyAssetTrendContentSource.indexOf('data-asset-trend-month-edit={slot.month}', monthlyAssetRowStart);
  const monthlyAssetRowBeforeArrow = monthlyAssetTrendContentSource.slice(monthlyAssetRowStart, monthlyAssetArrowStart);
  assert.ok(monthlyAssetRowStart > -1 && monthlyAssetArrowStart > monthlyAssetRowStart && !monthlyAssetRowBeforeArrow.includes('onClick='), 'monthly detail rows should stay inert until their small arrow is pressed');
  assert.ok(monthlyAssetTrendContentSource.includes('data-asset-trend-month-edit={slot.month}') && monthlyAssetTrendContentSource.includes('onClick={() => onEditMonth?.(slot.month)}'), 'the monthly detail arrow should hand off its exact month');
  assert.ok(monthlyBalanceEditorStart > -1 && monthlyBalanceEditorEnd > monthlyBalanceEditorStart && !monthlyBalanceEditorBlock.includes('closeMonthlyAssetTrend'), 'opening a monthly balance from the trend page should preserve that page behind the editor');
  assert.ok(analysisTabSource.includes('{showMonthsDetail ? monthlyAssetTrendPage : (') && analysisTabSource.includes('      )}\n\n      {showFillSnapshot && ('), 'the monthly balance editor should render above either asset page so closing or saving returns to the originating page');
