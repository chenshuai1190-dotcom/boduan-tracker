...
test('main trade entry modal uses compact four-step buy sell submission flow', () => {
  ...
  assert.ok(settingsTabSource.includes('v10.7.9.209'), 'settings version badge should document the fixed index card slots update');
  assert.ok(settingsChangelogSource.includes('v10.7.9.209'), 'settings changelog should document the fixed index card slots update');
  assert.ok(settingsChangelogSource.includes('三大指数固定卡位'), 'settings changelog should describe the fixed index card slots update');
  ...
  assert.ok(homeTabSource.includes("item.maskPrice ? '--' : fmtMoney(item.price, 2)"), 'home holdings should render -- only for stale current prices');
  ...
  assert.ok(tradesTabSource.includes("maskCurrentPrice ? '--' : fmtAmount(position.currentPrice, 3)"), 'trade holdings should render -- only for stale current prices');
