...
test('community competition is an isolated authenticated close-snapshot utility', () => {
  ...
  assert.ok(communityCompetitionPageSource.includes("tt('competition.nasdaq100', '纳指100基准（QQQ）')"), 'competition UI should identify the actual QQQ benchmark');
  ...
  assert.ok(communityCompetitionPageSource.includes("tt('competition.dataDisclosure', '收益基于用户正式交易记录与服务端收盘价快照，不代表券商认证。')"), 'ready and waiting states should disclose the snapshot source and lack of broker verification');
  ...
  assert.ok(i18nSource.includes("'competition.dataDisclosure': '收益基于用户正式交易记录与服务端收盘价快照，不代表券商认证。'"), 'Chinese disclosure should remain translated centrally');
...
test('main trade entry modal uses compact four-step buy sell submission flow', () => {
  ...
  assert.equal((settingsTabSource.match(/v10\.7\.9\.303/g) || []).length, 3, 'all three visible settings version surfaces should stay synchronized');
  assert.ok(settingsChangelogSource.includes("ver: 'v10.7.9.303', date: '2026-07-12', latest: true"), 'latest changelog entry should match the visible settings version');
...
test('asset and review module cards do not keep legacy scale interactions', () => {
  ...
  assert.equal((settingsTabSource.match(/v10\.7\.9\.303/g) || []).length, 3, 'settings version surfaces should document the current real competition release');
  assert.ok(settingsChangelogSource.includes('v10.7.9.218'), 'settings changelog should document the P&L report period stats update');
  assert.ok(settingsChangelogSource.includes('收益报表周期统计'), 'settings changelog should describe the P&L report period stats update');
  assert.ok(settingsChangelogSource.includes('v10.7.9.217'), 'settings changelog should document the P&L calendar visual update');
  assert.ok(settingsChangelogSource.includes('收益日历视觉优化'), 'settings changelog should describe the P&L calendar visual update');
...
test('review target page uses dark mobile cards and click action modals', () => {
  ...
  assert.ok(tradesTabSource.includes('fmtAmount(marketValue, 2)'), 'trade position market value should keep two decimal places like daily and holding pnl');
  assert.equal((settingsTabSource.match(/v10\.7\.9\.303/g) || []).length, 3, 'settings version surfaces should remain synchronized at the current local version');
  assert.ok(settingsChangelogSource.includes('v10.7.9.218'), 'settings changelog should document the P&L report period stats update');
  assert.ok(settingsChangelogSource.includes('收益报表周期统计'), 'settings changelog should describe the P&L report period stats update');
  assert.ok(settingsChangelogSource.includes('v10.7.9.217'), 'settings changelog should document the P&L calendar visual update');
  assert.ok(settingsChangelogSource.includes('收益日历视觉优化'), 'settings changelog should describe the P&L calendar visual update');
