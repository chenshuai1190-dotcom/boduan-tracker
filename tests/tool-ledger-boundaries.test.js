...
test('main trade entry modal uses compact four-step buy sell submission flow', () => {
  ...
  assert.ok(settingsTabSource.includes('v10.7.9.181'), 'settings version badge should document the trade modal input outline update');
  assert.ok(settingsChangelogSource.includes('v10.7.9.181'), 'settings changelog should document the trade modal input outline update');
...
test('asset and review module cards do not keep legacy scale interactions', () => {
  ...
  assert.ok(settingsTabSource.includes('v10.7.9.181'), 'settings version badge should document the latest trade modal input outline update');
  assert.ok(settingsChangelogSource.includes('v10.7.9.181'), 'settings changelog should document the latest trade modal input outline update');
  assert.ok(settingsChangelogSource.includes('交易录入输入框去白框'), 'settings changelog should describe the latest trade modal input outline update');
  assert.ok(settingsChangelogSource.includes('iOS 主屏幕恢复刷新加固'), 'settings changelog should retain the iOS PWA resume quote refresh update');
  assert.ok(settingsChangelogSource.includes('行情请求禁用浏览器缓存'), 'settings changelog should retain the no-cache quote refresh update');
  assert.ok(settingsChangelogSource.includes('股票行情即时刷新'), 'settings changelog should retain the quick quote refresh update');
  assert.ok(settingsChangelogSource.includes('EODHD 股票价格口径统一'), 'settings changelog should retain the EODHD quote basis update');
