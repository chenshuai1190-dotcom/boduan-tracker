...
test('approved modal families share the new shell without widening business boundaries', () => {
  ...
  assert.ok(assetCategoryReportPageStart > assetTrendPageStart && assetCategoryReportPageEnd > assetCategoryReportPageStart, 'monthly category changes should have a dedicated nested page boundary');
  ...
  assert.ok(assetTrendPageBlock.includes('<MonthlyAssetTrendContent') && assetTrendPageBlock.includes('onOpenMonthReport={openMonthlyAssetCategoryReport}'), 'asset month trend page should hand the exact selected month to the category report');
  ...
  assert.ok(assetCategoryReportPageBlock.includes('data-monthly-asset-category-report-page="true"') && assetCategoryReportPageBlock.includes('<MonthlyAssetCategoryReport'), 'the category report should render as a full-width nested asset page');
  assert.ok(assetCategoryReportPageBlock.includes('onClick={closeMonthlyAssetCategoryReport}') && assetCategoryReportPageBlock.includes("analysis.backToMonthlyAssetTrend"), 'the category report should return to the monthly trend page');
  assert.equal(assetCategoryReportPageBlock.includes('edit') || assetCategoryReportPageBlock.includes('Edit') || assetCategoryReportPageBlock.includes('修改'), false, 'the category report header should not expose an edit action');
  assert.equal(monthlyAssetCategoryReportSource.includes('onEdit') || monthlyAssetCategoryReportSource.includes('setShowFillSnapshot') || monthlyAssetCategoryReportSource.includes('db.'), false, 'the category report content must remain read-only');
  ...
  assert.ok(monthlyAssetTrendContentSource.includes('onClick={() => onOpenMonthReport?.(slot.month)}'), 'clicking anywhere on a monthly detail row should open its exact category report');
