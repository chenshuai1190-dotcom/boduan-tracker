...
test('asset page visual shell and local preview stay debuggable', () => {
  ...
  assert.ok(analysisTabSource.includes('const overviewChartReading = requestedChartSlot?.hasData') && analysisTabSource.includes('overviewChartModel.slots[overviewChartLatestIndex] || null'), 'the persistent chart reading should use the selected valid month or the latest recorded month');
  ...
  assert.ok(analysisTabSource.includes('data-asset-chart-month={selectedChartMonth}') && analysisTabSource.includes('selectedChartMoney.main') && analysisTabSource.includes('selectedChartMoney.decimal') && analysisTabSource.includes("splitCurrencyAmount(Math.abs(selectedChartChange), 'CNY', 2)") && analysisTabSource.includes('selectedChartChangeMoney.decimal'), 'the persistent reading should show the selected month and preserve two-decimal precision in both the amount and split change value');
  assert.doesNotMatch(analysisTabSource, /className="asset-report-selection"|\{selectedChartValue > 0 && \(/, 'the chart reading should not return to a conditionally inserted tooltip card');
  ...
  assert.ok(analysisTabSource.includes('connectGaps') && analysisTabSource.includes('showSelectedLabel={false}'), 'asset overview should preserve connected history with its separate persistent reading');
  ...
  assert.match(analysisTabSource, /ref=\{overviewChartInteractionRef\} className="asset-report-chart-explorer">\s*<div className="asset-report-chart-reading"[\s\S]*?<MonthlyAssetTrendChart/, 'the persistent reading and chart should share the outside-touch interaction boundary');
  assert.ok(analysisTabSource.includes("document.addEventListener('pointerdown', clearSelectedMonthOutsideOverviewChart, true)") && analysisTabSource.includes('if (overviewChartInteractionRef.current?.contains(event.target)) return;'), 'touching outside the chart and reading should reset selection to the latest recorded month');
...
test('Home, Trading, and Assets use continuous scoped reports', () => {
  ...
  assert.match(analysisTabCss, /\.asset-report-chart-amount,\s*\.asset-report-total\s*\{[^}]*font-size:\s*clamp\(28px,\s*8\.8vw,\s*40px\);[^}]*font-weight:\s*400;[^}]*line-height:\s*1\.18;/, 'family total and selected-month amount should share the responsive Home number hierarchy');
