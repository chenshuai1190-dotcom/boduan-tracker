...
test('main trade entry modal uses compact four-step buy sell submission flow', () => {
  ...
  assert.ok(settingsTabSource.includes('v10.7.9.203'), 'settings version badge should document the BTC iOS PWA connection recovery');
  assert.ok(settingsChangelogSource.includes('v10.7.9.203'), 'settings changelog should document the BTC iOS PWA connection recovery');
...
test('realtime quote refresh avoids duplicate requests and hides raw Safari network errors', () => {
  ...
  assert.ok(appSource.includes('IOS_PWA_REALTIME_SNAPSHOT_INTERVAL_MS = 2500'), 'iOS standalone stock and index mode should use fast authenticated snapshot polling');
