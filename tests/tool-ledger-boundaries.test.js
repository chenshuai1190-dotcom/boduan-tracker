...
test('main trade entry modal uses compact four-step buy sell submission flow', () => {
  ...
  assert.ok(settingsTabSource.includes('v10.7.9.204'), 'settings version badge should document the iOS PWA active snapshot speedup');
  assert.ok(settingsChangelogSource.includes('v10.7.9.204'), 'settings changelog should document the iOS PWA active snapshot speedup');
...
test('realtime quote refresh avoids duplicate requests and hides raw Safari network errors', () => {
  ...
  assert.equal(appSource.includes("fetchRealtimeSnapshot('/api/btc-realtime')"), false, 'iOS standalone mode should keep BTC on WebSocket instead of snapshot polling');
