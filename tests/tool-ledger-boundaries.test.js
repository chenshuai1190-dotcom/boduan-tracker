...
test('main trade entry modal uses compact four-step buy sell submission flow', () => {
  ...
  assert.ok(settingsTabSource.includes('v10.7.9.179'), 'settings version badge should document the iOS PWA resume quote refresh update');
  assert.ok(settingsChangelogSource.includes('v10.7.9.179'), 'settings changelog should document the iOS PWA resume quote refresh update');
  assert.ok(settingsChangelogSource.includes('iOS 主屏幕秒级恢复刷新'), 'settings changelog should describe the iOS PWA resume quote refresh update');
...
test('realtime quote refresh avoids duplicate requests and hides raw Safari network errors', () => {
  ...
  assert.ok(appSource.includes('IOS_PWA_RESUME_STALE_MS = 5000'), 'iOS PWA resume should treat frozen foreground heartbeats as stale quickly');
  assert.ok(appSource.includes('foregroundHeartbeatAtRef'), 'iOS PWA resume should track a foreground heartbeat');
  ...
  assert.ok(appSource.includes("trigger: pendingTrigger === 'auto-ios-touch-resume' ? pendingTrigger : 'auto-ios-resume-cloud'"), 'pending iOS PWA resume refreshes should flush after cloud data is ready');
...
test('asset and review module cards do not keep legacy scale interactions', () => {
  ...
  assert.ok(settingsTabSource.includes('v10.7.9.179'), 'settings version badge should document the latest iOS PWA resume quote refresh update');
  assert.ok(settingsChangelogSource.includes('v10.7.9.179'), 'settings changelog should document the latest iOS PWA resume quote refresh update');
  assert.ok(settingsChangelogSource.includes('iOS 主屏幕秒级恢复刷新'), 'settings changelog should describe the latest iOS PWA resume quote refresh update');
  assert.ok(settingsChangelogSource.includes('行情请求禁用浏览器缓存'), 'settings changelog should retain the no-cache quote refresh update');
  assert.ok(settingsChangelogSource.includes('股票行情即时刷新'), 'settings changelog should retain the quick quote refresh update');
  assert.ok(settingsChangelogSource.includes('EODHD 股票价格口径统一'), 'settings changelog should retain the EODHD quote basis update');
