...
test('positions columns preserve precise widths and horizontally accessible financial text', () => {
  ...
  assert.deepEqual(firstFour, [70, 108, 74, 102], 'market value, current price and daily P&L each gain another 1px; the identity width is unchanged');
  assert.equal(firstFour.reduce((sum, width) => sum + width, 0), 354, 'the four column minimums gain exactly another 3px in total');
