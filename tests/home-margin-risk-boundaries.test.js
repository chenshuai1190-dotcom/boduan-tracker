...
const marginPageCss = source('../src/pages/HomeMarginRiskPage.css');
...
test('Home margin risk uses a standalone page while keeping the bottom navigation', () => {
  ...
  const bottomNavGuard = appSource.match(/const hideBottomNavigation = ([^;]+);/)?.[1];
  for (const page of ['isPnlReportPage', 'isPnlSharePage', 'isStockPnlReportPage', 'isDebtManagerPage']) {
    assert.ok(bottomNavGuard?.includes(page), `${page} should hide the bottom tabs`);
  }
  assert.doesNotMatch(bottomNavGuard, /\bisHomeMarginRiskPage\b/, 'the margin page must keep the bottom navigation');
  ...
  assert.match(marginPageCss, /\.margin-report-sheet\s*\{[^}]*background:\s*#101112;/, 'the leverage guide and balance editor should use the opaque neutral report surface');
  assert.equal(marginPageSource.includes('radial-gradient'), false, 'the report sheet should not restore the retired decorative gradient');
